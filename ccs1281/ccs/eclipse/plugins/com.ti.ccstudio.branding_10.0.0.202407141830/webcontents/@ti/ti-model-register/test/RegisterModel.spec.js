...
import { RegisterModelEncoderType, RegisterModelDecoderType, RegisterModel } from '../lib/RegisterModel';
import { TiPromise } from '../../ti-core-assets/lib/TiPromise';
import { CodecRegistry } from '../../ti-target-configuration/lib/CodecRegistry';
import { ConstantBindValue, statusChangedEventType, scriptLogEventType, Status, valueChangedEventType, streamingDataEventType } from '../../ti-core-databind/lib/CoreDatabind';
...
describe('RegisterModel', () => {
    ...
    ;
    function getBinding(model, uri) {
        return model.getBinding(uri) || new ConstantBindValue();
    }
    class TestCodec extends Events {
        constructor() {
            super(...arguments);
        ...
        ;
        writeValue(entity, value, coreIndex) {
            this.entity = entity;
            this.coreIndex = coreIndex;
            this.value = value;
    ...
    class ChangeListener {
        ...
        ;
        verifyDataReceivedWas(newData) {
            expect(this.didReceiveData).to.be.true;
            expect(this.dataReceviced).to.equal(newData);
            this.didReceiveData = false;
        }
        ;
        verifyStatusChangedTo(newStatus) {
            const message = (newStatus && newStatus.message) ? newStatus.message : (newStatus || '');
    ...
    ;
    let scriptingData = undefined;
    function scriptingListener(details) {
        scriptingData = details;
    }
    ;
    function createRegisterJsonData(addr1 = '0x00', start = 8, addr2 = 0x3, enableCalculated = true) {
        const regJsonInfo = {
            info: {
                name: 'test'
            },
    ...
    for (let i = 0; i < modes.length; i++) {
        ...
        it('configure' + mode, () => {
            ...
            refreshBind = getBinding(model, '$refresh_interval');
        ...
        it('Stale bindings' + mode, async () => {
            ...
            reg2Bind = getBinding(model, 'reg2');
            expect(reg2Bind.status).to.be.null;
            expect(reg2Bind.isStale()).to.be.false;
            reg2FieldBind = getBinding(model, 'reg2.field');
            expect(reg2Bind.isStale()).to.be.false;
    ...
    let activeCore;
    let cores2reg2FieldBind;
    let cores2regBind;
    let cores2reg2Bind;
    let coresLength;
    let coresAllregBind;
    let coresAllreg2Bind;
    let coresAllRegField3Bind;
    let cores1regBind;
    it('RegisterAllBind Read', async () => {
        ...
        coresAllreg2Bind = getBinding(model, '$cores.all.reg2');
    ...
    it('Read Core Specific Register', async () => {
        ...
        cores2regBind = getBinding(model, '$cores.2.reg');
        ...
        cores2reg2Bind = getBinding(model, '$cores.2.reg2');
...
//# sourceMappingURL=RegisterModel.spec.js.map