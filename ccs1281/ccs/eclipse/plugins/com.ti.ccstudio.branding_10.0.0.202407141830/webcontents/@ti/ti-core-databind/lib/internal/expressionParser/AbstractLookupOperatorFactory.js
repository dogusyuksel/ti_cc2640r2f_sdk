...
export class AbstractLookupOperatorFactory {
    static testLookupBinding(lookupBinding, operator) {
        ...
        if (endPos >= 0) {
            if (endPos === uri.length - 1) {
                ...
                if (pos === 0) {
                    ...
                    if (lookupBinding) {
                        AbstractLookupOperatorFactory.testLookupBinding(lookupBinding, this.operator); // throw exception if invalid binding found.
                        const indexBindings = [];
