...
export class AbstractBinaryOperator extends Events {
    ...
    setValue(value, progress, forceWrite) {
        if (!this.readOnly) {
            try {
                // call doSetValue(newValue, isLeftParamConstand, constiableParam, constantValue)
    ...
    ;
    setDeferredMode(deferredMode, forceWrite = false) {
        this.leftOperand.setDeferredMode(deferredMode, forceWrite);
        this.rightOperand.setDeferredMode(deferredMode, forceWrite);
    }
    ;
    getValueCommitted() {
