...
const FileDownload = class {
    constructor(hostRef) {
        registerInstance(this, hostRef);
        /**
         * color to be passed down to the ti-button instance
         */
        this.appearance = 'primary';
    }
    /**
     * Internal copy of downloadjs 4.21
     * Allows us to handle creating a file for users to download.
     * MIT license.
     * @param window
     */
    _downloadjs(window) {
        // download.js v4.21, by dandavis; 2008-2018. [MIT] see http://danml.com/download.html for tests/usage
        return (data, strFileName, strMimeType) => {
            ...
            if (url && url.length < 2048) { // if no filename and no mime, assume a url was passed as the only argument
                ...
                if (anchor.href.indexOf(url) !== -1) { // if the browser determines that it's a potentially valid url path:
                    ...
                    ajax.onload = () => { this._downloadjs(window)(ajax.response, fileName); };
    ...
    _handleFileDownload() {
        ...
        x.open('GET', this.fileSrc, true);
        x.responseType = 'blob';
        x.onload = () => { this._downloadjs(window)(x.response, this.fileName, this.fileMime); };
        x.send();
    }
    render() {
        return (h(Host, { onClick: (ev) => { ev.preventDefault(); this._handleFileDownload(); } }, h("slot", null)));
    }
    get hostElement() { return getElement(this); }
};

export { FileDownload as ti_file_download };
