...
(function (ResponseKind) {
    ResponseKind[ResponseKind["Init"] = 0] = "Init";
    ResponseKind[ResponseKind["CloudAgentPort"] = 1] = "CloudAgentPort";
    ResponseKind[ResponseKind["LogMessage"] = 2] = "LogMessage";
    ResponseKind[ResponseKind["Connected"] = 3] = "Connected";
    ResponseKind[ResponseKind["ModuleMessage"] = 4] = "ModuleMessage";
    ResponseKind[ResponseKind["Closed"] = 5] = "Closed";
...
function setupSynchronousClient(cloudAgentDir, inProcess = false, TimeoutError = ModuleTimeoutError) {
    // Messaging scheme:
    ...
    const workerData = {
        ...
        requestReceiver,
        responseSender,
        cloudAgentDir: path.resolve(cloudAgentDir),
        inProcess,
    };
    ...
    function receive(timeoutAt) {
        ...
        if (response) {
            ...
            if (response.kind === ResponseKind.LogMessage) {
                ...
                if (response.level === LogLevel.Error) {
    ...
    function createModule(port, subProtocol) {
        const subModules = {};
        const listeners = {};
        ...
        function createSubModule(name) {
            ...
            const subModule = createModule(port, subProtocol);
            subModule.addListener("close", () => { delete subModules[name]; });
            return subModule;
        }
        ...
        for (;;) {
            ...
            if (response?.kind === ResponseKind.CloudAgentPort) {
                ...
                if ("port" in data) {
                    ...
                    throw new Error("Failed to start CloudAgent: " + data.error);
                }
            }
        }
    }
    ...
    // The worker thread will prevent node from closing
    process.on("exit", shutdown);
    // Wait for response indicating that worker thread initialized properly
    const timeoutAt = getTimeoutTime();
    for (;;) {
        const response = receive(timeoutAt);
        if (response?.kind === ResponseKind.Init) {
            const { error } = response;
            if (error) {
                logger.error(`Error occurred during setup: ${error}`);
                shutdown();
    ...
    return { launchCloudAgent, createModule, waitForEvent, getTimeout, setTimeout, shutdown, sleep };
}
exports.setupSynchronousClient = setupSynchronousClient;
function runWebsocketThread() {
    const { sharedBuffer, requestReceiver, responseSender, cloudAgentDir, inProcess } = worker_threads_1.workerData;
    const responsesPending = new Int32Array(sharedBuffer, 0, 1);
    function send(msg) {
    ...
    try {
        if (inProcess) {
            // Assume that cloudAgent is in ccs_base
            socketFunc = getInProcessSocket(path.join(cloudAgentDir, ".."));
        }
        else {
            socketFunc = requireCloudAgentNodeModule(cloudAgentDir, "ws");
        }
    }
    catch (err) {
        send({ kind: ResponseKind.Init, error: err.message });
    }
    const modules = {};
        ...
        switch (kind) {
            ...
            case RequestKind.LaunchCloudAgent:
                if (inProcess) {
                    ...
                    startCloudAgent(cloudAgentDir).then((port) => send({ kind: ResponseKind.CloudAgentPort, data: { port } }), (err) => send({ kind: ResponseKind.CloudAgentPort, data: { error: err.message } }));
                }
                break;
            default:
                assertUnreachable(kind);
    ...
    function requireCloudAgentNodeModule(cloudAgentDir, module_name) {
        return require(path.join(cloudAgentDir, "src", "node_modules", module_name));
    }
    ...
    function startCloudAgent(cloudAgentDir) {
        return new Promise((resolve, reject) => {
            if (!path.isAbsolute(cloudAgentDir)) {
                cloudAgentDir = path.join(process.cwd(), cloudAgentDir);
            }
            ...
            try {
                const node = path.join(cloudAgentDir, "node");
                const script = path.join(cloudAgentDir, "src", "main.js");
    ...
    // Indicate that we fully initialized
    send({ kind: ResponseKind.Init });
}
if (!worker_threads_1.isMainThread && !global.syncAgentMainThread) {
    runWebsocketThread();
}
