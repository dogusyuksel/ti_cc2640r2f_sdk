...
/*---------------------------------------------------------------------------
 * This is mklib version unknowable for MSP430 compiler  
...
const char *isa = "MSP430";
const char *version = "unknowable";
int         buildmodel = 2011;
const char *c_dir_name = "MSP430_C_DIR";

#define MAX_PREDICATE 6
...
library_t LIBRARIES[] = {
    { "rts430x_sc_sd_eabi_se_eh.lib", { "EXCEPTIONS","SMALL_DATA","MSPX","SMALL_ENUM","SMALL_CODE","EABI_TDEH" } },
    { "rts430x_lc_sd_eabi_se_eh.lib", { "EABI_TDEH","SMALL_ENUM","MSPX","EXCEPTIONS" } },
    { "rts430x_lc_sd_eabi_se.lib", { "MSPX","SMALL_ENUM" } },
    { "rts430x_lc_rd_eabi_se_eh.lib", { "EABI_TDEH","MSPX","SMALL_ENUM","EXCEPTIONS","RESTRICTED_DATA" } },
    { "rts430x_sc_sd_eabi_se.lib", { "SMALL_CODE","SMALL_ENUM","MSPX","SMALL_DATA" } },
    { "rts430x_lc_ld_eabi_eh.lib", { "EXCEPTIONS","MSPX","EABI_TDEH","LARGE_DATA" } },
    { "rts430x_sc_sd_eabi.lib", { "SMALL_DATA","MSPX","SMALL_CODE" } },
    { "rts430_eabi_se.lib", { "SMALL_ENUM" } },
    { "rts430_eabi.lib", {  } },
    { "rts430x_lc_ld_eabi_se.lib", { "LARGE_DATA","MSPX","SMALL_ENUM" } },
    { "rts430x_lc_rd_eabi.lib", { "MSPX","RESTRICTED_DATA" } },
    { "rts430x_lc_rd_eabi_se.lib", { "RESTRICTED_DATA","MSPX","SMALL_ENUM" } },
    { "rts430_eabi_eh.lib", { "EXCEPTIONS","EABI_TDEH" } },
    { "rts430x_lc_rd_eabi_eh.lib", { "EXCEPTIONS","RESTRICTED_DATA","EABI_TDEH","MSPX" } },
    { "rts430x_sc_sd_eabi_eh.lib", { "EABI_TDEH","MSPX","SMALL_CODE","SMALL_DATA","EXCEPTIONS" } },
    { "rts430x_lc_ld_eabi_se_eh.lib", { "EXCEPTIONS","EABI_TDEH","LARGE_DATA","MSPX","SMALL_ENUM" } },
    { "rts430x_lc_sd_eabi_eh.lib", { "EXCEPTIONS","EABI_TDEH","MSPX" } },
    { "rts430_eabi_se_eh.lib", { "EABI_TDEH","SMALL_ENUM","EXCEPTIONS" } },
    { "rts430x_lc_sd_eabi.lib", { "MSPX" } },
    { "rts430x_lc_ld_eabi.lib", { "LARGE_DATA","MSPX" } },
};

const char *OBJ_SUBDIRS[] = { "libcxx","libcxx/ext","libcxx/include","libcxx/support/runtime","libcxx/support/ti","machine","stlport","sys","xlocale" };
...
/*===========================================================================*/
/* Forward declarations of not-quite-so-system-specific functions            */
/*===========================================================================*/
...
/*---------------------------------------------------------------------------*/
/* Recursively make a directory named pathname. This function will make      */
/* every directory required to make the specified pathname. For instance     */
/* foo/bar will make foo first, then bar.                                    */
/*---------------------------------------------------------------------------*/
bool make_directory(const string &pathname)
{
   string tmp = pathname;
   ...
   string::size_type pos, start_pos = 1;
   while ((pos = pathname.find('/', start_pos)) != string::npos)
   {
      string tmp = pathname.substr(0, pos);
      start_pos = pos+1;

      if (!directory_exists(tmp))
         if (mkdir(tmp.c_str(), 0775) != 0)
            return false;
   }
...
FILE* fopen_or_die(const string& filename, const char* mode)
{
                       ...
                       const string &dst_dir)
{
    ...
    /*------------------------------------------------------------------------*/
    /* Make every subdirectory that might be needed.                          */
    /*------------------------------------------------------------------------*/
    for (int i=0; i < sizeof(OBJ_SUBDIRS)/sizeof(const char*); i++)
    {
       string obj_subdir = splice(obj_dir, OBJ_SUBDIRS[i]);
       if (verbose || dryrun) printf("mkdir %s\n", obj_subdir.c_str());
       if (logfile) fprintf(logfile, "mkdir %s\n", obj_subdir.c_str());
       if (!dryrun && !directory_exists(obj_subdir))
          make_directory_or_die(obj_subdir);
    }
    ...
    fprintf(gmake_cmd_file, "MV_CMD=%s\n",  find_program_or_die("mv").c_str());
