 ...
 * Make tirex core
 * @param {Object} args
 *  @param {module:scripts/build~BuildType} args.buildType
...
exports.make = function({buildType, platform, arch, nodeVersion},
    ...
    const dst = path.join(src, 'target', buildType, 'tirex3', 'ti-rex-core');
 ...
 * @param {string} src - The source directory.
 * @param {string} dst - The destination directory.
 * @param {module:scripts/build~BuildInfo} config - The config values.
 * @param {ErrorCallback} callback
 */
 ...
 * @param {Error} error
 * @param {Array.<module:scripts/build~UglifiedFileData>} files
 */

/**
 * @param {String} src
 * @param {String} dst
 * @param {module:scripts/build~UglifiedFiles} callback
 */
 ...
 * @param {module:scripts/build~BuildInfo} config
 * @param {ErrorCallback} callback
 */
function getBinaries(src, dst, {platform, arch, nodeVersion}, callback, production=false) {
    async.series([(callback) => {
        ...
        if (production) {
            ...
            async.map(devBinaries, (devBinary, callback) => {
                async.parallel([
                    callback => fs.remove(path.join(dst, 'bin', devBinary), callback),
                    callback => fs.remove(path.join(dst, '..', devBinary), callback)
                ], callback);
            }, callback);
        } else {
...
function getModules(src, dst, {platform,
    ...
    _async.series([(callback) => {
        ...
        _fs.emptyDir(path.join(dst, 'node_modules'), callback);
    }, (callback) => {
