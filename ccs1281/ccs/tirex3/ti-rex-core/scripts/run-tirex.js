...
function setValues(serverMode, callback, {debug, 
    ...
    // Note: some of these config files are not in production, so you can't use all configurations in production unless you add the config file to config/
    const configFolder = production ? 'config' : 'config-develop';

    let dinfraConfig = '';
    if (serverMode === 'localserver') {
        dinfraConfig = 'dinfra-desktop/dinfra.js _';
    }
    else if (!dinfra) {
        dinfraConfig = 'dinfra-desktop/dinfra.js _';
    }
    else {
        dinfraConfig = `${dinfra} ${configFolder}/dconfig_auser.json`;
    }

    let remoteServerConfig = null;
    if (serverMode === 'remoteserver') {
        if (handoffServer) {
            remoteServerConfig = `${configFolder}/custom/app_content_developer.json`;
        }
        else {
    ...
    if (serverMode === 'localserver') {
        if (isWindows()) {
            localServerConfig = `${configFolder}/app_localserver_win.json`;
        }
        else {
            localServerConfig = `${configFolder}/app_localserver.json`;
        }
    }
