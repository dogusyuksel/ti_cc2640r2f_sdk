 ...
 *  @param {String} args.ccsUrl
 *  @param {Array.String} args.packages - The set of packages we want to test.
 *  @param {stream.Writable} args.out - Where to output.
 * @param {module:scripts/ccs-dependent/test-projects~testProjectsCallback} callback
 */
exports.testProjects = function({tirexUrl, ccsUrl, packages=null, out=process.stdout}, callback) {
    { // process args
    ...
    async.waterfall([(callback) => {
        ...
        async.waterfall([(callback) => {
            async.mapSeries(args.importables, (importable, callback) => {
                async.waterfall([(callback) => {
                    util.testProject({
                        importable, ccsUrl: ccsUrl, logStream: args.logStream
                    }, callback);
                }, (result, callback) => {
                    if (result.failed > 0) {
...
exports.describe = 'Test all the projects in tirex in a desktop environment';
exports.builder = {
    tirexUrl: {
        alias: 't',
    ...
    ccsUrl: {
        alias: 'c',
        describe: 'The url of the desktop ccs to connect to',
        demand: true
    },
    packages: {
        alias: 'p',
...
exports.handler = function(argv) {
    exports.testProjects(argv, (err, {total, failed, error, log}={}) => {
        if (err) {
	    console.log(err);
            process.exit(1);
	}
