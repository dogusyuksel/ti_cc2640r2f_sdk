...
for my $element (@$data) {
  ...
  #search for .c file in the folder
	...
	#print "DIR $package_base_dir"."$link\n";
	#opendir(DIR, "$package_parent_dir"."$link") or die $!;
	#print "DIR $package_base_dir"."$db_link\n";
	opendir(DIR, "$package_base_dir"."$db_link") or die $!;
        my @file_array = ();
        my $file_count = 0;
		...
		if( $file =~ m/\.[ch]$/){
		  #since we are handling IAR .ipcf like a projectspec rather than a project folder we do not need to include files that are in the folder
		...
		if( $file =~ m/(.+)\.ipcf$/){
		  ...
		  $proj_fname = $1;
                  ...
                  $package_name =~ s/\\/\//g;
                  #assuming 3.0 spec 
	          $package_name =~ s/\.metadata[\/\\]\.tirex[\/\\]//g;  

		  $iarmacros = {
                     PROJ_DIR => '',
                     PROJ_FNAME => $proj_fname,
		     SDK_INSTALL_DIR => "$ARGV[0]/$package_name",
		     SIMPLELINK_MSP432_SDK_INSTALL_DIR => "$ARGV[0]/$package_name",
		     SIMPLELINK_CC13XX_CC26XX_SDK_INSTALL_DIR => "$ARGV[0]/$package_name",
		     SIMPLELINK_CC32XX_SDK_INSTALL_DIR => "$ARGV[0]/$package_name"
		  };

		  #print "====SIMPLELINK_MSP432_SDK_INSTALL_DIR===$iarmacros->{SIMPLELINK_MSP432_SDK_INSTALL_DIR}\n";

		  #exit;

	          #print "IPCF $package_base_dir"."$db_link$SL"."$file\n";
	          #open(IPCF, "$package_parent_dir"."$link$SL"."$file") or die $!;
	          open(IPCF, "$package_base_dir"."$db_link$SL"."$file") or die $!;
                  my $files_section = 0;
		  ...
		  while(<IPCF>){
			 ...
			 while($copy_to =~ /\$(.+?)\$/){  
			    $macro = $1;
			    if( $macro =~ /SDK_INSTALL_DIR/ ){
                              $copy_to = $copy_to =~ s/\$(.+?SDK_INSTALL_DIR)\$/$iarmacros->{SDK_INSTALL_DIR}/r;  
			      #print "SID CT $copy_to\n";
			    }
			    else{
			 ...
			 while($copy_from =~ /\$(.+?)\$/){  
			    $macro = $1;
			    if( $macro =~ /SDK_INSTALL_DIR/ ){
                              $copy_from = $copy_from =~ s/\$(.+?SDK_INSTALL_DIR)\$/$iarmacros->{SDK_INSTALL_DIR}/r;  
			      #print "SID CF $copy_from\n";
			    }
			    else{
			 ...
			 #We intentionally don't strip out the leading slash, could be root

			 #$proj_dir = "$package_parent_dir"."$link/";
			 $proj_dir = "$package_base_dir"."$db_link$SL";
			 ...
			 if(-d $copy_from){
		   	 ...
		   	 # the below few lines is to handle the File::Spec->abs2rel behavior being different on linux
			 if($proj_dir =~ /.metadata..tirex.\.\..\.\../){
				$proj_dir = "$`$'";
			 	#print "proj_dir has .metadata/.tirex/../../\n";
				#print "$proj_dir\n";
			 }


			 $relative_path = File::Spec->abs2rel(  $copy_from, $proj_dir );
			 #print "\nFROM  $copy_from\nTO    $proj_dir\n====$relative_path\n";

                         $file_array[$file_count++] = "+$relative_path/$copy_from_file -> $copy_to";
			 #print "+$relative_path/$copy_from_file --> $copy_to\n";
	 ...
	 system( "mkdir \"$dependency_dir$SL.dependencies\""); 
      ...
      while(<IN>){
	    ...
	    #print "relative_todir $relative_todir\n";
            ...
            $info_type = "buildflags";
	    $visibility = "-";
	    #we don't want to put out "-" any more. We don't have partial and incase REX-1417 is still an issue
            next; 
         }
