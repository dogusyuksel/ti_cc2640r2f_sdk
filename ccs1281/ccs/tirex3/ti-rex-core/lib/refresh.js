...
class RefreshManger {
     ...
     */
    constructor({dbs}) {
        this._contentPackagesConfigFile = vars.CONTENT_PACKAGES_CONFIG;
        this._dbs = dbs;
    ...
    _refreshTask({log, packageNames = null, refreshAll = true, clearAllData = true}, callback) {
        ...
        async.waterfall([
            ...
            if (err) {
                log.userLogger.error('Done: Database was not refreshed');
            }
            callback(err);
        });
    }
    ...
    _refreshAll({log, clearAllData}, callback) {
        ...
        dbBuilderUtils.readJsonWithComments(this._contentPackagesConfigFile, (err, packages) => {
            if (err) {
            ...
            async.map(packageObjects, (pkg, callback) => {
                fs.stat(path.join(vars.CONTENT_BASE_PATH, pkg.path), (err, stats) => {
                    if (err) {
                        log.userLogger.error('Package path not found: ' + path.join(vars.CONTENT_BASE_PATH, pkg.path));
