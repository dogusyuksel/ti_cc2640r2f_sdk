...
exports.makeofflineOrDownloadQuery = function(dbResources, dbOverviews, dbPureBundles, query, callback) {
    ...
    async.waterfall([
        ...
        function (resourcesReadOnly, singleResource, callback) {
            // Separate DB for overviews exists mainly for performance reasons since we need to query overviewsq
        ...
        function (result, extraOverviews, callback) {
            ...
            async.eachSeries([result.resources, result.overviews], function(records, callback) {
                async.eachSeries(records, function(record, callback) {
                    async.eachSeries(record.dependencies || [], function(dependency, callback) {
                        exports.findBundle({
                            ...
                            // Note we attempt to minimize number of dependent bundles when multiple ranges / specific versions are present; but it's not optimal
                            const bundleKey = dependency.refId;
                            if (!bundles[bundleKey]) { // new dependent bundle 
                                if (!err && bundle) {
                                    bundle.require = dependency.require || bundle.require || 'optional';
                                    bundle.message = dependency.message || bundle.message;
                                    bundles[bundleKey] = [bundle];
                                }
                            }
                            else {
                                if (!err && bundle) {
                                    const previousBundle = bundles[bundleKey].find((previousBundle) => {
                                        return previousBundle.id === dependency.refId &&
                                            versioning.satisfies(previousBundle.version, dependency.versionRange);
                                    });
                                    if (previousBundle) { // bundle previously found already: merge 'require': mandatory trumps everything
                                        ...
                                        bundles[bundleKey] = bundles[bundleKey].concat([bundle]);
                                    }
                                }
                            }
                            callback();
                ...
                result.dependentBundles = Object.keys(bundles)
                    .reduce((accum, key) => {
                        return accum.concat(bundles[key]);
                    }, []);
                setImmediate(callback, null, result, extraOverviews);
            });
        }
...
exports.findBundle = function(args, callback) {
    const {id, versionRange=null, version=null, dbOverviews, dbPureBundles, dbResources} = args;
    const query = {id};
    if (versionRange && version) {
        throw new Error('version and versionRange are mutually exclusive parameters');
    }
    const isExactVersion = version || !semver.validRange(versionRange);
    if (isExactVersion) {
        query.version = version || versionRange;
    }
        ...
        async.waterfall([(callback) => {
            if (!isExactVersion) {
                db.find(query, (err, results) => {
                    if (!err && results) { // select newest semver compatible version
                        const resultVersion = versioning.maxSatisfying(results.map(result => result.version), versionRange);
                        const result = results.find(result => result.version === resultVersion);
                        callback(err, result);
                    }
                    else {
                        callback(err, null);
                ...
                // Not necessary; but if it's an exact match (not a range) we don't need to go through all the versions
                db.findOne(query, callback);
            }
        }, (bundle, callback) => {
            if (bundle) {
                foundBundle = bundle;
                setImmediate(callback, 'EARLY_EXIT'); // found it, exit early
            } else {
...
exports.doQuery = function (dbResources, dbOverviews, dbDevices, dbDevtools, dbDownloads, req, res, callback) {
    ...
    if (responseCache.has(responseCacheKey)) {
        ...
        var responseCacheVal = responseCache.get(responseCacheKey);
        responseCacheVal.hits++;    // count number of hits
        callback(null,  responseCacheVal.response);
        return;
    }
    ...
    async.waterfall([
        ...
        function (resourcesReadOnly, overviews, parentRecords, fullPathFilter, callback) {
            ...
            addOpenExternally(data.resources, req.headers['user-agent']);
        ...
        function (data, callback) {
            processFolders(data, function () {
                ...
                if (debugDisableResponseCache !== true) {
                    ...
                    responseCache.set(responseCacheKey, {response: response, hits: 1, procTime: _endTime-_startTime}); // always cache responses
                }
    ...
    function addOpenExternally(resources, userAgent) {
        const isLocalServerClient =
            userAgent ? userAgent.indexOf(vars.LOCALSERVER_USER_AGENT_PREFIX) > -1 : false;
        for (const resource of resources) {
            //resource.viewLimitations = ['AWS']; //for testing
            if (resource.viewLimitations) {
                if (isLocalServerClient) {
                    resource.openExternally = true;
                } else {
                    resource.openExternally = resource.viewLimitations.some(
                        element => ['AWS', 'E2E'].indexOf(element) > -1);
                }
            }
        }
    }
            ...
            async.series([
                ...
                function (callback) {
                    if (resource._importProjectCCS != null) {
                        ...
                        exports.detectDeviceVariantAndDevtool(dbDevices, dbDevtools, resource, req.query.device, req.query.devtool, req.query.path, function (deviceVariantExact, deviceVariantCandidates, deviceFamily, devtoolExaxt, devtoolCandidates) {
                            // CCS project, projectspec
                            if (resource.resourceType === 'project.ccs' ||
                                (resource.resourceType === 'projectSpec' && (!resource.advanced || !resource.advanced.overrideProjectSpecDeviceId))) {
                                ...
                                callback();
                            } else if (resource.resourceType === 'projectSpec' && resource.advanced && resource.advanced.overrideProjectSpecDeviceId) {
                        ...
                        if (overviewResource.name === folder.text) {
                            if (found === false) {
                                ...
                                if (overviewResource.resourceType === 'packageOverview') {
                                    folder.resourceType = overviewResource.resourceType;
                                    folder.package = overviewResource.package;
                                    folder.packageId = overviewResource.packageId;
                                    folder.packageVersion = overviewResource.packageVersion;
                                    folder.packageUId = overviewResource.packageUId;
                                    folder.packageOrder = overviewResource.packageOrder;
                                    ...
                                    folder.license = overviewResource.license; // needed after removing licence field from all other resources as part of db size reduction
                                }
                            } else {
    ...
    dbResources.findNoDeepCopy(normalizedQueries.resourceQuery, function (err, queryResultsReadOnly) {
        if (err) {
            ...
            async.each(queryResultsReadOnly, function (queryResultReadOnly, callback) {
                var result = {};
                results.push(result);
                result.location = path.join(vars.CONTENT_BASE_PATH, queryResultReadOnly.link);
                ...
                result.coreTypes = queryResultReadOnly.coreTypes;
...
exports.processResultsSync = function(queryResultReadOnly, fullPathFilter, query, reqRoute, reqUrl) {
    ...
    for (var i = 0; i < queryResultReadOnly.length; i++) {
        ...
        // make deep copy of fullPaths
        const _fullPathsCpy = [];
        for (let i = 0; i < recordReadOnly.fullPaths.length; i++) {
            _fullPathsCpy[i] = [];
            for (let j = 0; j < recordReadOnly.fullPaths[i].length; j++) {
                _fullPathsCpy[i][j] = recordReadOnly.fullPaths[i][j];
            }
        }
        ...
        if (query.download === 'true' || query.makeoffline === 'true' || query.removeoffline === 'true') {
            ...
            for (var i4 = 0; i4 < _fullPathsCpy.length; i4++) {
                ...
                // mark resource records that matched device, category and path as 'keep'
                // and add download / makeoffline links
                if (fullPathCpy.length === 0) {
                    recordAux.keep = true;
                    data.totalCount += 1;
                    if (recordReadOnly.fullPaths[0][0] !== vars.META_2_1_TOP_CATEGORY.devtools.text &&
                        recordReadOnly.fullPaths[0][0] !== vars.META_2_1_TOP_CATEGORY.devices.text) {
                        if (recordReadOnly.linkType !== 'external' &&
                            recordReadOnly.linkType !== 'file.executable' &&
                            recordReadOnly.linkType !== 'web.app') {
                            ...
                            if (recordReadOnly.resourceType === 'packageOverview' || !recordReadOnly.allowPartialDownload) {
                ...
                if (fullPathCpy[0] !== 'NO_MATCH' && recordReadOnly.resourceType !== 'overview' &&
                    ...
                    if (folderIndex === -1) {
                        ...
                        if (!recordReadOnly.allowPartialDownload) {
                        ...
                        // disable download for top Software node and all nodes in 'Device Documentation' and 'Development Tools'
                        if (recordReadOnly.fullPaths[0][0] === vars.META_2_1_TOP_CATEGORY.devtools.text ||
                            recordReadOnly.fullPaths[0][0] === vars.META_2_1_TOP_CATEGORY.devices.text ||
                            newFolder.text === vars.META_2_1_TOP_CATEGORY.software.text) {
...
exports.report = function() {
    ...
    responseCache.forEach( (value, key) => {
        list.push({hits: value.hits, procTime: value.procTime, key: key });
    });
    list.sort( (a, b) => {
        return a.hits - b.hits;
    } ).reverse();  // most hit on top
    let cMsg = {};
    const date = new Date();
    cMsg.date = date.toISOString();
    cMsg.entries = list;

    // write
    logger.info('Query response cache summary: ', cMsg);
};
