...
function createZipArchive(basePath, fileList, os, isBundle, downloadFilename, archiveId, dbDownloads, progressInfo, callback) {
    ...
    async.eachSeries(fileList, function (file, callback) { // use eachSeries to prevent too many file handles open for appendFile
        ...
        function havePlatformSpecificZip(client_os, callback) {
            var cmd;
            var zip_from_repo_file = path.join(fileList[0], path.basename(fileList[0]));
            ...
            fs.stat(path.join(vars.ZIPS_FOLDER, zip_from_repo_file), (err) => {
                const zip_from_repo_file_exists = !err;
                const use_all = fs.existsSync(path.join(vars.ZIPS_FOLDER, zip_from_repo_file_all));
                ...
                if (!zip_from_repo_file_exists && !use_all) {
                    ...
                    exec(cmd, {'cwd': vars.ZIPS_FOLDER}, function (error, stdout, stderr) {
                        if (error !== null) {
                            ...
                            logger.info(`Using platform-specific zip: ${zip_from_repo_file }`);
                            dbDownloads.insert([
