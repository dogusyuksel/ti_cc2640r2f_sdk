...
Localserver.prototype.init = function(vars, dbs, myHttpPort, callback) {
    ...
    this.contentPackagesConfigPath = path.dirname(vars.CONTENT_PACKAGES_CONFIG);
    ...
    if (vars.START_IN_OFFLINE_MODE) {
        logInfo('Start in offline mode with remote server host: ' + vars.REMOTESERVER_BASEURL);
        callback(false);
    } else {
...
function OfflineTransaction(parentServer, req, res) { // TODO: Oliver: Is parentServer needed? Isn't this always the same as localserver?
        ...
        parentServer.remoteGet(self.transaction.req.originalUrl, function (result, status) {
            ...
            if (_isStatusOK(status)) {
                ...
                async.each(result.dependentBundles, function(dependentBundle, callback) {
                    query.findBundle(
                        {
                            id: dependentBundle.id,
                            version: dependentBundle.version,
                            dbOverviews: parentServer.dbs.dbOverviews,
                            dbPureBundles: parentServer.dbs.dbPureBundles,
                            dbResources: parentServer.dbs.dbResources
                        }, function(err, bundle) {
                            if (err) {
                                return callback(err);
    ...
    this.postprocess = function(callback) {
        ...
        rexdb.makeBackup(vars.DB_BASE_PATH);
        ...
        async.series([
            ...
            setImmediate(function() {
                debugtrace('<= offlinePostprocess()');
                rexdb.removeBackup(vars.DB_BASE_PATH);
                callback(err, self.transaction);
            });
        });
    };
    ...
    this.remoteGet(url, function(result, status) {
        ...
        if(_isStatusOK(status)) {
            ...
            if(result.rejected) {
                ...
                serverState.updateUrl = result.updateUrl;
...
Localserver.prototype.removeofflineResources = function(req, res) {
    // force to remove whole package, temporary workaround for legacy codes, should use bundles directly
    return this._removeofflinePackage(req, res);
};

Localserver.prototype._removeofflinePackage = function(req, res) {
    const self = this;
    debugtrace('=> removeofflinePackage() : ' + req.originalUrl);
    if(res) {
        res.send(202);
    }

    const progressId = req.query.progressId;
    const progressMon= new Progress({progressId: progressId, name:'Removing offline resources', server:this});
    ...
    async.waterfall([
        function (callback) {
            // pre-process
            progressMon.name = 'Finding package ID';
            let packageObj = null;
            if (req.query.id) {
                // by id, single resource {
                self.dbs.dbResources.findOne({_id: req.query.id}, function (err, result) {
                    if (result) {
                        // find resource's parent package
                        self.dbs.dbOverviews.find({resourceType: 'packageOverview', packageUId: result.packageUId}, function (err, _packages) {
                            if (_packages && _packages.length > 0) {
                                packageObj = _packages[0];
                            }
                            callback(null, packageObj);
                        });
                    }
                    else {
                        callback("Invalid resource ID", packageObj);
                    }
                });
            }
            else if (req.query.path && req.query.path.length !== 0 && req.query.package) {
                ...
                self.dbs.dbOverviews.find({resourceType: 'packageOverview'}, function (err, _packages) {
                    for (let np = 0; np < _packages.length; np++) {
                        ...
                        if (pkg.rootCategory.length > pathCats.length) {
                            // deeper than input path, child node
                            continue;
                        }
                        // find exact category match
                        let matched = true;
                        for (let nc = 0; nc < pkg.rootCategory.length; nc++) {
                        ...
                        if (!packageObj) {
                            ...
                            // already found
                            if (pkg.rootCategory.length > packageObj.rootCategory.length) {
                                // pick the lowest in the stack
                                packageObj = pkg;
                            }
                        }
                    }
                ...
                callback("Invalid request", packageObj);
            }
            // ]
        },
        function (packageObj, callback) {
            ...
            if(packageObj) {
                self.packageImporter.packageImported(packageObj.packageId, packageObj.version, false);
                self.dbs.dbResources.remove({packageUId: packageObj.packageUId}, function () {
                    self.dbs.dbOverviews.remove({packageUId: packageObj.packageUId}, function () {
                        self.dbs.dbPureBundles.remove({packageUId: packageObj.packageUId}, function () {
                            if(packageObj.localPackagePath != null) {
                                fs.remove(packageObj.localPackagePath, function (err) {
                                    if (err) {
                                        logError(err);
                                    }
                                    callback(null, packageObj);   // discard the error and continue
                                });
                            }
                            else {
                                callback(null, packageObj);
...
Localserver.prototype.syncRemoteServer = function(forced, callback) { // TODO: Oliver: Would syncProductTree be a better name?
    ...
    async.series([
        function(callback) {
            ...
            syncDB('/api/packages?supplements=true', self.dbRemotePackages, true, false, function(err, changed, result) {
    ...
    async.waterfall([
        ...
        function(resultOffline, callback) {
            if (serverState.useRemoteContent === true) {
                request({
                    method: 'GET',
                    uri: vars.REMOTESERVER_BASEURL + req.originalUrl,
                    json: true
...
function PackageImporter(server) {
    ...
    /**
     * Clean local DBs: dbOverviews, dbPureBundles, dbResources.
     * Expecting caller will do scan and importAll afterward.
     */
    this.clean = function (callback) {
        debugtrace('=> PackageImporter.clean()');
        var self = this;
        ...
        async.series([
            function(callback) {
                // clean resources
                self.server.dbs.dbResources.remove({}, function() {
            ...
            function(callback) {
                // clean packages
                self.server.dbs.dbPureBundles.remove({}, function() {
            ...
            function(callback) {
                // clean overviews
                self.server.dbs.dbOverviews.remove({}, function() {
            ...
            function(callback) {
                // clean CCS
                if(self.server.ccsAdapter.isEnabled()) {
                    self.server.ccsAdapter.removeExtRef();
                }
                callback();
            },
            function(callback) {
                // clean local
                self.server.lppRemote.clear();
                self.server.dbRemotePackages.find({}, function (err, _packages) {
                    self.server.lppRemote.insert(_packages);
                    callback();
                });
            }
        ], function(err) {
    ...
    this.scan = function (callback) {
        ...
        async.series([
            ...
            function(callback) {
                // removed items
                var _listPR = [];
                // eachSeries because simultaneous db.remove calls are unsafe
                async.eachSeries(self.listPR, function (pkg, callback) {
                    self.lppDiscovered.removeByVID(pkg.id, pkg.version);
                    if (pkg.localState === LocalState.REMOVED) {
                        // clean up bundlesDB & resourcesDB
                        // REX-1751: defer DB update to importAll
                        _listPR.push(pkg);
                        setImmediate(callback);
                    }
                    else {
            ...
            // REX-1751: fake new package if only removed detected
    ...
    this._scanLocal = function (localPackages) {
        ...
        // [ Case: Content base path changed from c:\ti\tirex-content to c:\ti,
        //   the content base path is no longer a dedicated sub-folder, need to scan differently.
        if (spaths.length === 1 && this.lppDiscovered._comparePath(spaths[0], vars.CONTENT_BASE_PATH)) {
            // tirex-content base path == c:\ti
            this._findFileSinglePass(vars.CONTENT_BASE_PATH, foundTirexFiles, foundMappedHeader, 0, 4);
        }
        // ]
        // [ no more tirex-content folder and allows mulitple search paths
        /*
        else {
        ...
        */
            ...
            for (var i = 0; i < spaths.length; i++) {
                var spath = path.normalize(spaths[i]);
                this._findFileSinglePass(spath, foundTirexFiles, foundMappedHeader, 0, 4);
            }
        }
        // ]
...
//            packageHeader.localmeta = true;   // TODO - support offline import with metadata
            ...
            if(packageHeader) {
                if(this.server.lppRemote.findByVID(packageHeader.id, packageHeader.version)) {
                    ...
                    logInfo('local package does not exist in cloud server: '+packageHeader.id+'__'+packageHeader.version);
                }
            }
        }
        ...
        if(_lppCCS != null) {
            for(i=0; i<_lppCCS.pkgs.length; i++) {
                ...
                // No more tirex managed content folder
                /*
            ...
            if(dir) {
                ...
                */
                try {
                    ...
                    const sPaths = this.packageSearchPaths.paths;
                    let inSearchPaths = false;
                    for(var j=0; j<sPaths.length; j++) {
                        let sPath = sPaths[j];
                        if(vars.HOST === 'win32') {
                            sPath = path.normalize(sPath).toLowerCase();
                        }
                        if(dir.toLowerCase().indexOf(sPath.toLowerCase()) === 0) {
                            inSearchPaths = true;
                            break;
                        }
                    }
                    if(!inSearchPaths) {
                        // not in search paths, treat as removed
    ...
    // TODO : remove this retired function
    ...
    this._findFileSinglePass = function (dir, foundTirexFiles, foundMappedHeader, level, maxLevel) {
        let myLevel = level+1;
        if(myLevel > maxLevel) {
            // too deep, stop
            return;
        ...
        if (this.lppDiscovered._comparePath(path.basename(dir), 'tirex-product-tree')) {
            // tirex-product-tree, skip
            return;
        }

        let hdr = this.server.lppRemote.findByDir(path.basename(dir));
        if(hdr != null) {
            if(foundTirexFiles) {
                // optimized for special case:
                // root folder is a known package installFolder and has package.tirex,json in .metadata/.tirex.
                let pkgMetaFilepath = path.join(dir, MetadataFile.PACKAGE);
                if (fs.existsSync(pkgMetaFilepath) === true) {
                    debugtrace('found package: '+pkgMetaFilepath);
                    foundTirexFiles.push(pkgMetaFilepath);
                    return true;   // found package.tirex.json
                }
                pkgMetaFilepath = path.join(dir, MetadataFile.METAROOT, MetadataFile.TIREXROOT, MetadataFile.PACKAGE);
            ...
            if(foundMappedHeader) {
                // found known folder
                hdr.localPackagePath = dir;
                debugtrace('known package path: '+hdr.localPackagePath);
                foundMappedHeader.push(hdr);
                return true;   // assuming no subpackages
            }
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            if (foundTirexFiles && file === MetadataFile.PACKAGE) { // 'package.tirex.json'
                ...
                try {
                    let stat = fs.lstatSync(filepath);   // be aware of shortcut / symbolic link
                    if (stat.isDirectory()) {
                        subdirs.push(filepath);
                    }
                } catch(err) {
        ...
        for (let si = 0; si < subdirs.length; si++) {
            if (this.lppDiscovered._comparePath(path.basename(subdirs[si]), 'eclipse')) {
                // has 'eclipse' folder, => known CCS folder, skip
                return;
            }
        }

        // REX-1481
        // has package.tirex,json in .metadata/.tirex but not visible in cloud.
        const pkgMetaFilepath = path.join(dir, MetadataFile.METAROOT, MetadataFile.TIREXROOT, MetadataFile.PACKAGE);
        if (fs.existsSync(pkgMetaFilepath) === true) {
            return;   // valid package, skip it
        }


        // keep searching sub-folders
        for (let j = 0; j < subdirs.length; j++) {
            let found = this._findFileSinglePass(subdirs[j], foundTirexFiles, foundMappedHeader, myLevel, maxLevel);
        }
    };
    this._readHeader = function (file) {
        ...
        try {
            ...
            logError('Error reading ' + file + ' : ' + err.message);
        }
        return header;
    };
    this.importAll = function(progressId, res) {
        // REX-1751: move DB update for removed packages into this function
        const self = this;
        const _added = self.getNewPackages();
        const _removed = self.listPR;
        // process removed packages first
        this.cleanupRemovedPkgsDB(_removed, function () {
            if (_added.length === 0) {
                // nothing to import, provide a fake progress
                const progressMon = new Progress({progressId: progressId, name:'Import offline packages', server:self.server});
                progressMon.start();
                progressMon.end();
                if(res) {
                    res.send(202);
                ...
                // regular import
                self.import(_added, progressId, res);
            }
        });
    };
    this.cleanupRemovedPkgsDB = function(packages, callback) {
        const self = this;
        if (self.listPR.length === 0) {
            return setImmediate(callback);
        }

        async.eachSeries(self.listPR, function (pkg, callback) {
            const removePkg = {packageId: pkg.id, packageVersion: pkg.version};
    ...
    function PackageSearchPaths(server) {
        this.server = server;
        this._init = function () {
            var _dir = path.dirname(vars.DB_BASE_PATH);
            this.paths = [];
                ...
                function (callback) {
                    ...
                    async.each(self.transaction.overviews, function (overview, callback) {
                        ...
                        ['link', 'icon', 'image'].forEach((prop) => {
                            if (overview[prop]) {
                                overview[prop] = overview[prop].replace(self.importRemotePackagePath, self.importPackageLocalPath);
                            }
                        });
...
//                        parent.server.dbs.dbOverviews.insertOrSkipSync(self.transaction.overviews);
        ...
        var progressMon = new Progress({progressId: param.progressId, name:'Importing package: '+this.importPackageName+' v'+this.importPackageVersion, server:server});
        progressMon.addChild(new Progress({name: 'Preparing', func: this.prepare, funcParent: this}), 5);
...
function CCSAdapter(myHttpPort) {
    ...
    if(vars.CCS_LOCALPORT != null) {
        ...
        this.localAddr = 'http://localhost:' + this.localPort + '/ide/';    // CCS7.2 IDE
    ...
    this.translateProjectPath = function(cpath) {
        ...
        // [ CCDSK-2494
        // Content base has been changed from c:\ti\tirex-content to c:\ti,
        // any offline reference to @ti-rex-content will be broken.
        // Hack: try using the known subpath tirex-content if needed
        let _urlObj = urlParser.parse(cpath, true);
        let _location = _urlObj.query.location;
        let _contentBase = vars.CONTENT_BASE_PATH;
        if( _location && _location.includes('@ti-rex-content') ) {
            let _absLoc = _location.replace('@ti-rex-content', _contentBase);
            if (fs.existsSync(_absLoc) === false) {
                _contentBase = path.join(vars.CONTENT_BASE_PATH,'tirex-content');
                _absLoc = _location.replace('@ti-rex-content', _contentBase);
                if (fs.existsSync(_absLoc) === true) {
                    cpath = cpath.replace('@ti-rex-content', _contentBase);
                }
            }
        }
        // ]
    ...
    this.removeExtRef = function() {
        var tirex_2_file = path.join(os.homedir(), 'ti/CCSExternalReferences', 'TIRex_2');
        fsutils.removeFile(tirex_2_file, false, null);
        this.savedSearchPaths = [];
    };
    this.readExtRef = function() {
        var paths = [];
        var tirex_2_file = path.join(os.homedir(), 'ti/CCSExternalReferences', 'TIRex_2');
        if (fs.existsSync(tirex_2_file) === false) {
            return paths;
        }
        var body = fs.readFileSync(tirex_2_file, 'utf8');
        ...
        this.savedSearchPaths = paths;
        return paths;
    };
    this.updateExtRef = function(paths, notify, merge) {
        ...
        // builtin
        // if(paths.indexOf(vars.CONTENT_BASE_PATH) === -1) {
        //     paths.push(vars.CONTENT_BASE_PATH);
        // }

        if(merge) {
            var _svpaths = this.readExtRef();
            for (var j = 0; j < _svpaths.length; j++) {
                if(paths.indexOf(_svpaths[j]) === -1) {
                    paths.push(_svpaths[j]);
                }
            }
        }
    ...
    this.redirectAPI = function(from, to, req, res) {
        ...
        if (from && to) {
            _urlObj.pathname = _urlObj.pathname.replace(from, to);
        }
