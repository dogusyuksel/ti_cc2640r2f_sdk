...
exports.makeOfflineBundles = function(vidList, progressId, dbDevices, dbDevtools, dbOverviews, dbResources, dbPureBundles, downloadToFolder) {
    ...
    // [ REX-921: save absolute local path in DBs
    if(!downloadToFolder) {
        downloadToFolder = vars.CONTENT_BASE_PATH;
    }
    // ]
    async.eachSeries(vidArr, function(vid, callback) {
        makeOfflineBundle(vid, dbDevices, dbDevtools, dbOverviews, dbResources, dbPureBundles, downloadToFolder, progressInfo, callback);
    }, function(err) {
...
exports.downloadBundleMetadata = function(vid, progressInfo, callback) {
    return downloadBundleFilesOrMetadata(vid, 'makeoffline', progressInfo, null, callback);
};
 ...
 */
function downloadBundleFilesOrMetadata(vid, downloadOrMakeoffline, progressInfo, downloadToFolder, callback) {
    var zipFilePath = '';
    ...
    async.waterfall([
        ...
        function (previousBody, callback) {
            ...
            var theRequest = request.get({
                url:zipLocation,
                strictSSL: false
            }); // using https to download from Akamai to prevent proxy virus scanners from disrupting download: Akamai server did not have a valid SSL certificate
            ...
            theRequest
                ...
                .on('response', function (res) {
                    if (res.statusCode === 200) {
                        ...
                        if (!calledback) {
                            calledback = true;
                            callback('This package is not supported on your OS.');
                        }
                    }
                    else {
                        {
                            if (!calledback) {
                                calledback = true;
                                callback('getBundleFilesOrMetadata error: ' + res.req.url + ': expected status code 200 but got ' + res.statusCode + vars.HOST);
                            }
                        }
                    }
                })
        ...
        function (zipFilePath, zipFileSize, dstPath, callback) {
            ...
            if (process.platform === 'win32') {
                ...
                // node-unzip not handling zip64 format
                var worked = 0;
                const onEntry = function(entry) {
                    worked += entry.uncompressedSize;
                    progressInfo.setStageWorked(worked, zipFileSize);
                };
                extract(zipFilePath, dstPath, onEntry, function(err) {
                    progressInfo.stageDone();
                    if (err) {
...
function makeOfflineBundle(vid, dbDevices, dbDevtools, dbOverviews, dbResources, dbPureBundles, downloadToFolder, progressInfo, callback) {
    var dependentBundles = [];
    var dstPath;
    async.waterfall([
            // request remote server to archive and send all files of the bundle
            function(callback) {
                downloadBundleFilesOrMetadata(vid, 'download', progressInfo, downloadToFolder, function(err) {
                    callback(err);
                });
            },
            // get the metadata records for the bundle from the remote server
            function(callback) {
                downloadBundleFilesOrMetadata(vid, 'makeoffline', progressInfo, null, function(err, _dstPath) {
                    dstPath = _dstPath;
                    var data;
                    if (err) {
                        return callback(err);
                    ...
                    if (!downloadToFolder) {
                        // if relocate needed, defer dbResource and dbOverviews update until package info is available
                        dbOverviews.insert(data.overviews, function() {
                            dbResources.insert(data.resources, function() {
                                callback(null, data.resources, data.overviews);
                            });
                        });
                    } else {
                        callback(null, data.resources, data.overviews);
            ...
            function(resources, overviews, callback) {
                ...
                request.get({
                    ...
                    if (err) {
                        ...
                        if (downloadToFolder) {
                            const installPath = path.join(downloadToFolder, bundleRecord.packagePath);
                            if (bundleRecord.image) {
                                bundleRecord.image = bundleRecord.image.replace(bundleRecord.packagePath, installPath);
                            }
                        }

                        upsertBundleRecord(bundleRecord, function() {
                            callback(null, bundleRecord, resources, overviews);
                        });
                    }
                });
            },
            // check if the bundle has a core dependency definition; this is a type of nested bundle and needs to be marked local too
            function(bundleRecord, resources, overviews, callback) {
                async.each(bundleRecord.coreDependencies, function (coreDep, callback) {
                    ...
                    setImmediate(callback, null, bundleRecord, resources, overviews);
                });
            },
            ...
            function(bundleRecord, resources, overviews, callback) {
                ...
                // [ REX-921
                if (downloadToFolder) {
                    installPath = path.join(downloadToFolder, bundleRecord.packagePath);
                }
                // ]
                if (installPath == null) {
                ...
                for (var i = 0; i < resources.length; i++) {
                    ...
                    if (resource.linkType === 'local') {
                        ...
                        const _relbase = vars.CCS_CLOUD_IMPORT_PATH + '/' + bundleRecord.packagePath;
                        if (resource._importProjectCCS) {
                            let _ccsImportProject = resource._importProjectCCS.replace(/\\/g, '/');
                            _ccsImportProject = _ccsImportProject.replace(_relbase, installPath);
                        ...
                        if (resource._createProjectCCS) {
                            ...
                            _createProjectCCS = _createProjectCCS.replace(_relbase, installPath);
                ...
                for (let i = 0; i < overviews.length; i++) {
                    ...
                    ['link', 'icon', 'image'].forEach((prop) => {
                        if (overview[prop]) {
                            overview[prop] = overview[prop].replace(bundleRecord.packagePath, installPath);
                        }
                    });
                }
                dbResources.insert(resources, function(err) {
                    dbOverviews.insert(overviews, function(err) {
                        bundleRecord.localPackagePath = installPath;
