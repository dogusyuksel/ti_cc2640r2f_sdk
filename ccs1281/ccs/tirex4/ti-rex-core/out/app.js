...
function setupTirex({ config: configPassedIn, dconfig, dinfraPath }, callback) {
    ...
    const config = appConfig_1.processConfig(configPassedIn);
    const logger = logger_1.createDefaultLogger(dinfra);
    ...
    async.series([
        ...
        callback => {
            if (config.dbTablePrefix && config.dbTablePrefix !== vars_1.Vars.DB_TABLE_PREFIX_AUTO) {
                ...
                dbUpdateInfo_1.fetchLastUpdateInfoCb(dinfra, (err, dbLastUpdateInfo) => {
...
function initApp({ config, varsInstance, dinfra, sqldbFactory, myHttpPort, dinfraLogger }, callback) {
    ...
    app.use(async (req, res, next) => {
        try {
            ...
            executeRoute_1.sendError(res, new rexError_1.RexError({
    ...
    app.use(async (req, res, next) => {
        try {
            if (req.url === 'index.html') {
                ...
                const modifiedUrlQuery = routing_helpers_1.getUrlQuery(parsedUrl.query);
    ...
    const rex = rex_1.Rex({ dinfraLogger, vars: varsInstance });
    ...
    app.use('/', routes_1.getRoutes({ dinfra, config, desktopServer, rex }));
    // refresh and exit
...
function registerPort(config, dinfra, dconfig, logger, sqldbFactory, callback) {
    ...
    dinfra.registerService(serviceName, serviceVersion, params).on('registered', () => {
        ...
        dbUpdateInfo_1.registerDatabaseUpdateHandler(dinfra, () => {
