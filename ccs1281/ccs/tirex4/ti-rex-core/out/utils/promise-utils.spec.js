...
describe('promisePipe', () => {
    it('should successfully pipe two streams', async () => {
        ...
        await promise_utils_1.promisePipe([reader, writer]);
        chai_1.expect(writer.writtenData).to.equal(reader.mockData);
    });
    it('should successfully pipe more streams', async () => {
        const reader = new MockReader();
        const writer = new MockWriter();
        await promise_utils_1.promisePipe([reader, new MockTransform(), writer]);
        chai_1.expect(writer.writtenData).to.equal(reader.mockData + reader.mockData);
    });
    it('should catch errors at the beginning', async () => {
        try {
            await promise_utils_1.promisePipe([new MockReader(true), new MockTransform(), new MockWriter()]);
            chai_1.expect('should not get here').to.equal(null);
        }
        catch (e) {
            chai_1.expect(e.toString()).to.contain('Mock Error');
        }
    });
    it('should catch errors in the middle', async () => {
        try {
            await promise_utils_1.promisePipe([new MockReader(), new MockError(), new MockWriter()]);
            chai_1.expect('should not get here').to.equal(null);
        }
        catch (e) {
            chai_1.expect(e.toString()).to.contain('Mock Error');
    ...
    it('should catch errors at the end', async () => {
        try {
            await promise_utils_1.promisePipe([new MockReader(), new MockTransform(), new MockError()]);
            chai_1.expect('should not get here').to.equal(null);
        }
        catch (e) {
            chai_1.expect(e.toString()).to.contain('Mock Error');
    ...
    it('should detect invalid streams', async () => {
        try {
            await promise_utils_1.promisePipe([new MockWriter(), new MockReader()]);
            chai_1.expect('should not get here').to.equal(null);
        }
        catch (e) {
            chai_1.expect(e.toString()).to.contain('invalid arguments');
        }
    });
});
