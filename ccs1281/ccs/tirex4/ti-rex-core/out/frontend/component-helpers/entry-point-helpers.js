...
offlineDevices) {
    ...
    return Promise.all([apis.getFilterOptions(), apis.getPackages(), apis.getPackageGroups()])
        ...
        const filter = filter_helpers_1.getFilterFromBrowserUrlQuery(urlQuery, packageGroups, filterOptions);
        return {
            filter,
            filterOptions,
            packages,
        ...
        if (!id) {
            ...
            return public_id_helpers_1.getNodeDbIdFromPublicId(id, state.packageGroups, appPropsInitial.urlQuery, appPropsInitial.apis).then(id => ({
                id,
                state
            }), (err) => {
        ...
        .then(({ id, state }) => component_helpers_1.getNodeData(id, apis).then(nodeData => ({ nodeData, state })))
        .then(({ state, nodeData }) => ({
        ...state,
...
function useAvailableOfflineBoardsAndDevices(args) {
    ...
    const { result: rexBoardsAndDevices } = use_local_apis_1.useGetOfflineBoardsAndDevices({
        appProps: args.appPropsInitial,
        errorCallback: args.errorCallback,
        allowNoAgent: false
    });
    // Get Targetdb based devices from CCS server
    const { result: ccsDevicesInfo } = use_local_apis_1.useGetCcsDevices({
        appProps: args.appPropsInitial,
        targetFilter: null,
        errorCallback: args.errorCallback,
        allowNoAgent: false
    ...
    return use_async_operation_1.useAsyncOperation({
        ...
        dependencies: [!!args.appPropsInitial, !!rexBoardsAndDevices && !!ccsDevicesInfo],
        errorCallback: args.errorCallback,
