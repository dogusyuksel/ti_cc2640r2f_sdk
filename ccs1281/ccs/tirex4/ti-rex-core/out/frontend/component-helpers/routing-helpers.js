...
function getLinkPrefix() {
    const { role } = util_1.getServerConfig();
    return role ? `/${role}` : '';
}
...
function getUrlQuery(queryString) {
    ...
    util_2.getObjectKeys(browserQueryObject).forEach(urlQueryKey => {
        switch (urlQueryKey) {
            // filter keys (arrays)
            case 'compilers':
            case 'devices':
            ...
            case 'packageDependencies': {
                setQueryObjectValue(queryObject, browserQueryObject, urlQueryKey, item => item && helpers_1.getQueryParamAsArray(item));
                break;
            }
            ...
            default:
                util_2.assertNever(urlQueryKey);
...
function setQueryObjectValue(queryObject, browserQueryObject, urlQueryKey, transformFunc) {
    util_2.setValueForPair(queryObject, browserQueryObject, urlQueryKey, transformFunc);
...
function getCorrectedNodeQueryParam(recievedNodeQueryParam) {
    // split publicId
    const { nodePublicId, packagePublicId, packageGroupPublicId, packageGroupVersion } = public_id_helpers_1.getPublicIdParts(recievedNodeQueryParam);
    ...
    // combine publicId
    return public_id_helpers_1.getPublicIdFromMinimalIds({
        nodePublicId,
