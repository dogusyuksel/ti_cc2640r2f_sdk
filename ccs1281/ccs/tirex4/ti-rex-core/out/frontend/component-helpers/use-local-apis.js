...
function useGetPackageInstallInfo(args) {
    ...
    return useApiWithEvent({
        ...
        evtHandling: onResultUpdated => {
            ...
            return () => {
                appProps.localApis.removeListener("OnInstallInfoUpdated" /* ON_INSTALL_INFO_UPDATED */, listener);
            };
        },
        placeholder: []
    });
...
function useGetInstalledPackages(args) {
    ...
    return useApiWithEvent({
        ...
        evtHandling: onResultUpdated => {
            ...
            return () => {
                appProps.localApis.removeListener("OnInstalledPackagesUpdated" /* ON_INSTALLED_PACKAGES_UPDATED */, listener);
            };
        },
        placeholder: []
    });
...
function useUpdateOfflineBoardsAndDevices(args) {
    const { appPropsInitial: appProps } = args;
    return useApi({
        ...args,
        allowNoAgent: false,
        api: agent => {
...
function useGetCcsDevices(args) {
    const { appProps, targetFilter } = args;
    return useApi({
        ...args,
        allowNoAgent: false,
        api: agent => {
...
function useGetAgentMode(args) {
    ...
    return useApi({
        ...
        dependencies: [!!appProps],
        placeholder: util_1.fallbackIsDesktop() ? 'desktop' : 'cloud'
    });
}
...
function useGetProgress(args) {
    ...
    return useApiWithEvent({
        ...
        evtHandling: onResultUpdated => {
            ...
            return () => {
                appProps.localApis.removeListener("OnProgressUpdated" /* ON_PROGRESS_UPDATED */, listener);
            };
        },
        placeholder: {}
    });
...
function useApiWithEvent(args) {
    ...
    const [getState, setState] = use_state_1.useState({ result: null });
    const agent = useAgentModule({ errorCallback });
    const { result: resultInitial, ...rest } = use_async_operation_1.useAsyncOperation({
        operation: async () => {
            if (!agent || typeof agent === 'string') {
                return null;
            }
...
function useAgentModule(args) {
    ...
    const { result: agentState } = use_cloud_agent_1.useCloudAgent({
        dependencies: [],
        errorCallback
    });
    if (!agentState || !agentState.agent) {
        if (agentState &&
            !agentState.agent &&
            agentState.cloudAgentInitState !== "NotInitialized" /* NOT_INITIALIZED */) {
