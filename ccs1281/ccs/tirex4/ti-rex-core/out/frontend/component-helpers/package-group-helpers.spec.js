...
describe('[frontend] PackageGroupHelpers', function () {
    ...
    describe('isLatest', function () {
        ...
        it('Should be truthy for latest package', async function () {
            ...
            expect_1.expect(args).to.be.true;
        });
        ...
        it('Should be falsey for the newest version of a package which is explictly added (as not latest)', async function () {
            ...
            expect_1.expect(args).to.be.false;
        });
        ...
        async function callMethod({ group, params, data }) {
            await util_2.setupEnzymeTest({ data });
    ...
    describe('getPackageGroups', function () {
        before(async function () {
            await util_2.setupEnzymeTest({ data: data3 });
        });
        after(function () {
            util_2.cleanupEnzymeTest();
        });
...
function validateResult(inputGroups, outputGroups) {
    expect_1.expect(outputGroups).to.exist;
    expect_1.expect(outputGroups.length).to.equal(inputGroups.length);
    outputGroups.map((outputGroup, idx) => {
        expect_1.expect(outputGroup).to.deep.equal({ ...inputGroups[idx], isLatest: outputGroup.isLatest });
        if (inputGroups[idx].isLatest) {
            expect_1.expect(outputGroup.isLatest).to.equal(inputGroups[idx].isLatest);
        }
    });
}
