...
function fallbackIsDesktop() {
    ...
    const isCCSEclipse = !!(windowAsAny.ti &&
        windowAsAny.ti.debug &&
        windowAsAny.ti.debug.cloudagent);
    const isCCSTheia = !!getTheiaPort();
    return isCCSEclipse || isCCSTheia;
}
...
function getPlatform() {
    ...
    return download_1.resolvePackageZipForOS(parser.getOS().name || '');
}
...
async function getTICloudAgentObject() {
    ...
    async function _getTICloudAgentObject() {
        if (navigator.userAgent.includes('CCStudio')) {
