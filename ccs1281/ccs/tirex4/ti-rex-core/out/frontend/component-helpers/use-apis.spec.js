...
describe('[frontend] use-apis', function () {
    this.timeout(20000);
    before(async () => {
        browser_emulator_1.browserEmulator();
    });
    after(() => {
        util_1.cleanupEnzymeTest();
    });
    describe('useApi', function () {
        beforeEach(async function () {
            await util_1.setupEnzymeTest({ data: data1 });
        });
        afterEach(function () {
            util_1.cleanupEnzymeTest();
        ...
        it('Should only make the call when the trigger is true', async function () {
            ...
            const { wrapper } = util_2.getDataFromHook(hook);
            // Update, wait, verify
            expect_1.expect(spy.called).to.be.false;
            deps.trigger = true;
            wrapper.setProps({ hook });
            await util_2.waitForHookWrapperResult(wrapper, hook, val => hook_tests_1.getResult(val, errorCallback));
            expect_1.expect(spy.calledOnce).to.be.true;
        });
        it('Should not update the result when trigger is false', async function () {
            ...
            deps.api = spy2;
            wrapper.setProps({ hook });
            const result = await util_2.waitForHookWrapperResult(wrapper, hook, val => hook_tests_1.getResult(val, errorCallback));
            expect_1.expect(result).to.equal(1);
            // Set trigger, expect update
            deps.trigger = true;
            wrapper.setProps({ hook });
            const result2 = await util_2.waitForHookWrapperResult(wrapper, hook, val => hook_tests_1.getResult(val, errorCallback));
            expect_1.expect(result2).to.equal(500);
            expect_1.expect(spy2.calledOnce).to.be.true;
        });
        // Helpers
        function createHook(args) {
            return () => use_apis_1._useApi({ dependencies: [], apis: new apis_1.APIs(), errorCallback, ...args }).result;
        }
    });
    describe('useApi - manageControl', function () {
        ...
        it('Should acquire control when trigger is set', async function () {
            ...
            deps.trigger = true;
            wrapper.setProps({ hook });
            await delay_1.delay(10); // Get off event queue to let hooks react to dependency change
            // Verify
            expect_1.expect(aquireControlSpy.calledOnce).to.be.true;
        });
        it('Should call the api when control is acquired', async function () {
            const { wrapper, hook, deps, apiSpy, aquireControlSpy } = setup();
            deps.trigger = true;
            wrapper.setProps({ hook });
            await delay_1.delay(10); // Get off event queue to let hooks react to dependency
            // Make sure we aquired control before checking if api called
            expect_1.expect(aquireControlSpy.calledOnce).to.be.true;
            expect_1.expect(apiSpy.calledOnce).to.be.true;
        });
        it('Should release control when hook is done', async function () {
            ...
            await util_2.waitForHookWrapperResult(wrapper, hook, val => hook_tests_1.getResult(val, errorCallback));
            // Verify
            expect_1.expect(releaseControlSpy.calledOnce).to.be.true;
        });
        function setup() {
    ...
    describe('useGetNodes', function () {
        ...
        });
        {
            ...
            hook_tests_1.doCommonTests(() => setup(), appProps => createHook({ ids: nodeDbIds }, appProps), result => {
                expect_1.expect((result || []).map(item => item.nodeDbId)).to.deep.equal(nodeDbIds);
        ...
        }
        {
            ...
            hook_tests_1.doDependencyTest(() => setup(), appProps => createHook(deps, appProps), (count, result, spy) => createVerifyResult(deps, count, result, spy), errorCallback, deps, updatedDeps);
        }
        // Helpers
        ...
        function createVerifyResult(deps, count, result, spy) {
            ...
            const url = `${"api/nodesData" /* GET_NODES_DATA */}?${QueryString.stringify(params)}`;
            expect_1.expect(result).to.be.not.null;
    ...
    describe('useGetExtendedNodes', function () {
        ...
        });
        {
            ...
            hook_tests_1.doCommonTests(() => setup(), appProps => createHook({ id: nodeDbId }, appProps), result => {
                expect_1.expect(result && result.nodeDbId).to.equal(nodeDbId);
            }, errorCallback, 1 + ajaxCallCountOffset);
        }
        {
            const deps = {
        ...
        function createHook(deps, appProps) {
            return () => {
                const { id } = deps;
                // Throw error if any deps are null (currently none can be for this api)
                return use_apis_1.useGetExtendedNodes({
                    id,
                    apis: appProps.apis,
                    errorCallback
                }).result;
        ...
        function createVerifyResult(deps, count, result, spy) {
            ...
            const url = `${"api/nodeExtendedData" /* GET_NODE_EXTENDED_DATA */}?${QueryString.stringify(params)}`;
            expect_1.expect(result).to.be.not.null;
            return verifyResultForApi(expectedCount, [url], spy);
        }
    });
    describe('useGetFilteredChildrenNodes', function () {
        ...
        {
            hook_tests_1.doCommonTests(() => setup(), appProps => createHook({ parentIds: [packageNode2.nodeDbId], urlQuery: {} }, appProps), result => {
                expect_1.expect(_.flatten(result).map(item => item.nodeDbId)).to.deep.equal([
        ...
        it('Should handle dependencies changing', async function () {
            ...
            // parentIds update
            {
                util_3.setValueForPair(deps, updatedDeps, 'parentIds', val => val);
            ...
            // urlQuery update
            {
                util_3.setValueForPair(deps, updatedDeps, 'urlQuery', val => val);
                const result = await updateAndWait();
                const nodesDataUrl = getNodesDataUrl(result);
                const filteredChildrenNodeIdsUrl = getFilteredChildrenNodeIdsUrl(deps, appProps);
        ...
        function createHook(deps, appProps) {
            return () => {
                ...
                // Throw error if any deps are null (currently none can be for this api)
                return use_apis_1.useGetFilteredChildrenNodes({
                    parentIds,
                    urlQuery,
                    apis: appProps.apis,
                    errorCallback
        ...
        function getFilteredChildrenNodeIdsUrl(deps, appProps) {
            ...
            const query = filter_helpers_1.getServerFilterQueryFromBrowserUrlQuery(urlQuery, packageGroups, filterOptions);
            const params = {
                parentDbId: parentIds,
                ...filter_helpers_1.sortQuery(query)
            };
            const url = `${"api/filteredChildrenNodeIds" /* GET_FILTERED_CHILDREN_NODE_IDS */}?${QueryString.stringify(params)}`;
            return url;
        }
    ...
    describe('useGetFiltredTableItems', function () {
        ...
        });
        {
            ...
            hook_tests_1.doCommonTests(() => setup(), appProps => createHook({
                parentId: tableItem1.tableItemNodeDbId,
                urlQuery: {},
                page: page_1.Page.EXPLORE
            }, appProps), result => {
                expect_1.expect((result || []).map(item => item.tableItemDbId)).to.deep.equal(expectedItems.map(item => item.tableItemDbId));
            }, errorCallback, 1 + ajaxCallCountOffset);
        }
        {
            const deps = {
            ...
            hook_tests_1.doDependencyTest(() => setup(), appProps => createHook(deps, appProps), (count, result, spy, data) => createVerifyResult(deps, count, result, spy, data), errorCallback, deps, updatedDeps);
        }
        async function setup() {
            ajaxGetSpy = sinon.spy(ajax_1.ajax, 'get');
        ...
        function createHook(deps, appProps) {
            return () => {
                ...
                // Throw error if any deps are null (currently none can be for this api)
                return use_apis_1.useGetFilteredTableItems({
                    parentId,
                    urlQuery,
                    page,
                    apis: appProps.apis,
        ...
        function getFilteredTableItemsUrl(deps, appProps) {
            ...
            const url = `${"api/filteredTableItemsData" /* GET_FILTERED_TABLE_ITEMS_DATA */}?${QueryString.stringify(params)}`;
            return url;
        }
    });
    describe('useGetNodeDataForTableItemVariant', function () {
        ...
        {
            hook_tests_1.doCommonTests(() => setup(), appProps => createHook({ id: tableItem2.tableItemDbId, urlQuery: {}, variant: variant2.variant }, appProps), result => {
                expect_1.expect(result && result.nodeDbId).to.deep.equal(variant2.nodeDbId);
            }, errorCallback, 1 + ajaxCallCountOffset);
        }
        {
            const deps = {
        ...
        }
        {
            ...
            hook_tests_1.doWaitUntilOptionsSetTest(() => setup(), appProps => createHook(deps, appProps), (count, result, spy, data) => createVerifyResult(deps, count, result, spy, data), errorCallback, deps, updatedDeps);
        }
        async function setup() {
            ajaxGetSpy = sinon.spy(ajax_1.ajax, 'get');
            const { appProps } = await setupCommon(data2);
        ...
        function createHook(deps, appProps) {
            return () => {
                ...
                return use_apis_1.useGetNodeDataForTableItemVariant({
                    id,
                    urlQuery,
                    variant,
                    apis: appProps.apis,
        ...
        function getNodeDataForTableItemVariantUrl(deps, appProps) {
            ...
            const url = `${"api/nodeDataForTableItemVariant" /* GET_NODE_DATA_FOR_TABLE_ITEM_VARIANT */}?${QueryString.stringify(params)}`;
            return url;
        }
    });
    describe('useExpandNode', function () {
        ...
        {
            hook_tests_1.doCommonTests(() => setup(), appProps => createHook({ id: packageNode2.nodeDbId, urlQuery: {} }, appProps), result => {
                expect_1.expect(result).to.be.not.null;
            }, errorCallback, 1 + ajaxCallCountOffset);
        }
        {
        ...
        function createHook(deps, appProps) {
            return () => {
                ...
                // Throw error if any deps are null (currently none can be for this api)
                const { result } = use_apis_1.useExpandNode({
                    id,
                    urlQuery,
                    apis: appProps.apis,
                    errorCallback
        ...
        function createVerifyResult(deps, count, result, spy, appProps) {
            ...
            const url = `${"api/expandedFilteredDescendantNodesData" /* GET_EXPANDED_FILTERED_DESCENDANT_NODES_DATA */}?${QueryString.stringify(params)}`;
            return verifyResultForApi(expectedCount, [url], spy);
        }
    });
    describe('useGetSearchSuggesstions', function () {
        ...
        {
            hook_tests_1.doCommonTests(() => setup(), appProps => createHook({ text: 'foo', urlQuery: {} }, appProps), result => {
                expect_1.expect(result).to.be.not.null;
            }, errorCallback, 1 + ajaxCallCountOffset);
        }
        {
        ...
        function createHook(deps, appProps) {
            return () => {
                ...
                // Throw error if any deps are null (currently none can be for this api)
                return use_apis_1.useGetSearchSuggesgtions({
                    text,
                    urlQuery,
                    apis: appProps.apis,
                    errorCallback
        ...
        function createVerifyResult(deps, count, result, spy, appProps) {
            ...
            const url = `${"api/searchSuggestions" /* GET_SEARCH_SUGGESTIONS */}?${QueryString.stringify(params)}`;
            return verifyResultForApi(expectedCount, [url], spy);
        }
    });
    describe('useGetImportInfo', function () {
        ...
        {
            const nodeDbId = packageNode2.nodeDbId;
            hook_tests_1.doCommonTests(() => setup(), appProps => createHook({ id: nodeDbId, urlQuery: {} }, appProps), result => {
                expect_1.expect(result).to.be.not.null;
            }, errorCallback, 1 + ajaxCallCountOffset);
        }
        {
        ...
        function createHook(deps, appProps) {
            return () => {
                const { id, urlQuery } = deps;
                // Throw error if any deps are null (currently none can be for this api)
                return use_apis_1.useGetImportInfo({
                    id,
                    urlQuery,
                    apis: appProps.apis,
                    errorCallback
        ...
        function createVerifyResult(deps, count, result, spy, appProps) {
            ...
            const url = `${"api/importInfo" /* GET_IMPORT_INFO */}?${QueryString.stringify(params)}`;
            return verifyResultForApi(expectedCount, [url], spy);
        }
    });
    describe('useGetPackages', function () {
        ...
        {
            hook_tests_1.doCommonTests(() => setup(), data => createHook(data), result => {
                expect_1.expect(result).to.be.not.null;
            }, errorCallback);
        }
        // Helpers
        ...
        function createHook(apis) {
            return () => {
                return use_apis_1.useGetPackages({
                    apis,
                    errorCallback
                }).result;
            };
    ...
    describe('useGetPackageGroups', function () {
        ...
        function createHook(apis) {
            return () => {
                return use_apis_1.useGetPackageGroups({
                    apis,
                    errorCallback
                }).result;
            };
    ...
    describe('useGetFilterOptions', function () {
        ...
        function createHook(apis) {
            return () => {
                return use_apis_1.useGetFilterOptions({
                    apis,
                    errorCallback
                }).result;
            };
    ...
    describe('useGetNodeDbId', function () {
        ...
        });
        {
            ...
            hook_tests_1.doCommonTests(() => setup(), appProps => createHook({
                ...
                expect_1.expect(result).to.deep.equal(node.nodeDbId);
            }, errorCallback, 1 + ajaxCallCountOffset);
        }
        it('Should handle dependencies changing', async function () {
            ...
            // Wait & verify first result
            {
                ...
                expect_1.expect(result).to.deep.equal(node1.nodeDbId);
                await verifyResultForApi(expectedCount, [url], spy);
            }
            // Update, wait, & verify
            {
                util_3.setValueForPair(deps, updatedDeps, 'nodePublicId', val => val);
                util_3.setValueForPair(deps, updatedDeps, 'packageGroupPublicUid', val => val);
                util_3.setValueForPair(deps, updatedDeps, 'packagePublicId', val => val);
                wrapper.setProps({ hook });
                const url = getUrl(deps);
                const expectedCount = 2 + ajaxCallCountOffset;
                const result = await util_2.waitForHookWrapperResult(wrapper, hook, val => hook_tests_1.getResult(val, errorCallback));
                expect_1.expect(result).to.deep.equal(node2.nodeDbId);
                await verifyResultForApi(expectedCount, [url], spy);
            }
        });
        // Helpers
        ...
        function createHook(deps, appProps) {
            return () => {
                ...
                return use_apis_1.useGetNodeDbId({
                    nodePublicId,
        ...
        function getUrl(deps) {
            ...
            if (packageGroupPublicUid) {
                ...
                url = `${"api/nodePublicIdToDbId" /* GET_NODE_PUBLIC_ID_TO_DB_ID */}?${QueryString.stringify(params)}`;
            }
            else {
                const params = {
                    nodePublicId,
    ...
    describe('useGetRex3LinkToDbId', function () {
        ...
        });
        {
            ...
            hook_tests_1.doCommonTests(() => setup(), appProps => createHook({ link }, appProps), result => {
                expect_1.expect(result).to.deep.equal(dbId);
            }, errorCallback, 1 + ajaxCallCountOffset);
        }
        {
            const deps = {
        ...
        function createHook(deps, appProps) {
            return () => {
                const { link } = deps;
                // Throw error if any deps are null (currently none can be for this api)
                return use_apis_1.useGetRex3LinkToDbId({
                    link,
                    apis: appProps.apis,
                    errorCallback
                }).result;
        ...
        function createVerifyResult(deps, count, result, spy) {
            ...
            const url = `${"api/rex3LinkToDbId" /* GET_REX3_LINK_TO_DB_ID */}?${QueryString.stringify(params)}`;
            expect_1.expect(result).to.be.not.null;
            return verifyResultForApi(expectedCount, [url], spy);
        }
    });
...
async function verifyResultForApi(expectedCallCount, expectedArgs, spy) {
    expect_1.expect(spy.callCount).to.equal(expectedCallCount);
    const args = _.flatten(spy.args).slice(expectedCallCount - expectedArgs.length, expectedCallCount);
    expect_1.expect(expectedArgs.sort()).to.deep.equal(args.sort());
}
async function setupCommon(data) {
    await util_1.setupEnzymeTest({ data });
    const appProps = await create_app_props_1.createAppProps({
        page: page_1.Page.EXPLORE,
        urlQuery: {}
    });
    return { appProps };
