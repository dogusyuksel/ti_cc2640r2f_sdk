...
async function getNodeDbIdFromPublicId(publicId, allGroups, urlQuery, apis, masterName) {
    ...
    const dbId = await apis.getNodeDbId(nodePublicId, group ? group.packageGroupPublicUid : null, packagePublicId, group ? package_group_helpers_1.isLatest(group, urlQuery, allGroups) : false, masterName);
    return dbId;
}
...
function getPublicIdFromIds({ nodePublicId, packageGroupPublicUid, packagePublicUid, allGroups, allPackages, urlQuery }) {
    if (!packageGroupPublicUid) {
        ...
        const isLatestVersion = package_group_helpers_1.isLatest(group, urlQuery, allGroups);
...
function getPublicIdData(publicId, allGroups) {
    ...
    const [group = null] = package_group_helpers_1.getPackageGroups([`${packageGroupPublicId}__${packageGroupVersion}`], allGroups);
    if (!group) {
        return null;
    }
