...
function useCloudAgent({ dependencies, errorCallback }) {
    const [getState, setState] = use_state_1.useState(getInitialAgentState());
    return use_async_operation_1.useAsyncOperation({
        operation: async () => {
            try {
                ...
                throw util_1.convertToCloudAgentError(e);
            }
        },
        dependencies,
        errorCallback
    ...
    async function getAgentState() {
        ...
        if (getState().agent) {
            ...
            return {
                agent: null,
                cloudAgentInitState: "UnknownError" /* UNKNOWN_ERROR */,
            ...
            return CloudAgent.Init().then(agent => {
                return {
                    agent,
                    cloudAgentInitState: "Success" /* SUCCESS */,
                    cloudAgentInitErrors: null
                };
            }, err => {
                return {
                    agent: null,
                    cloudAgentInitState: "AgentNotInstalled" /* AGENT_NOT_INSTALLED */,
...
function getInitialAgentState() {
    return {
        agent: null,
        cloudAgentInitState: "NotInitialized" /* NOT_INITIALIZED */,
        cloudAgentInitErrors: null
    };
}
