...
function jumpToNode(appProps, publicId, options = {}) {
    const link = routing_helpers_1.getNodeLink({
        publicId,
...
async function importProject(appProps, publicId) {
    const dbId = await public_id_helpers_1.getNodeDbIdFromPublicId(publicId, appProps.packageGroups, appProps.urlQuery, appProps.apis);
    if (!dbId) {
...
function getUrlQuery(appProps, options) {
    ...
    util_1.getObjectKeys(options).forEach(item => {
        switch (item) {
            case 'chapter':
...
function getNodePublicIdFromNodeInfo(appProps, nodeInfo) {
    ...
    return public_id_helpers_1.getPublicIdFromIds({
