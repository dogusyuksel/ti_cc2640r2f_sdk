...
function sortQuery(query) {
    ...
    const sorted = { ...query };
    util_1.getObjectKeys(query).forEach(key => {
        util_1.setValueForPair(sorted, query, key, item => (Array.isArray(item) ? item.sort() : item));
    });
    return sorted;
}
...
function hasBrowserUrlQueryFilterItemsChanged(previousUrlQuery, nextUrlQuery) {
    ...
    util_1.getObjectKeys({ ...previousUrlQuery, ...nextUrlQuery }).forEach(urlQueryKey => {
        switch (urlQueryKey) {
            case 'compilers':
            case 'devices':
            case 'devtools':
            ...
            case 'nodeType':
                if (previousUrlQuery[urlQueryKey]) {
                    util_1.setValueForPair(previousFilterOnlyUrlQuery, previousUrlQuery, urlQueryKey, item => item);
                }
                if (nextUrlQuery[urlQueryKey]) {
                    util_1.setValueForPair(nextFilterOnlyUrlQuery, nextUrlQuery, urlQueryKey, item => item);
                }
                break;
            ...
            case 'theiaTheme':
                break;
            default:
                util_1.assertNever(urlQueryKey);
...
function getBrowserUrlQueryFilterAsString(urlQuery, includeSelectedNode = false, includeAll = false) {
    ...
    util_1.getObjectKeys(urlQuery).forEach(urlQueryKey => {
        switch (urlQueryKey) {
            case 'compilers':
            case 'devices':
            case 'devtools':
            ...
            case 'r':
            case 'search':
                util_1.setValueForPair(filterOnlyUrlQuery, urlQuery, urlQueryKey, item => item);
                break;
            case 'node':
    ...
    util_1.getObjectKeys(urlQuery).forEach(urlQueryKey => {
        switch (urlQueryKey) {
            ...
            case 'r':
                ...
                filter[filterKey] = value ? package_group_helpers_1.getPackageGroups(value, allGroups) : [];
                break;
            case 'search':
...
function getServerFilterQueryFromBrowserUrlQuery(urlQuery, allGroups, filterOptions) {
    ...
    query.filterPackageGroup = get_package_groups_from_browser_url_query_1.getPackagesGroupsFromBrowserUrlQuery(addedPackageGroups, removedPackageGroups, allGroups, filterOptions).map(group => group.packageGroupPublicUid);
    return query;
}
...
function getUpdatedBrowserUrlQueryItemsFromUpdatedFilterItems(updatedFilterItems) {
    ...
    util_1.getObjectKeys(updatedFilterItems).forEach(key => {
        switch (key) {
            case 'compilers':
            case 'devices':
            case 'devtools':
...
function getUpdatedBrowserUrlQuery(currentUrlQuery, updatedItems) {
    ...
    util_1.getObjectKeys(updatedItems).forEach(filterKey => {
        const item = updatedItems[filterKey];
        if (item) {
            util_1.setValueForPair(urlQuery, updatedItems, filterKey, item => item);
        }
        else if (item === null) {
