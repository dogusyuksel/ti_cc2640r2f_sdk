...
describe('[frontend] use-local-apis', function () {
    ...
    describe('useApi', function () {
        afterEach(() => {
            ...
            mock_agent_1.uninstallCloudAgent();
        });
        it('Should handle agent not available (allowNoAgent = true)', async function () {
            ...
            const hook = () => use_local_apis_1._useApi({
                errorCallback,
                allowNoAgent: true,
                api: () => operation1(),
                dependencies: [],
            ...
            const { result } = await util_2.waitForHookResult(hook, val => hook_tests_1.getResult(val, errorCallback));
            expect_1.expect(result).to.deep.equal(placeholder);
        });
        it('Should handle agent not available (allowNoAgent = false)', async function () {
            ...
            await expect_1.expect(util_2.waitForHookResult(hook, val => hook_tests_1.getResult(val, errorCallback))).to.eventually.be.rejectedWith('Could not get agent');
        });
        it('Should get the result from an api', async function () {
            await mock_agent_1.installCloudAgent({}).Init();
            const hook = () => use_local_apis_1._useApi({
                errorCallback,
                allowNoAgent: false,
                api: () => operation1(),
            ...
            const { result } = await util_2.waitForHookResult(hook, val => hook_tests_1.getResult(val, errorCallback));
            expect_1.expect(result).to.deep.equal(operationResult1);
        });
    });
    describe('useApiWithEvent', function () {
        ...
        it('Should handle agent not available (allowNoAgent = true)', async function () {
            const placeholder = 'Placeholder value';
            const hook = () => use_local_apis_1._useApiWithEvent({
                errorCallback,
                allowNoAgent: true,
                api: () => operation1(),
                dependencies: [],
        ...
        it('Should handle an event', async function () {
            ...
            const { result, wrapper } = await util_2.waitForHookResult(hook, val => hook_tests_1.getResult(val, errorCallback));
            expect_1.expect(result).to.deep.equal(operationResult1);
            // Send event
            util_2.doAct(() => {
            ...
            expect_1.expect(result2).to.deep.equal(updatedResult);
        });
    });
    describe('useGetPackageInstallInfo', function () {
        ...
        hook_tests_1.doCommonTests(() => setup(displayTestOptions), getHook, result => {
            expect_1.expect(result).to.deep.equal(displayTestOptions.installInfo);
        }, errorCallback);
        {
            ...
            const updateTestOptions = {
                ...
                tirexTriggerEvents: {
                    ...
                    ["OnInstallInfoUpdated" /* ON_INSTALL_INFO_UPDATED */]: {
                        delay: EVENT_DELAY_MS,
                        data: updatedInstallInfo
                    }
                }
            };
            hook_tests_1.doUpdateTest(() => setup(updateTestOptions), getHook, (result, done) => {
                expect_1.expect(result).to.deep.equal(done ? updatedInstallInfo : displayTestOptions.installInfo);
            }, errorCallback, EVENT_DELAY_MS);
        }
        // Helpers
        async function setup(options) {
            const agent = await mock_agent_1.installCloudAgent(options).Init();
            ...
            const spies = mock_agent_1.getFullRexCloudAgentModuleSpies();
            return { data: appProps, spy: spies.getPackageInstallInfo };
        }
        function getHook(appProps) {
            return () => use_local_apis_1.useGetPackageInstallInfo({ appProps, errorCallback }).result;
        }
    });
    describe('useGetInstalledPackages', function () {
        ...
        hook_tests_1.doCommonTests(() => setup(displayTestOptions), getHook, result => {
            expect_1.expect(result).to.deep.equal(displayTestOptions.localPackages);
        }, errorCallback);
        {
            ...
            const updateTestOptions = {
                ...
                tirexTriggerEvents: {
                    ...displayTestOptions.tirexTriggerEvents,
                    ["OnInstalledPackagesUpdated" /* ON_INSTALLED_PACKAGES_UPDATED */]: {
                        delay: EVENT_DELAY_MS,
            ...
            hook_tests_1.doUpdateTest(() => setup(updateTestOptions), getHook, (result, done) => {
                expect_1.expect(result).to.deep.equal(done ? updatedInstalledPackages : displayTestOptions.localPackages);
            }, errorCallback, EVENT_DELAY_MS);
        }
        // Helpers
        async function setup(options) {
        ...
        function getHook(appProps) {
            return () => use_local_apis_1.useGetInstalledPackages({ appProps, errorCallback }).result;
        }
    });
    describe('useGetAgentMode', function () {
        ...
        hook_tests_1.doCommonTests(() => setup(displayTestOptions), getHook, result => {
            expect_1.expect(result).to.deep.equal(displayTestOptions.agentMode);
        }, errorCallback);
        it('Should handle agentMode = cloud', async function () {
            ...
            const { result } = await util_2.waitForHookResult(getHook(appProps), val => hook_tests_1.getResult(val, errorCallback));
            expect_1.expect(result).to.deep.equal(options.agentMode);
        });
        // Helpers
        async function setup(options) {
            const agent = await mock_agent_1.installCloudAgent(options).Init();
        ...
        function getHook(appProps) {
            return () => use_local_apis_1.useGetAgentMode({ appProps, errorCallback }).result;
        }
    });
    describe('useGetProgress - REX-729#1', function () {
        ...
        hook_tests_1.doCommonTests(() => setup(displayTestOptions), getHook, result => {
            expect_1.expect(Object.keys(result || {})).to.have.lengthOf(1);
        }, errorCallback);
        hook_tests_1.doUpdateTest(() => setup(displayTestOptions), getHook, (result, done) => {
            result = result || {};
            const keys = Object.keys(result);
            expect_1.expect(keys).to.have.lengthOf(1);
            const progress = result[keys[0]];
            expect_1.expect(progress.isComplete).to.equal(done);
        }, errorCallback, EVENT_DELAY_MS);
        // Helpers
        async function setup(options) {
            const agent = await mock_agent_1.installCloudAgent(options).Init();
            ...
            register1(delay_1.delay(EVENT_DELAY_MS));
            return { data: appProps, spy: spies.getProgress };
        }
        function getHook(appProps) {
            return () => use_local_apis_1.useGetProgress({ appProps, errorCallback }).result;
        }
    });
    describe('useClearTaskProgress', function () {
        ...
        });
        {
            ...
            hook_tests_1.doTriggerTest(async () => {
                const result = await setup(displayTestOptions);
                // Wait until progress done
                await delay_1.delay(EVENT_DELAY_MS * 2);
        ...
        }
        {
            ...
            hook_tests_1.doDependencyTest(async () => {
                const result = await setup(displayTestOptions);
                // Wait until progress done
                await delay_1.delay(EVENT_DELAY_MS * 2);
                deps.progressId = result.data.progressId;
        ...
        async function setup(options) {
            ...
            // Inject progress
            const register1 = await rexModule._addProgressTask();
            const progressId = register1(delay_1.delay(EVENT_DELAY_MS));
            const register2 = await rexModule._addProgressTask();
            const progressId2 = register2(delay_1.delay(EVENT_DELAY_MS));
        ...
        function createHook(deps, appProps) {
            return () => {
                ...
                return use_local_apis_1.useClearTaskProgress({
                    appProps,
                    errorCallback,
                    progressId,
                    trigger
        ...
        async function verifyResult(expectedCallCount, args, spy, result) {
            expect_1.expect(result).to.be.true;
            expect_1.expect(spy.callCount).to.equal(expectedCallCount);
            expect_1.expect(spy.args[expectedCallCount - 1]).to.deep.equal(args);
        }
    });
    describe('useImportProject', function () {
        ...
        {
            const deps = {
                resourceType: "project.ccs" /* CCS */,
                packageUid: package2.packagePublicUid,
                location: '/home/auser/bar',
                targetId: null,
                trigger: false
            };
            hook_tests_1.doTriggerTest(() => setup(displayTestOptions), appProps => createHook(deps, appProps), (count, result, spy) => createVerifyResult(deps, count, result, spy), errorCallback, deps);
        }
        {
            const deps = {
                location: null,
            ...
            hook_tests_1.doWaitUntilOptionsSetTest(() => setup(displayTestOptions), appProps => createHook(deps, appProps), (count, result, spy) => createVerifyResult(deps, count, result, spy), errorCallback, deps, updatedDeps);
        }
        {
            const deps = {
                location: '/home/auser/bar',
            ...
            const updatedDeps = {
                ...
                resourceType: "project.energia" /* ENERGIA */,
                targetId: '8',
                packageUid: package61.packagePublicUid
            };
            hook_tests_1.doDependencyTest(() => setup(displayTestOptions), appProps => createHook(deps, appProps), (count, result, spy) => createVerifyResult(deps, count, result, spy), errorCallback, deps, updatedDeps);
        }
        async function setup(options) {
            const agent = await mock_agent_1.installCloudAgent(options).Init();
            const appProps = await create_app_props_1.createAppProps({
        ...
        function createHook(deps, appProps) {
            return () => {
                ...
                return use_local_apis_1.useImportProject({
                    appProps,
                    errorCallback,
                    resourceType,
                    packageUid,
    ...
    describe('useInstallPackage', function () {
        ...
        function createHook(deps, appProps) {
            return () => {
                ...
                return use_local_apis_1.useInstallPackage({
                    appProps,
                    errorCallback,
                    installLocation,
                    pkg,
    ...
    describe('useUninstallPackage', function () {
        ...
        function createHook(deps, appProps) {
            return () => {
                ...
                return use_local_apis_1.useUninstallPackage({
                    appProps,
                    errorCallback,
                    pkg,
                    trigger
