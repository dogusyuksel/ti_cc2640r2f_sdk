...
class AutoDetect {
    constructor() {
        ...
        this.handleChange = (detectPromise = null) => {
    ...
    maybeDetectDevices(deviceDetector) {
        return deviceDetector.filesNeeded().then(filesNeeded => {
            if (filesNeeded) {
                const result = {
                    type: "HOST_FILES_MISSING" /* HOST_FILES_MISSING */,
                    handler: () => {
    ...
    detectDevices(deviceDetector) {
        return deviceDetector
            .detectDebugProbes()
            .then(debugProbes => promise_utils_1.mapSerially(debugProbes.probes, probe => deviceDetector.detectDeviceWithProbe(probe)))
