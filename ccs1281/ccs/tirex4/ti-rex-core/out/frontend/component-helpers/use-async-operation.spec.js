...
describe('[frontend] useAsyncOperation', function () {
    ...
    it('Should return the result when ready', async function () {
        ...
        const { result } = await util_2.waitForHookResult(() => use_async_operation_1.useAsyncOperation({ operation, dependencies: [], errorCallback }), val => hook_tests_1.getResult(val.result, errorCallback));
        expect_1.expect(result).to.deep.equal(operationResult);
    });
    it('Should report a rejected operation', async function () {
        ...
        await expect_1.expect(util_2.waitForHookResult(() => use_async_operation_1.useAsyncOperation({ operation, dependencies: [], errorCallback }), val => hook_tests_1.getResult(val.result, errorCallback))).to.eventually.be.rejectedWith(operationResult.message);
    });
    it('Should return null initially', function () {
        const operation = operation1;
        const { data } = util_2.getDataFromHook(() => use_async_operation_1.useAsyncOperation({ operation, dependencies: [], errorCallback }));
        expect_1.expect(data.result).to.be.null;
    });
    it('Should update the result when dependencies change', async function () {
        ...
        let pass = true;
        const hook = () => use_async_operation_1.useAsyncOperation({
            operation: () => operation1(pass),
            dependencies: [pass],
            errorCallback
        });
        const { wrapper, result } = await util_2.waitForHookResult(hook, val => hook_tests_1.getResult(val.result, errorCallback));
        expect_1.expect(result).to.deep.equal(operResult);
        expect_1.expect(errorCallback.current.called).to.be.false;
        // Update dep and see if result is updated
        pass = false;
        wrapper.setProps({ hook });
        await expect_1.expect(util_2.waitForHookWrapperResult(wrapper, hook, val => hook_tests_1.getResult(val.result, errorCallback))).to.eventually.be.rejectedWith(operResult2.message);
    });
    it('Should reset the state when the dependencies change (i.e result should be null)', async function () {
        ...
        pass = false;
        wrapper.setProps({ hook });
        const { result: updatedResult } = util_2.getDataFromHookWrapper(wrapper, hook);
        expect_1.expect(updatedResult).to.be.null;
    });
    it('Should not update the state when dependencies change but keepResultsBetweenUpdates is true', async function () {
        ...
        wrapper.setProps({ hook });
        const { result: updatedResult } = util_2.getDataFromHookWrapper(wrapper, hook);
        expect_1.expect(updatedResult).to.equal(operResult);
    });
    it('Should skip updates if multiple updates get queued', async function () {
        ...
        const hook = () => {
            const { dep1 } = deps;
            return use_async_operation_1.useAsyncOperation({
                operation,
                dependencies: [dep1],
                errorCallback
            });
        ...
        const { wrapper, result: result1 } = await util_2.waitForHookResult(hook, val => hook_tests_1.getResult(val.result, errorCallback));
        expect_1.expect(result1).to.equal(1);
        ...
        for (x in _.range(3)) {
            deps.dep1++;
            wrapper.setProps({ hook });
            await delay_1.delay(10);
        }
        // Wait for promises
        let activePromiseIndicies;
        do {
            activePromiseIndicies = promise_syncronization_helpers_1.getActivePromiseIndicies();
            await promise_syncronization_helpers_1.waitForPromisesToResolvePromise(activePromiseIndicies);
        } while (!_.isEmpty(activePromiseIndicies));
        // Expect only 2 updates to go though (initial one, second skipped, then third)
        expect_1.expect(count).to.equal(4);
    });
    it('Should do all updates if doAllUpdates is true', async function () {
        ...
        expect_1.expect(count).to.equal(5);
    });
    // Loading
    it('Should mark something as loading if it takes beyond the threshold wait time', async function () {
        ...
        let count = 1;
        const operation = async () => {
            await delay_1.delay(util_3.LOADING_DELAY_MS * 2);
            return count++;
        };
        const hook = () => {
            const { dep1 } = deps;
        ...
        {
            const { shouldDisplayLoadingUI, result } = util_2.getDataFromHookWrapper(wrapper, hook);
            expect_1.expect(result).to.be.null;
            expect_1.expect(shouldDisplayLoadingUI).to.be.false;
        }
        await delay_1.delay(util_3.LOADING_DELAY_MS);
        {
            const { shouldDisplayLoadingUI, result } = util_2.getDataFromHookWrapper(wrapper, hook);
            expect_1.expect(result).to.be.null;
            expect_1.expect(shouldDisplayLoadingUI).to.be.true;
        }
    });
    it('Should only set initial loading values for first load', async function () {
        ...
        // First load
        {
            const { shouldDisplayInitalLoadingUI, initalLoadInProgress } = util_2.getDataFromHookWrapper(wrapper, hook);
            expect_1.expect(shouldDisplayInitalLoadingUI).to.be.false;
            expect_1.expect(initalLoadInProgress).to.be.true;
        }
        await delay_1.delay(util_3.LOADING_DELAY_MS);
        {
            const { shouldDisplayInitalLoadingUI, initalLoadInProgress, result } = util_2.getDataFromHookWrapper(wrapper, hook);
            expect_1.expect(result).to.be.null;
            expect_1.expect(shouldDisplayInitalLoadingUI).to.be.true;
            expect_1.expect(initalLoadInProgress).to.be.true;
        }
        await promise_syncronization_helpers_1.waitForPromisesToResolvePromise(promise_syncronization_helpers_1.getActivePromiseIndicies());
        // Second load
        deps.dep1++;
        wrapper.setProps({ hook });
        await delay_1.delay(util_3.LOADING_DELAY_MS);
        {
            const { shouldDisplayInitalLoadingUI, initalLoadInProgress, shouldDisplayLoadingUI, result } = util_2.getDataFromHookWrapper(wrapper, hook);
            expect_1.expect(result).to.be.null;
            expect_1.expect(shouldDisplayInitalLoadingUI).to.be.false;
            expect_1.expect(initalLoadInProgress).to.be.false;
            expect_1.expect(shouldDisplayLoadingUI).to.be.true;
        }
    });
    it("Should handle multiple back to back operations well (2 operations who separately don't equal the loading delay but together do)", async function () {
        ...
        let count = 1;
        const operation = async () => {
            await delay_1.delay(util_3.LOADING_DELAY_MS * 0.6);
            return count++;
        };
        const hook = () => {
            const { dep1 } = deps;
        ...
        deps.dep1++;
        wrapper.setProps({ hook });
        await delay_1.delay(util_3.LOADING_DELAY_MS * 0.4);
        {
            const { shouldDisplayLoadingUI, result } = util_2.getDataFromHookWrapper(wrapper, hook);
            expect_1.expect(result).to.be.null;
            expect_1.expect(shouldDisplayLoadingUI).to.be.false;
