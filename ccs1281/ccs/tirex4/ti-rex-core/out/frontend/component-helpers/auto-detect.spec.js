...
describe('[frontend] AutoDetect', () => {
    ...
    afterEach(() => mock_agent_1.uninstallCloudAgent());
    ...
    it('should resolve with files needed if files must be downloaded', async () => {
        ...
        expect_1.expect(result.type).to.be.equal("HOST_FILES_MISSING" /* HOST_FILES_MISSING */);
        if (result.type === "HOST_FILES_MISSING" /* HOST_FILES_MISSING */) {
            expect_1.expect(result.handler).to.exist;
        }
    });
    it('should resolve with detected devices after downloading files', async () => {
        const agent = await mock_agent_1.installCloudAgent({ filesNeeded: 1 }).Init();
    ...
    [
        ...
        it(`should resolve with a cloud agent error at any point (${errorPoint})`, async () => {
            ...
            expect_1.expect(result.type).to.be.equal("UNKNOWN_ERROR" /* UNKNOWN_ERROR */);
            if (result.type === "UNKNOWN_ERROR" /* UNKNOWN_ERROR */) {
                expect_1.expect(result.error).to.be.equal('host communication error');
            }
        });
        if (index >= 2) {
            it(`should resolve with a cloud agent error after downloading files (${errorPoint})`, async () => {
                const agent = await mock_agent_1.installCloudAgent({
                    [errorPoint]: true,
                    filesNeeded: 3
                }).Init();
                const detector = new auto_detect_1.AutoDetect();
    ...
    ['attach', 'detach'].forEach(change => {
        it(`should respond to a ${change} event`, async () => {
            const agent = await mock_agent_1.installCloudAgent({ [change]: 500 }).Init();
            const detector = new auto_detect_1.AutoDetect();
            const changeDetected = new Promise(resolve => detector.addChangeListener(resolve));
            let result = await detector.detect(agent);
            verifySuccess(result);
...
function verifySuccess(result, expectedDevices = mock_device_detector_module_1.initialDevices) {
    expect_1.expect(result.type).to.be.equal("SUCCESS" /* SUCCESS */);
    if (result.type === "SUCCESS" /* SUCCESS */) {
        expect_1.expect(result.detectedDevices).to.be.deep.equal(mock_device_detector_module_1.attachedDevices.slice(0, expectedDevices));
    }
}
