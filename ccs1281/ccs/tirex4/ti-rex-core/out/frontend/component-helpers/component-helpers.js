...
function updateFilter(updatedItems, urlQuery, history) {
    const urlQueryUpdated = filter_helpers_1.getBrowserUrlQueryFromUpdatedFilterItems(updatedItems, urlQuery);
...
function getSelectedGroups(addedPackages, removedPackages, allGroups, filterOptions) {
    let selectedGroups = get_package_groups_from_browser_url_query_1.getPackagesGroupsFromBrowserUrlQuery(addedPackages, removedPackages, allGroups, filterOptions);
