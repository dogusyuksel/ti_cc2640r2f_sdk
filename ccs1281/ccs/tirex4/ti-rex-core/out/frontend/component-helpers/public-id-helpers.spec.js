...
describe('[frontend] PublicIdHelpers', function () {
    before(async () => {
        ...
        await util_2.setupEnzymeTest({ data: data1 });
    });
    after(() => {
        util_2.cleanupEnzymeTest();
    });
    describe('getNodeDbIdFromPublicId', function () {
        it('Should handle a node with no group', async function () {
            ...
            expect_1.expect(dbId).to.deep.equal(folderNodeNoGroup.nodeDbId);
        });
        it('Should handle a node with a group (group is LATEST)', async function () {
            ...
            expect_1.expect(dbId).to.deep.equal(folderNodeWithValidGroup.nodeDbId);
        });
        ...
        it('Should handle an incorrect group being passed in', async function () {
            ...
            expect_1.expect(dbId).to.equal(null);
        });
        it('Should handle a node with a group but no group provided', async function () {
            return expect_1.expect(callMethod(folderNodeWithValidGroup, null, util_1.LATEST))
    ...
    describe('getPublicIdFromIds', function () {
        it('Should handle a node with no group', async function () {
            ...
            expect_1.expect(publicId).to.deep.equal(node.nodePublicId);
        });
        it('Should handle a node with a group (group is LATEST)', async function () {
            const node = folderNodeWithValidGroup;
            const group = packageGroup2;
            const publicId = await callMethod(node);
            expect_1.expect(publicId).to.deep.equal(`${node.nodePublicId}__${group.packageGroupPublicId}__${util_1.LATEST}`);
        });
        it('Should handle a node with a group (group is not LATEST)', async function () {
            const node = folderNodeWithValidGroup;
            const group = packageGroup2;
            ...
            expect_1.expect(publicId).to.deep.equal(`${node.nodePublicId}__${group.packageGroupPublicId}__${group.packageGroupVersion}`);
        });
