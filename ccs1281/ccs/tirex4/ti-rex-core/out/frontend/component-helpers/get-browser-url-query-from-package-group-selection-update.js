...
function getBrowserUrlQueryFromPackageGroupSelectionUpdate(query, pkgGroup, update, useLatest, allGroups, _filterOptions) {
    // Validation
    const isHighestGroupVersion = package_group_helpers_1.isHighestVersion(pkgGroup, allGroups);
    ...
    const addedPackageGroups = package_group_helpers_1.getPackageGroups(query.a || [], allGroups);
    const removedPackageGroups = package_group_helpers_1.getPackageGroups(query.r || [], allGroups);
    // Latest handling
    ...
    const urlQuery = filter_helpers_1.getBrowserUrlQueryFromUpdatedFilterItems(updatedFilterItems, query);
    return urlQuery;
}
...
function applyPackageSelectionUpdateToSameExistingFields(newGroup, update, addedPackageGroups, removedPackageGroups) {
    ...
    else if (update === "Removed" /* Removed */ &&
