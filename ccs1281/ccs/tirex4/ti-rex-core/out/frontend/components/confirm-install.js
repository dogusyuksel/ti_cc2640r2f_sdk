...
const INSTALL_ITEM_TEXT = {
    ...
    ["main" /* MAIN */]: 'requested package',
    ["optional" /* OPTIONAL */]: 'optional',
    ["module" /* MODULE */]: 'optional',
    ["moduleGroup" /* MODULE_GROUP */]: 'optional',
    ["selectedModuleGroup" /* SELECTED_MODULE_GROUP */]: 'yes',
    ["na" /* NA */]: ''
};
const useConfirmInstallStyles = material_ui_styles_imports_1.makeStyles((theme) => {
    const { noFlex, horizontalFlexContainer } = styles_1.stylesCommon(theme);
    return {
        root: {},
...
exports.ConfirmInstall = React.forwardRef((props, ref) => {
    ...
    const moduleDependenciesNoSelected = getState()
        .selection.filter(item => item.installPackageData.installItemType === "moduleGroup" /* MODULE_GROUP */)
    ...
    const packageDependencies = getState()
        ...
        item.installPackageData.installItemType !==
            "selectedModuleGroup" /* SELECTED_MODULE_GROUP */ &&
        item.installPackageData.installItemType !== "main" /* MAIN */)
        .sort(sortByPackageName);
    const renderInstallRow = (selectionData, type) => {
        ...
        return type === 'dependency' ? (React.createElement(ConfirmInstallRow, { appProps: appProps, installedPackages: props.installedPackages, selectionData: selectionData, onChange: (installData, value) => {
                ...
                if (installData.installItemType === "moduleGroup" /* MODULE_GROUP */ &&
                    value &&
                    selectedModuleGroup &&
    ...
    const uniqInstallItems = _.uniqBy(_.flatten(getState()
        ...
        if (item.installPackageData.installItemType ===
            ...
            item.installPackageData.origin === "Remote" /* REMOTE */) {
    ...
    })), item => `${item.data.packagePublicId}__${item.origin !== "NotFound" /* NOT_FOUND */
        ? item.data.packageVersion
        : item.data.versionRange}`);
    const installSize = _.sum(uniqInstallItems.map(item => {
        return item.origin === "Remote" /* REMOTE */
            ? (item.data.installSize && item.data.installSize[util_1.getPlatform()]) || 0
            : 0;
    }));
    return (React.createElement("div", Object.assign({ className: classnames_1.default(classes.root, className) }, rest, { ref: ref }),
        ...
        React.createElement("div", { className: classes.footerContainer },
            React.createElement(index_1.DropDownList, { className: classes.footerItem, name: "Install Location", items: installLocations.map((location, idx) => {
                    return {
                        label: location,
                        id: idx.toString(),
                        elementType: "NonNested" /* NON_NESTED */,
                        onClick: () => {
            ...
            React.createElement(material_ui_imports_1.Typography, { className: classnames_1.default(classes.footerItem, classes.downloadSizeContainer), variant: "subtitle2" },
                "Total download size ",
                util_1.humanFileSize(installSize))),
        getFooterText()));
    function getInitialSelection() {
        const fullSelection = props.installItems.map(item => {
            ...
            if (localPkg) {
                ...
                return { result, support: "unknown" /* UNKNOWN */ };
            }
            ...
            if (remotePackage) {
                ...
                return {
                    result,
                    support: remotePackage.downloadUrl[util_1.getPlatform()]
                        ? "supported" /* SUPPORTED */
        ...
        return fullSelection.map(item => ({
            ...
            selected: item.support !== "notSupported" /* NOT_SUPPORTED */ &&
                item.result.installItemType !== "moduleGroup" /* MODULE_GROUP */,
    ...
    function getUpdatedSelection(installPackageDataOuter, value) {
        return getState().selection.map(({ installPackageData, selected, ...rest }) => {
            if (installPackageData.origin === "NotFound" /* NOT_FOUND */) {
                return installPackageDataOuter.origin === "NotFound" /* NOT_FOUND */ &&
    ...
    function getSelectedUids(keepPlaceholders = false) {
        const uids = getState().selection.map(item => {
            if (!item.selected || item.installPackageData.origin !== "Remote" /* REMOTE */) {
                return '';
            }
    ...
    function getFooterText() {
        ...
        if (_.isEmpty(uids)) {
            ...
            const message = util_1.getPlatform() === response_data_1.Platform.MACOS
...
function ConfirmInstallRow(props) {
    ...
    const installData = selectionData.installPackageData;
    const { nameOrId, version } = getInstallDataPresentationInfo(installData);
    const isDisabled = installData.origin !== "Remote" /* REMOTE */ || support === "notSupported" /* NOT_SUPPORTED */;
    const installSize = installData.origin === "Remote" /* REMOTE */ &&
        installData.data.installSize &&
        installData.data.installSize[util_1.getPlatform()];
    return (React.createElement(material_ui_imports_1.TableRow, Object.assign({ className: classnames_1.default(classes.root, className) }, rest),
        React.createElement(material_ui_imports_1.TableCell, { padding: "checkbox" },
            React.createElement(material_ui_imports_1.Checkbox, Object.assign({ checked: isDisabled
                    ? installData.origin === "Local" /* LOCAL */
        ...
        React.createElement(material_ui_imports_1.TableCell, null, installSize ? util_1.humanFileSize(installSize) : ''),
...
function ConfirmInstallRowModuleGroup(props) {
    ...
    if (isDisabled && !isInstalled) {
        selectionData.support = "notSupported" /* NOT_SUPPORTED */;
    }
    else if (isDisabled) {
        selectionData.support = "unknown" /* UNKNOWN */;
    }
    else {
        selectionData.support = "supported" /* SUPPORTED */;
    }
    ...
    return (React.createElement(material_ui_imports_1.TableRow, Object.assign({ className: classnames_1.default(classes.root, className) }, rest),
        React.createElement(material_ui_imports_1.TableCell, { padding: "checkbox" },
            React.createElement(material_ui_imports_1.Checkbox, Object.assign({ checked: isDisabled ? isInstalled : selectionData.selected, inputProps: {
                    id: util_1.TEST_ID.installSelectCheckbox(installData.data.packagePublicId, version)
                }, onChange: (_evt, value) => {
                    onChange(installData, value);
                } }, (isDisabled ? { disabled: true } : {})))),
    ...
    function getModuleGroupInfo(selectionData) {
        const installData = selectionData.installPackageData;
        if (installData.origin !== "Remote" /* REMOTE */) {
            const { isDisabled, installSize } = getInfoFromInstallData(installData);
            return {
                isDisabled,
                installSize,
                installData,
                isInstalled: installData.origin === "Local" /* LOCAL */
            };
        }
        else {
    ...
    function getInfoFromInstallData(installData) {
        const isDisabled = installData.origin !== "Remote" /* REMOTE */ ||
            !installData.data.downloadUrl[util_1.getPlatform()];
        const installSize = installData.origin === "Remote" /* REMOTE */ &&
            installData.data.installSize &&
            installData.data.installSize[util_1.getPlatform()];
...
function getRowDescription(selectionData) {
    ...
    let extraInfo = false;
    if (selectionData.installPackageData.origin === "NotFound" /* NOT_FOUND */) {
        description = (description ? `${description}` : '') + ' (error: package unavailable)';
        extraInfo = true;
    }
    else if (selectionData.installPackageData.origin === "Local" /* LOCAL */) {
        description =
            (description ? `${description}` : '') +
                ` (package already installed at ${selectionData.installPackageData.data.localPackagePath})`;
        extraInfo = true;
    }
    else if (selectionData.support === "notSupported" /* NOT_SUPPORTED */) {
        description =
...
function getInstallDataPresentationInfo(installData) {
    const nameOrId = installData.origin === "NotFound" /* NOT_FOUND */
        ? installData.data.packagePublicId
        : installData.data.name;
    const version = installData.origin === "NotFound" /* NOT_FOUND */
