...
const useErrorBoundaryInnerStyles = material_ui_styles_imports_1.makeStyles((theme) => {
...
const ErrorBoundaryInner = (props) => {
    ...
    const isDesktop = util_1.fallbackIsDesktop();
    // Hooks
    ...
    return (React.createElement("div", Object.assign({ className: classnames_1.default(classes.root, className), onMouseMove: () => {
            ...
            if (error instanceof errors_1.CloudAgentError || error instanceof errors_1.SessionIdError) {
                ...
                const previousTime = parseInt(localStorage.getItem("LastAutomaticPageRefresh" /* LAST_AUTOMATIC_PAGE_REFRESH */) ||
                    '0', 10) || 0;
                localStorage.setItem("LastAutomaticPageRefresh" /* LAST_AUTOMATIC_PAGE_REFRESH */, currentTime.toString());
                // Only refresh, if we haven't tried to do so in the last 30 seconds
                if (currentTime - previousTime > 30000) {
                    message_1.refreshTirexPage();
                }
            }
        } }, rest),
