...
///////////////////////////////////////////////////////////////////////////////
const TITLE_MAP = {
    [0 /* CONFIRM_INSTALL */]: (pkg) => `Installing ${pkg}`,
    [1 /* LICENSE */]: () => 'Licenses',
    [2 /* INSTALL_PACKAGE */]: () => 'Installing Packages'
};
const NUM_STEPS = Object.keys(TITLE_MAP).length;
const useInstallStyles = material_ui_styles_imports_1.makeStyles((theme) => {
    const {} = styles_1.stylesCommon(theme);
    return {
        root: {},
        button: {
            fontWeight: 400,
...
const _Install = (props, ref) => {
    ...
    // Events
    const onClick = util_1.evtHandler((evt) => {
        evt.preventDefault();
        onOpen();
    ...
    const onClickDoNothing = util_1.evtHandler(() => {
    ...
    const actionText = modifyInstall ? 'Modify Install' : 'Install';
    ...
    return (React.createElement("div", Object.assign({ className: classnames_1.default(classes.root, className) }, rest, { ref: ref }), content));
};
exports.Install = with_cloud_agent_1.withCloudAgent({})(React.forwardRef(_Install));
const InstallInner = (props) => {
    ...
    const { result: installedPackages, shouldDisplayLoadingUI: showLoading1 } = use_local_apis_1.useGetInstalledPackages({
    ...
    const { result: desktopVersion, shouldDisplayLoadingUI: showLoading3 } = use_local_apis_1.useGetVersion({
        appProps: props.appProps,
        errorCallback: props.appProps.errorCallback
    });
    const { result: agentMode } = use_local_apis_1.useGetAgentMode({
        appProps: props.appProps,
        errorCallback: props.appProps.errorCallback
    });
    React.useEffect(() => {
        ...
        if (selectedPackages && installLocation && activeStep === 2 /* INSTALL_PACKAGE */) {
            ...
            const packages = _.uniqBy(_.flatten(selectedPackages.map(packageUid => {
                ...
                if (pkg.moduleGroup) {
                    const modules = confirm_install_1.getModulesFromModuleGroup(pkg.moduleGroup, props.appProps.packages, installedPackages);
                    return modules
                        .filter((item) => item.origin === "Remote" /* REMOTE */)
                        .map(item => item.data);
                }
                else {
                    return [pkg];
            ...
            localStorage.setItem("LastInstallLocation" /* LAST_INSTALL_LOCATION */, installLocation);
        }
    }, [getState().activeStep === 2 /* INSTALL_PACKAGE */, !!installedPackages]);
    React.useEffect(() => {
        // Record the install
        if (!agentMode) {
            return;
        ...
        if (selectedPackages && activeStep === 2 /* INSTALL_PACKAGE */) {
            ...
            packages.map(pkg => {
                analytics_1.handlePackageInstall({ pkg, agentMode }).catch(err => {
                    console.error(err);
                });
            });
        }
    }, [getState().activeStep === 2 /* INSTALL_PACKAGE */, agentMode]);
    ...
    // shouldDisplayLoadingUI: showLoading3
     } = use_local_apis_1.useInstallPackage({
        appProps: props.appProps,
    ...
    else if (activeStep === 0 /* CONFIRM_INSTALL */) {
        contentInner = confirmInstallContent();
    }
    else if (activeStep === 1 /* LICENSE */) {
    ...
    const showNext = (!isLoading && activeStep === 0 /* CONFIRM_INSTALL */) || isLastStep;
    const mainInstallItem = props.installItems.find(item => item.installItemType === "main" /* MAIN */);
    ...
    return (React.createElement(React.Fragment, null,
            ...
            React.createElement(React.Fragment, null,
                isLoading
                    ...
                    : TITLE_MAP[activeStep](mainInstallItemData
                        ...
                        : ''),
                activeStep === 0 /* CONFIRM_INSTALL */ ? (React.createElement(material_ui_imports_1.Typography, { variant: "subtitle2", component: "div" }, "Powered by SDK Composer")) : null)),
        ...
        React.createElement(material_ui_imports_1.DialogActions, null,
            showNext &&
                !shouldBlockNext && (React.createElement(material_ui_imports_1.Button, { id: util_1.TEST_ID.installNextButton, onClick: () => {
    ...
    function installPackageContent() {
        return (React.createElement(material_ui_imports_1.DialogContentText, { id: util_1.TEST_ID.installConfirmationDialog }, props.skipInstallingMessage && activeStep === 2 /* INSTALL_PACKAGE */
            ? ''
    ...
    // Helpers
    function blockNext() {
        if (activeStep === 0 /* CONFIRM_INSTALL */) {
            const { selectedPackages } = getState();
...
function getInstallItemsFromRequestedItem(packagePublicUid, packages) {
    ...
    const { moduleGroups, modules, dependencies } = pkg;
    const installItems = [
        {
            packagePublicId: pkg.packagePublicId,
            versionRange: pkg.packageVersion,
            installItemType: "main" /* MAIN */
        },
        ...moduleGroups.map(item => {
            ...
            const moduleGroup = moduleGroupPkg && moduleGroupPkg.moduleGroup && moduleGroupPkg.moduleGroup.packages;
            const tempFullSDK = moduleGroupPkg && pkg && moduleGroupPkg.name === pkg.name; // TODO remove, workaround for 4.12.0 not having a way to identify the full sdk (in the case there are multiple packages with all the groups)
            return {
                packagePublicId: item.packagePublicId,
                versionRange: item.versionRange,
                installItemType: moduleGroup && moduleGroup.length === modules.length + 1 && tempFullSDK
                    ? "selectedModuleGroup" /* SELECTED_MODULE_GROUP */
                    : "moduleGroup" /* MODULE_GROUP */
            };
        }),
        ...dependencies.map(item => {
            let installItemType = "na" /* NA */;
            switch (item.dependencyType) {
                case "mandatory" /* MANDATORY */:
                    installItemType = "mandatory" /* MANDATORY */;
                    break;
                case "optional" /* OPTIONAL */:
                    installItemType = "optional" /* OPTIONAL */;
                    break;
                default:
                    util_2.assertNever(item.dependencyType);
                    if (util_1.isBrowserEnvironment()) {
...
function useGetInstallLocations({ appProps }) {
    const { result: installLocations, ...rest } = use_local_apis_1.useGetPackageInstallInfo({
        appProps,
        errorCallback: appProps.errorCallback
    });
    const defaultLocation = localStorage.getItem("LastInstallLocation" /* LAST_INSTALL_LOCATION */) || '';
    if (!installLocations) {
