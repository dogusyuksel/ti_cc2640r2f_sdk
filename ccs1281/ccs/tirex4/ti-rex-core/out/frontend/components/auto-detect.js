    ...
    if (progress) {
        ...
        switch (detectionResult.type) {
            case "SUCCESS" /* SUCCESS */:
                ...
                break;
            case "HOST_FILES_MISSING" /* HOST_FILES_MISSING */:
                content = (React.createElement(material_ui_imports_1.Tooltip, { title: "Additional files are required to detect your device(s)" },
                    React.createElement(material_ui_imports_1.Button, { id: util_2.TEST_ID.autoDetectButton, variant: "outlined", onClick: util_2.evtHandler(() => {
            ...
            case "UNKNOWN_ERROR" /* UNKNOWN_ERROR */:
                ...
                break;
            default:
                util_1.assertNever(detectionResult);
                throw new Error('Unknown result');
        }
    }
    return content;
};
exports.AutoDetect = with_cloud_agent_1.withCloudAgent({})(React.forwardRef(_AutoDetect));
function useAutoDetect(props) {
    ...
    // Events
    const onChanged = React.useCallback(util_2.evtHandler(() => {
        getState().updateCounter.setValue();
        setState({ forceUpdate: {} });
    }, props.appProps.errorCallback), []);
    const onProgress = React.useCallback(util_2.evtHandler((progress) => {
    ...
    const { result: detectionResultInner, ...rest } = use_async_operation_1.useAsyncOperation({
        operation: async () => {
            if (!agent) {
                return null;
            }
