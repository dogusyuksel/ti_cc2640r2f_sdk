...
const packageMsp = {
    ...
    packageType: "MainPackage" /* MAIN_PACKAGE */,
    dependencies: [],
    isInstallable: true,
...
const packageSla = {
    ...
    packagePublicUid: '7',
    packageGroupPublicUids: [PACKAGE_GROUP_PUBLIC_UID_1],
    packageType: "SubPackage" /* SUB_PACKAGE */,
    dependencies: [
        {
...
const leafNode = {
    ...
    descriptor: {
        ...
        icon: "SourceCode_C" /* SOURCE_CODE_C */
    },
    contentType: "Other" /* OTHER */,
    packagePublicUid: 'foo',
    packageGroupPublicUid: 'pgrp_foo',
...
describe('[frontend] ServerInterface', function () {
    ...
    this.timeout(10000);
    before(() => browser_emulator_1.browserEmulator());
    describe("api/nodesData" /* GET_NODES_DATA */, function () {
        let ajaxHarness;
        let serverInterface;
        beforeEach(function () {
        ...
        function validateLeafNode(inputNode, hasChildren, resultNode) {
            expect_1.expect(resultNode).to.exist;
            ...
            expect_1.expect(resultNode).to.deep.equal(expectNode);
        }
    ...
    describe("api/nodeExtendedData" /* GET_NODE_EXTENDED_DATA */, function () {
        let ajaxHarness;
        let serverInterface;
        beforeEach(function () {
            serverInterface = new server_interface_1.ServerInterface();
        ...
        it('Should handle a request for the root node by rejecting it', async function () {
            await ajaxHarness.setupTestFakeServer(data1);
            return expect_1.expect(serverInterface.getExtendedNodesData('1'))
                .to.eventually.be.rejectedWith(errors_1.NetworkError)
                .and.eventually.have.property('statusCode', '400');
        });
        it('Should handle an invalid node id', async function () {
            await ajaxHarness.setupTestFakeServer(data1);
            return expect_1.expect(serverInterface.getNodesData([folderNode3.nodeDbId]))
                .to.eventually.be.rejectedWith(errors_1.NetworkError)
                .and.eventually.have.property('statusCode', '404');
        });
        it('Should handle no node ids provided by rejecting it', async function () {
            await ajaxHarness.setupTestFakeServer(data1);
            return expect_1.expect(serverInterface.getExtendedNodesData(''))
                .to.eventually.be.rejectedWith(errors_1.NetworkError)
                .and.eventually.have.property('statusCode', '400');
        });
        it('Should handle all node types and variants', async function () {
    ...
    describe("api/filteredChildrenNodeIds" /* GET_FILTERED_CHILDREN_NODE_IDS */, function () {
        let ajaxHarness;
        let serverInterface;
        beforeEach(function () {
            serverInterface = new server_interface_1.ServerInterface();
        ...
        it('Should handle a non-existent parent id', async function () {
            await ajaxHarness.setupTestFakeServer(data2);
            return expect_1.expect(serverInterface.getFilteredChildrenNodeIds([folderNode3.nodeDbId], {
                filterPackageGroup: [],
                filterDevice: [device1.name]
            }))
                .to.eventually.be.rejectedWith(errors_1.NetworkError)
        ...
        function validateResult(result, expectedResultIds) {
            expect_1.expect(result).to.exist;
            const { parentToChildDbId } = result;
            expect_1.expect(parentToChildDbId).to.exist;
            Object.keys(parentToChildDbId).map(id => {
                const resultIds = parentToChildDbId[id];
                if (expectedResultIds[id].length > 0) {
                    expect_1.expect(resultIds).to.deep.equal(expectedResultIds[id]);
                }
                else {
                    expect_1.expect(resultIds).to.be.empty;
                }
            });
        }
    });
    describe("api/expandedFilteredDescendantNodesData" /* GET_EXPANDED_FILTERED_DESCENDANT_NODES_DATA */, function () {
        let ajaxHarness;
        let serverInterface;
        beforeEach(function () {
            serverInterface = new server_interface_1.ServerInterface();
        ...
        it('Should handle an invalid node id', async function () {
            await ajaxHarness.setupTestFakeServer(data3);
            return expect_1.expect(serverInterface.getExpandedFilteredDescendantNodesData('4', {
                filterPackageGroup: [],
                filterDevice: [device1.name]
            }))
                .to.eventually.be.rejectedWith(errors_1.NetworkError)
    ...
    describe("api/filteredTableItemsData" /* GET_FILTERED_TABLE_ITEMS_DATA */, function () {
        const data = {
        ...
        function validateResult(result, expectedItems, requestedParent) {
            ...
            expect_1.expect(_.isEmpty(children)).to.be.false;
            expect_1.expect(children.map(item => item.tableItemDbId)).to.deep.equal(expectedItems.map(item => item.tableItemDbId));
        }
    });
    describe("api/nodeDataForTableItemVariant" /* GET_NODE_DATA_FOR_TABLE_ITEM_VARIANT */, function () {
        let ajaxHarness;
        let serverInterface;
        beforeEach(function () {
            serverInterface = new server_interface_1.ServerInterface();
        ...
        function validateResult(result, variant) {
            ...
            expect_1.expect(nodeData.nodeDbId).to.deep.equal(variant.nodeDbId);
        }
    });
    describe("api/filterOptions" /* GET_FILTER_OPTIONS */, function () {
        let ajaxHarness;
        let serverInterface;
        beforeEach(function () {
            serverInterface = new server_interface_1.ServerInterface();
        ...
        function validateResult(result, data) {
            ...
            expect_1.expect(result).to.deep.equal(expected);
        }
    });
    describe.skip("api/rootNode" /* GET_ROOT_NODE */, function () { });
    describe("api/searchSuggestions" /* GET_SEARCH_SUGGESTIONS */, function () {
        let ajaxHarness;
        let serverInterface;
        beforeEach(function () {
            serverInterface = new server_interface_1.ServerInterface();
        ...
        it('Should get the suggestions for something with matches', async function () {
            ...
            expect_1.expect(result.sort()).to.deep.equal(['device-foo', 'devtool-foo', 'gcc-foo'].sort());
        });
        it('Should handle something without matches (returns empty array)', async function () {
            ...
            expect_1.expect(result).to.have.length(0);
        });
    });
    describe("api/packages" /* GET_PACKAGES */, function () {
        let ajaxHarness;
        let serverInterface;
        beforeEach(function () {
            serverInterface = new server_interface_1.ServerInterface();
        ...
        function validateResult(result, data) {
            expect_1.expect(result).to.deep.equal(data);
        }
    });
    describe("api/packageGroups" /* GET_PACKAGE_GROUPS */, function () {
        let ajaxHarness;
        let serverInterface;
        beforeEach(function () {
            serverInterface = new server_interface_1.ServerInterface();
        ...
        it('Should get a list of package groups', async function () {
            ...
            expect_1.expect(result).to.deep.equal([packageGroup1]);
        });
        it('Should get a list of package groups (auto generated package groups)', async function () {
            ...
            await ajaxHarness.setupTestFakeServer(data);
            const result = await serverInterface.getPackageGroups();
            expect_1.expect(result).to.have.length(1);
            expect_1.expect(result[0].packagesPublicUids.sort()).to.deep.equal([
        ...
        it('Should stable sort the groups by version (newest to oldest)', async function () {
            ...
            await ajaxHarness.setupTestFakeServer(data);
            const result = await serverInterface.getPackageGroups();
            expect_1.expect(result).to.deep.equal([packageGroup2, packageGroup1, packageGroup3]);
        });
    });
    describe.skip("api/nodeDownload" /* GET_NODE_DOWNLOAD */, function () { });
    describe.skip("api/importProject" /* GET_IMPORT_PROJECT */, function () { });
    describe("api/importInfo" /* GET_IMPORT_INFO */, function () {
        let ajaxHarness;
        let serverInterface;
        beforeEach(function () {
            serverInterface = new server_interface_1.ServerInterface();
        ...
        it('Should handle an invalid node id', async function () {
            await ajaxHarness.setupTestFakeServer(data1);
            return expect_1.expect(serverInterface.getImportInfo(folderNode3.nodeDbId, {
                filterPackageGroup: []
            }))
                .to.be.eventually.be.rejectedWith(errors_1.NetworkError)
                .and.eventually.have.property('statusCode', '404');
        ...
        function validateResult(result, expectedItems) {
            expect_1.expect(result.targets.sort()).to.deep.equal(expectedItems.sort());
        }
    });
    describe.skip("api/serverConfig" /* GET_SERVER_CONFIG */, function () { });
    describe("api/nodePublicIdToDbId" /* GET_NODE_PUBLIC_ID_TO_DB_ID */, function () {
        let ajaxHarness;
        let serverInterface;
        beforeEach(function () {
            serverInterface = new server_interface_1.ServerInterface();
        ...
        it('Should handle a node which exists and has a group', async function () {
            ...
            expect_1.expect(nodeDbId).to.deep.equal(node.nodeDbId);
        });
        ...
        async function negativeTest(nodePublicId, packageGroupPublicUid, packagePublicId) {
            return expect_1.expect(serverInterface.getNodePublicIdToDbId(nodePublicId, packageGroupPublicUid, packagePublicId, false))
                .to.be.eventually.rejectedWith(errors_1.NetworkError)
                .and.eventually.have.property('statusCode', '404');
        }
    });
    describe.skip("api/rex3LinkToDbId" /* GET_REX3_LINK_TO_DB_ID */, function () { });
});
