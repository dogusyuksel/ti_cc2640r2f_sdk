...
class APIsInternal {
    constructor() {
        this.serverInterface = new server_interface_1.ServerInterface();
        this.cacheInterface = new apis_cache_interface_1.ApisCacheInterface();
    ...
    getNodeInfoForResourceId(query) {
        const cacheKey = QueryString.stringify({ ...filter_helpers_1.sortQuery(query) });
    ...
    static measureTime(promise, label) {
        ...
        return util_1.measureTime(promise, label);
    }
    /**
    ...
    getChildrenNodesData(childrenNodeIds) {
        ...
        return this.getNodes(_.chain(parentIds)
            ...
            return util_1.objectFromKeyValuePairs(Object.entries(getParentIndices()).map(([parentId, { start, end }]) => ({
                key: parentId,
        ...
        function getParentIndices() {
            ...
            return util_1.objectFromKeyValuePairs(parentIndcies.map(({ parentId, startEnd }) => ({ key: parentId, value: startEnd })));
        }
    }
    /**
    ...
    getFilteredChildrenNodeIds(parentIds, query, cacheKeys) {
        return this.serverInterface.getFilteredChildrenNodeIds(parentIds, query).then(result => {
            ...
            return util_1.objectFromKeyValuePairs(pairs.map(({ parentId, childrenIds }) => ({ key: parentId, value: childrenIds })));
        });
    }
    /**
    ...
    getCacheKeyForTableItemVariant(tableItemId, query, variant) {
        ...
        const variantToString = util_2.getTableItemVariantAsString(variant);
