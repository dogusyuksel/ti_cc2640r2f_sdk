...
describe('[frontend] APIs', function () {
    ...
    describe('getNodes', function () {
        ...
        it('Should fill the caches with the appropriate data', async function () {
            ...
            expect_1.expect(Object.keys(data.cacheInterface.getNodesDataBulk(nodeIds).nodesData)).to.deep.equal(nodeIds);
            expect_1.expect(nodes.map(item => {
        ...
        it('Should only call getNodes for the missing nodes', async function () {
            ...
            expect_1.expect(getNodesDataSpy.calledOnce).to.equal(true);
            const args = await data.apis.getNodes([packageNode2.nodeDbId, packageNode3.nodeDbId]);
            expect_1.expect(getNodesDataSpy.callCount).to.equal(2);
            validateNode(args, [packageNode2.nodeDbId, packageNode3.nodeDbId]);
            expect_1.expect(getNodesDataSpy.getCall(1).args[0]).to.deep.equal([packageNode3.nodeDbId]);
        });
        function validateNode(result, expectedIds) {
            const ids = result.map(node => node.nodeDbId);
            expect_1.expect(ids).to.deep.equal(expectedIds);
        }
    });
    describe('getExtendedNodes', function () {
        ...
        it('Should fill the caches with the appropriate data', async function () {
            ...
            expect_1.expect(fromCache && fromCache.nodeDbId).to.deep.equal(node.nodeDbId);
        });
    });
    describe('getFilteredChildrenNodes', function () {
        ...
        it('Should only call the server for relations if we called it with another filter with the same parent id', async function () {
            ...
            expect_1.expect(getFilteredChildrenNodeIdsSpy.calledOnce).to.equal(true);
            expect_1.expect(getNodesDataSpy.calledOnce).to.equal(true);
            const args = await data.apis.getFilteredChildrenNodes([packageNode2.nodeDbId], {
                devices: [device2.publicId]
            });
            expect_1.expect(getFilteredChildrenNodeIdsSpy.callCount).to.equal(2);
            expect_1.expect(getNodesDataSpy.callCount).to.equal(2);
        ...
        it('Should only call the server for parents not in the cache', async function () {
            ...
            expect_1.expect(getFilteredChildrenNodeIdsSpy.callCount).to.equal(2);
            expect_1.expect(getNodesDataSpy.callCount).to.equal(2);
            expect_1.expect(getFilteredChildrenNodeIdsSpy.getCall(1).args[0]).to.deep.equal([
        ...
        it('Should only call the server for relations not in the cache', async function () {
            ...
            expect_1.expect(getFilteredChildrenNodeIdsSpy.callCount).to.equal(3);
            expect_1.expect(getNodesDataSpy.callCount).to.equal(2);
            expect_1.expect(getFilteredChildrenNodeIdsSpy.getCall(2).args[0]).to.deep.equal([
        ...
        it('Should only call the server for data not in the cache', async function () {
            ...
            expect_1.expect(getFilteredChildrenNodeIdsSpy.calledOnce).to.equal(true);
            expect_1.expect(getNodesDataSpy.callCount).to.equal(3);
            expect_1.expect(getNodesDataSpy.getCall(2).args[0]).to.deep.equal([packageNode6.nodeDbId]);
            validateNode(args, [packageNode6.nodeDbId, packageNode5.nodeDbId]);
        });
        function validateNode(result, expectedIds) {
            const ids = result
            ...
            expect_1.expect(result).to.be.lengthOf(expectedIds.length);
            expect_1.expect(ids).to.deep.equal(expectedIds);
        }
    });
    describe('expandNode', function () {
        ...
        it('Should fill the caches with the appropriate data', async function () {
            ...
            expect_1.expect(data.cacheInterface.getFilteredChildrenNodeIds(apis_cache_interface_1.ApisCacheInterface.urlQueryCacheKey(rootNode, params))).to.deep.equal([packageNode2.nodeDbId]);
            expect_1.expect(data.cacheInterface.getFilteredDescendentNodeIds(apis_cache_interface_1.ApisCacheInterface.urlQueryCacheKey(rootNode, params))).to.deep.equal([packageNode2.nodeDbId, packageNode3.nodeDbId]);
            expect_1.expect(Object.keys(data.cacheInterface.getNodesDataBulk([
                packageNode2.nodeDbId,
                packageNode3.nodeDbId
            ]).nodesData)).to.deep.equal([packageNode2.nodeDbId, packageNode3.nodeDbId]);
            expect_1.expect([packageNode2, packageNode3].map(item => {
                const pkg = pkgs.find(pkg => pkg.packagePublicUid === item.packagePublicUid);
                return data.cacheInterface.getNodeDbId({
                    nodePublicId: item.nodePublicId,
                    packageGroupPublicUid: item.packageGroupPublicUid ||
        ...
        it('Should call the server again if we have the data but not the relations', async function () {
            await data.ajaxHarness.setupTestFakeServer(data1);
            await data.apis.expandNode(rootNode, { devices: [device1.publicId] });
            expect_1.expect(getExpandedFilteredDescendantNodesDataSpy.calledOnce).to.equal(true);
            expect_1.expect(getNodesDataSpy.callCount).to.equal(0);
            data.cacheInterface._clearFilteredDescendentNodeIdsCache();
            await data.apis.expandNode(rootNode, { devices: [device1.publicId] });
            expect_1.expect(getExpandedFilteredDescendantNodesDataSpy.callCount).to.equal(2);
            expect_1.expect(getNodesDataSpy.callCount).to.equal(0);
        });
        it('Should only call the server for the data if we have all the relations', async function () {
            ...
            await data.apis.expandNode(rootNode, { devices: [device1.publicId] });
            expect_1.expect(getExpandedFilteredDescendantNodesDataSpy.calledOnce).to.equal(true);
            expect_1.expect(getNodesDataSpy.callCount).to.equal(1);
        });
    });
    describe('acquireControl & releaseControl', function () {
        ...
        it('Should allow you to call APIs after acquiring control', async function () {
            ...
            await expect_1.expect(apiPromise).to.eventually.be.fulfilled;
        });
        it('Should only give you control when the previous master releases control', async function () {
            ...
            setTimeout(() => {
                expect_1.expect(hasControl).to.be.false;
            ...
            await Promise.all([
                new Promise(async function (resolve, reject) {
                    try {
                        await expect_1.expect(acquirePromise2).to.be.eventually.fulfilled;
                        hasControl = true;
                        resolve();
                    }
                    catch (e) {
        ...
        it('Should not allow API calls without a master to go through until the current master releases control', async function () {
            ...
            await Promise.all([
                new Promise(async function (resolve, reject) {
                    try {
                        await expect_1.expect(apiPromise).to.be.eventually.fulfilled;
                        hasControl = true;
                        resolve();
                    }
                    catch (e) {
        ...
        it('Should not allow API calls with a different master to go through', async function () {
            ...
            await expect_1.expect(apiPromise).to.be.eventually.rejected;
        });
    });
});
///////////////////////////////////////////////////////////////////////////////
