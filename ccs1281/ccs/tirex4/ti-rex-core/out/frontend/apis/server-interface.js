...
class ServerInterface {
    constructor() {
        ...
        // Promise trackers
        // Nodes
        this.nodesDataPromises = new promise_tracker_1.PromiseTracker();
        this.extendedNodesDataPromises = new promise_tracker_1.PromiseTracker();
        this.filteredChildrenNodesPromises = new promise_tracker_1.PromiseTracker();
        this.filteredTableItemsDataPromises = new promise_tracker_1.PromiseTracker();
        this.nodeDataForTableItemVariantPromises = new promise_tracker_1.PromiseTracker();
        this.tableViewFiltersPromises = new promise_tracker_1.PromiseTracker();
        this.expandedFilteredDescendantNodesDataPromises = new promise_tracker_1.PromiseTracker();
        this.searchSuggestionsPromises = new promise_tracker_1.PromiseTracker();
        // Actions
        this.importInfoPromises = new promise_tracker_1.PromiseTracker();
        // Basic data
        this.packagesPromises = new promise_tracker_1.PromiseTracker();
        this.packageGroupsPromises = new promise_tracker_1.PromiseTracker();
        this.filterOptionsPromises = new promise_tracker_1.PromiseTracker();
        // Other
        this.nodePublicIdToDbIdPromises = new promise_tracker_1.PromiseTracker();
        this.rex3LinkToDbIdPromises = new promise_tracker_1.PromiseTracker();
        this.searchResultsPagePromises = new promise_tracker_1.PromiseTracker();
        this.nodeInfoForResourceIdPromises = new promise_tracker_1.PromiseTracker();
        this.nodeInfoForGlobalIdPromises = new promise_tracker_1.PromiseTracker();
    }
    ///////////////////////////////////////////////////////////////////////////////
    /// Public methods
    ///////////////////////////////////////////////////////////////////////////////
    ...
    getNodesData(ids) {
        ...
        const url = `${"api/nodesData" /* GET_NODES_DATA */}?${queryString}`;
        return this.getFromUrl(url, this.nodesDataPromises);
    }
    getExtendedNodesData(id) {
        const params = { dbId: id };
        const queryString = QueryString.stringify(params);
        const url = `${"api/nodeExtendedData" /* GET_NODE_EXTENDED_DATA */}?${queryString}`;
    ...
    getFilteredChildrenNodeIds(parentIds, query) {
        ...
        const queryString = QueryString.stringify(params);
        const url = `${"api/filteredChildrenNodeIds" /* GET_FILTERED_CHILDREN_NODE_IDS */}?${queryString}`;
    ...
    getExpandedFilteredDescendantNodesData(parentId, query) {
        ...
        const queryString = QueryString.stringify(params);
        const url = `${"api/expandedFilteredDescendantNodesData" /* GET_EXPANDED_FILTERED_DESCENDANT_NODES_DATA */}?${queryString}`;
        ...
        if (util_1.getServerConfig().offline) {
            ...
            if (devicePublicId) {
                ...
                if (offlineDevice) {
                    ...
                    tableItems = lodash_1.default(tableItemsBySortKey)
                        .entries()
                        .sortBy(([k]) => k)
                        .map(([, v]) => v)
                        .value();
            ...
            return Promise.resolve({
                parentToChildDbId: {
                    [util_1.getServerConfig().rootNodeDbId]: tableItems
                }
            });
        }
        else {
            ...
            const queryString = QueryString.stringify(params);
            const url = `${"api/filteredTableItemsData" /* GET_FILTERED_TABLE_ITEMS_DATA */}?${queryString}`;
    ...
    async getTableViewFilters(parentId, query, isProjectWizard) {
        if (util_1.getServerConfig().offline) {
            ...
            const queryString = QueryString.stringify(params);
            const url = `${"api/tableViewFilters" /* GET_TABLE_VIEW_FILTERS */}?${queryString}`;
    ...
    getNodeDataForTableItemVariant(tableItemDbId, query, variant) {
        if (util_1.getServerConfig().offline) {
            ...
            return Promise.resolve({ dbIdToData: { '2': node } });
        }
        else {
            const params = {
                tableItemDbId,
            ...
            const queryString = QueryString.stringify(params);
            const url = `${"api/nodeDataForTableItemVariant" /* GET_NODE_DATA_FOR_TABLE_ITEM_VARIANT */}?${queryString}`;
            return this.getFromUrl(url, this.nodeDataForTableItemVariantPromises);
        }
    }
    getSearchSuggestions(text, query) {
        query = filter_helpers_1.sortQuery(query);
        ...
        const queryString = QueryString.stringify(params);
        const url = `${"api/searchSuggestions" /* GET_SEARCH_SUGGESTIONS */}?${queryString}`;
    ...
    getImportInfo(id, query) {
        ...
        const queryString = QueryString.stringify(params);
        const url = `${"api/importInfo" /* GET_IMPORT_INFO */}?${queryString}`;
    ...
    getPackages() {
        if (util_1.getServerConfig().offline) {
            ...
            const url = "api/packages" /* GET_PACKAGES */;
    ...
    getPackageGroups() {
        if (util_1.getServerConfig().offline) {
            ...
            const url = "api/packageGroups" /* GET_PACKAGE_GROUPS */;
    ...
    getFilterOptions() {
        if (util_1.getServerConfig().offline) {
            ...
            const url = "api/filterOptions" /* GET_FILTER_OPTIONS */;
    ...
    getNodeInfoForResourceId({ resourceId, packageId, packageVersion, device, devtool }) {
        ...
        const queryString = QueryString.stringify(params);
        const url = `${"api/getNodeInfoForResourceId" /* GET_NODE_INFO_FOR_RESOURCE_ID */}?${queryString}`;
    ...
    getNodeInfoForGlobalId({ globalId, device, devtool }) {
        ...
        const queryString = QueryString.stringify(params);
        const url = `${"api/getNodeInfoForGlobalId" /* GET_NODE_INFO_FOR_GLOBAL_ID */}?${queryString}`;
    ...
    getNodePublicIdToDbId(nodePublicId, packageGroupPublicUid, packagePublicId, isLatest) {
        ...
        const queryString = QueryString.stringify(params);
        const url = `${"api/nodePublicIdToDbId" /* GET_NODE_PUBLIC_ID_TO_DB_ID */}?${queryString}`;
    ...
    getRex3LinkToDbId(linkField) {
        ...
        const queryString = QueryString.stringify(params);
        const url = `${"api/rex3LinkToDbId" /* GET_REX3_LINK_TO_DB_ID */}?${queryString}`;
