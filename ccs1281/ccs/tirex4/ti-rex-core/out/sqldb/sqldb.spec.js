...
describe('SQLDB', function () {
    ...
    suite('Standard', () => {
        ...
        suite('resources', () => {
            test('fields_test_compiler_kernel', 'should have compiler and kernel fields in the resources', async () => {
                ...
                expect_1.expect(records).to.not.be.empty;
            });
            test('fields_test_override_projspec_device', 'should have override_projspec_device in resources', async () => {
                ...
                expect_1.expect(records).to.be.empty;
                ...
                for (const testArgs of packageTreeTestData) {
                    test(testArgs.testName, testArgs.testDescription, async () => {
                        ...
                        if (testArgs.shouldExist) {
                            expect_1.expect(node).to.exist;
                        }
                        else {
                            expect_1.expect(node).to.not.exist;
                        }
                    });
                }
            };
        ...
        suite('custom ids', () => {
            ...
            function verifyNode(node, args) {
                ...
                expect_1.expect(node.hashedNodePublicId, 'node public id').to.equal(args.expectedNodePublicId);
                if (args.expectedPackageGroup) {
                    expect_1.expect(node.packageGroup, 'package group').to.include(args.expectedPackageGroup);
                }
                if (args.expectedPackage) {
                    expect_1.expect(node.package, 'package').to.include(args.expectedPackage);
                }
            }
            for (const args of mainTestArgsList) {
                function verifySingleNode(nodes) {
                    expect_1.expect(nodes).to.have.lengthOf(1);
                ...
                if (args.hasDevice) {
                    test(baseTestName + 'AndDevice', baseTestDescr + ' and devices', async () => {
                        ...
                        if (args.notDevice) {
                            ...
                            expect_1.expect(nodes).to.be.empty;
                        }
                    });
                }
                if (args.hasDevtool) {
            ...
            for (const args of multiDeviceNodeTestArgsList) {
                ...
                test(baseTestName + 'AndMultiDeviceNodes2', baseTestDescr + ' but no devices when resource has multiple device nodes', async () => {
                    ...
                    expect_1.expect(nodes).to.have.lengthOf(3);
        ...
        suite('table view', () => {
            ...
            test('tableView2', "should be possible to get a package node's table view rows", async () => {
                ...
                expect_1.expect(tableViewItems).to.not.be.empty;
                expect_1.expect(_.find(tableViewItems, o => o.name === 'CCS Project -- DSS' &&
            ...
            test('tableView3', "should be possible to get a package node's table view rows filtered on keywords and " +
                ...
                expect_1.expect(tableViewItems).to.not.be.empty;
                expect_1.expect(tableViewItems).to.have.lengthOf(1);
                expect_1.expect(tableViewItems[0]).to.include({
                    name: 'CCS Project -- DSS',
            ...
            test('tableView4', "should be possible to get a package node's table view rows filtered on two compilers", async () => {
                ...
                expect_1.expect(tableViewItem).to.include({
                    name: 'network_terminal'
                });
                expect_1.expect(tableViewItem.variants).to.include.deep.members([
            ...
            test('tableView5', 'should be possible to get the expected nodes for a table view item with several variants', async () => {
                ...
                expect_1.expect(packageGroupIds).to.have.lengthOf(1);
                const allVariants = [
                ...
                for (const compiler of [undefined, 'iar', 'gcc', 'ccs']) {
                    ...
                    expect_1.expect(networkTermItems).to.have.lengthOf(1);
                    ...
                    expect_1.expect(networkTermItem).to.include({
                    ...
                    expect_1.expect(networkTermItem.variants).to.have.deep.members(expectedVariants);
                    const nodeIds = [];
                    for (const variant of expectedVariants) {
                        const node = await rexdb.tableViews.getNode(networkTermItem.id, { deviceId }, variant);
                        expect_1.expect(node).to.exist.and.to.include({
                            name: 'network_terminal'
                        });
                        if (variant.compiler === 'iar') {
                            expect_1.expect(node).to.include({
                                resourceType: 'project.iar'
                            });
                            expect_1.expect(node.readmeNodePublicId).to.not.exist;
                        }
                        else {
                            expect_1.expect(node).to.include({
                                resourceType: 'projectSpec'
                            });
                            expect_1.expect(node.readmeNodePublicId).to.exist;
                        }
                        nodeIds.push(node.id);
                    }
                    expect_1.expect(_.uniq(nodeIds)).to.have.lengthOf(expectedVariants.length, 'expected each variant to have a unique node');
                }
            });
        });
        suite('delete', () => {
            ...
            test('deletePackageGroup', 'should be possible to delete a package group', async () => {
                ...
                expect_1.expect(packageGroup.state).to.equal('published');
                _.each(await getPackageGroupChildRowCounts(packageGroup.id), (count, table) => {
                    expect_1.expect(count, `${table}-count`).to.be.above(0);
                });
                expect_1.expect(otherPackageGroup.state).to.equal('published');
                ...
                expect_1.expect((await getPackageGroup(packageGroupToDeleteUid)).state).to.equal('unpublished');
                ...
                expect_1.expect((await getPackageGroup(packageGroupToDeleteUid)).state).to.equal('deleted');
                _.each(await getPackageGroupChildRowCounts(packageGroup.id), (count, table) => {
                    expect_1.expect(count, `${table}-count`).to.equal(0);
                });
                expect_1.expect(await rexdb.manage.checkDatabaseReferentialIntegrity()).to.be.empty;
                // Verify that other package group hasn't been affected by the deletion
                expect_1.expect((await getPackageGroup(otherPackageGroupUid)).state).to.equal('published');
                expect_1.expect(await getPackageGroupChildRowCounts(otherPackageGroup.id), 'post-delete row counts of other').to.deep.equal(originalRowCountsOfOther);
            });
        });
        suite('importSuite', () => {
                ...
                if (!skipImport) {
                    ...
                    for (const packageGroup of getPackageGroupsToImport(incrPackageGroupUids)) {
                        ...
                        for (const id of packageGroupInternalIds) {
                            await rexdb.manage.publishPackageGroup(types_1.publicVIdToString(id));
                        }
                    }
                }
            });
    ...
    suite('Benchmarks', async () => {
        suite('import', async () => {
            ...
            before(async () => {
                ...
                await rexdb.manage.publishFilterDevSet(filterDevSetId);
                const time = util_2.hrtimeToSec(process.hrtime(start));
            ...
            for (const [name, description, additionalFilters, timeLimit] of testData) {
                test(name, description, async () => {
                    ...
                    for (let i = 0; i < iterations; i++) {
                        ...
                        const time = util_2.hrtimeToMillisec(process.hrtime(start));
                        expect_1.expect(tableViewItems).to.not.be.empty;
                        if (tableViewSize === undefined) {
                            tableViewSize = tableViewItems.length;
                        }
                        else {
                            expect_1.expect(tableViewSize).to.equal(tableViewItems.length, 'table view length should be constant');
                        }
                        const failOnSlowBenchmarks = false;
                        if (failOnSlowBenchmarks) {
                            expect_1.expect(time, 'expected to be faster (ms)').to.be.lessThan(timeLimit ? timeLimit : 100);
                        }
                        // Stats
                        total += time;
    ...
    async function benchmarkResourceTreeTraversal(name, testData) {
        ...
        expect_1.expect(testFilterPackageGroups, 'at least one package group expected').to.not.be.empty;
        ...
        expect_1.expect(groupWithMainPackageFound, 'no groups have main packages').to.be.true;
        ...
        async function doRandomTraversal(testDataSet, i) {
            ...
            if (searchSubstring) {
                ...
                // or partial db).
                expect_1.expect(searchTokens, 'search tokens').is.not.empty;
        ...
        async function getNodeChildren(nodeId) {
            ...
            const getChildrenCallTime = util_2.hrtimeToSec(process.hrtime(start)) * 1000;
            // Track call info
            pathNodes.push({
                id: nodeId,
        ...
        _.each(stats, (queryStats, queryName) => {
            ...
            _.each(queryStats, (paramStats, testParams) => {
                ...
                paramStats.forEach(stat => {
                    totalNsTime += util_2.hrtimeToNanosec(stat.time);
                    csvStringifier.write([
                        stat.seq,
                        queryName,
                        testParams,
                        util_2.hrtimeToSec(stat.time),
                        stat.rowCount,
                ...
                if (callCount % 2 === 0) {
                    medianNsTime =
                        (util_2.hrtimeToNanosec(paramStats[paramStats.length / 2].time) +
                            util_2.hrtimeToNanosec(paramStats[paramStats.length / 2 - 1].time)) /
                            2;
                }
                else {
                    medianNsTime = util_2.hrtimeToNanosec(paramStats[Math.floor(paramStats.length / 2)].time);
                }
                rexdb.console.fine('    Summary:\n' +
                    '      min:    ' +
                    util_2.hrtimeToSec(paramStats[0].time) +
                    '\n' +
                    '      median: ' +
                    util_2.nstimeToSec(medianNsTime) +
                    '\n' +
                    '      mean:   ' +
                    util_2.nstimeToSec(meanNsTime) +
                    '\n' +
                    '      max:    ' +
                    util_2.hrtimeToSec(paramStats[paramStats.length - 1].time) +
                    '\n');
            });
        });
        csvStringifier.end();
...
async function testResourceTreeTraversal() {
    ...
    */
    ...
    expect_1.expect(childNodeAttributes.packageIds, 'packages ids').to.not.be.empty;
    expect_1.expect(Array.from(childNodeAttributes.linkExts)).to.include.members([undefined, '.html']);
    // TODO: Select better package group set so we can test for 'e' too
    expect_1.expect(Array.from(childNodeAttributes.linkTypes), 'link types').to.include.members([
        undefined,
        'local'
    ]);
    expect_1.expect(Array.from(childNodeAttributes.icons), 'icons').to.include.members([
        undefined,
    ...
    expect_1.expect(Array.from(childNodeAttributes.resourceTypes), 'resource types').to.include.members([
        undefined,
        'packageOverview',
        'overview',
        'file'
    ...
    expect_1.expect(Array.from(childNodeAttributes.fileTypes), 'file types').to.include.members([]);
    expect_1.expect(Array.from(childNodeAttributes.leafTypes), 'leaf types').to.include.members([
        true,
        false
    ]);
    async function doRandomTraversal(testDataSet, i) {
        ...
        if (testDataSet.filters) {
            ...
            switch (filter.type) {
                case 'devtool':
                    ...
                    expect_1.expect(devtool, 'devtool ' + filter.name).to.exist;
                    filters.devtoolId = devtool.id;
                    break;
                case 'device':
                    const device = allDevices.find(device => device.name === filter.name && device.id !== null);
                    expect_1.expect(device, 'device ' + filter.name).to.exist;
                    filters.deviceId = device.id;
                    break;
                case 'compiler':
                    filters.compiler = filter.name;
...
async function testPackageTreeTraversal() {
    ...
    function traverseNodePackageTree(node, level = 0) {
        ...
        expect_1.expect(!node.packageGroupId || node.resourceType === 'packageOverview', 'a foundation or package overview node was expected').to.be.true;
...
async function lookupNodeOnPublicId() {
    ...
    for (const nodeLookup of nodeLookups) {
        ...
        expect_1.expect(nodes, nodeLookup.descr + ': # of presentation nodes').to.have.length(1);
        const node = nodes[0];
        expect_1.expect(node.name, nodeLookup.descr + ": node's name").to.equal(nodeLookup.expectedNodeName);
...
async function testPackageRetrieval(criteria) {
    ...
    if (criteria == null) {
        ...
        expect_1.expect(c2000WarePackage).to.exist.and.to.deep.include({
        ...
        expect_1.expect(msp432Package).to.exist.and.to.deep.include({
            moduleOf: { packageId: 'aPackage', versionRange: '12.34.56.78' }
        });
        expect_1.expect(msp432Package.dependencies).to.include.deep.members([
            {
        ...
        expect_1.expect(msp432Package.modules).to.include.deep.members([
            {
                refId: 'com.ti.SIMPLELINK_MSP432_SDK',
                versionRange: '1.50.00.12',
                require: undefined,
...
function expectValidDbId(id, message) {
    expect_1.expect(id, message).to.exist;
    expect_1.expect(id, message).to.be.greaterThan(-1);
}
...
function expectValidPackageGroup(packageGroup, allPackages) {
    ...
    expect_1.expect(packageGroup.headNodeIds).to.not.be.empty;
    expect_1.expect(packageGroup.packageIds).to.not.be.empty;
...
function expectValidPackageDbId(id, allPackages) {
    expectValidDbId(id);
    expect_1.expect(allPackages.find(entry => id === entry.id), `package with id ${id} not found`).to.exist;
}
function verifyRunTime(start, maxAllowableDuration) {
    expect_1.expect(Date.now() - start, 'maximum allowable run time exceeded').to.be.at.most(maxAllowableDuration);
}
...
async function cleanDb() {
    ...
    try {
        ...
        const query = promisifyAny_1.promisifyAny(conn.query).bind(conn);
...
async function getPackageGroup(packageGroupUid) {
    ...
    expect_1.expect(packageGroups, `package groups with uid ${packageGroupUid}`).to.have.length(1);
