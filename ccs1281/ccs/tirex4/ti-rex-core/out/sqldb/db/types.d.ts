import { LinkForDownload, LinkType, ResourceType, Dependencies, Compiler, Kernel, ViewLimitation, PlatformAttribute, ModuleOf, SortType, NumericPlatformAttribute, ModuleGroup, PackageSubType, FeatureType } from '../../lib/dbBuilder/dbTypes';
...
export declare type DbValue = string | number | boolean | null | undefined;
export declare type ContentSourceType = 'pkggrp' | 'devset';
...
export declare type ContentSourceState = 'incomplete' | 'imported' | 'published' | 'unpublished' | 'deleting' | 'deleted';
export declare type DbPackageGroup = Omit<DbContentSource, 'priority'>;
export declare type DbFilterDevSet = Omit<DbContentSource, 'mainPackageId' | 'filterDevSetId' | 'packagesToListVersionsFrom'>;
export declare type PartialDbContentSource = Partial<DbContentSource> & {
    publicVId: string;
    publicId: string;
    version: string;
};
export declare type PartialDbPackageGroup = Omit<PartialDbContentSource, 'priority'>;
export declare type PartialDbFilterDevSet = Omit<PartialDbContentSource, 'mainPackageId' | 'filterDevSetId'>;
...
export declare type DbDependencies = Dependencies & {
    type: DependType;
};
...
export declare type PartialDbPackage = Partial<DbPackage> & {
    uid: string;
};
...
export declare type RequireType = 'optional' | 'mandatory' | 'implicit';
export declare type DependType = 'd' | 'm' | 'g';
...
export declare type NodeRowWithPackageInfo = NodeRow & {
...
export declare type MinimalNodeRow = Pick<NodeRow, 'public_id_w0' | 'public_id_w1'>;
...
export declare type PartialDbNode = Partial<DbNode>;
...
export declare type PartialDbResource = Partial<DbResource> & {
    jsonId: string;
};
...
export declare type FilterType = 'compiler' | 'descr' | 'device' | 'devtool' | 'freq-chunk' | 'ide' | 'kernel' | 'language' | 'node' | 'resClass' | 'tag';
...
export declare type PartialDbFilter = Partial<DbFilter> & {
    type: FilterType;
    name: string;
};
...
export declare type PartialDbFilterChunk = Partial<DbFilterChunk> & {
    chunk: string;
};
...
export declare type PartialDbDevtool = Partial<DbDevtool> & {
    name: string;
};
...
export declare type PartialDbDevice = Partial<DbDevice> & {
    publicId: string;
};
export declare function rowToDevice(row: DeviceRow): DbDevice;
export declare type DbResourceGroup = DbObj & Pick<DbResource, 'type' | 'name' | 'shortDescription' | 'fileType' | 'packageId' | 'link' | 'linkType' | 'icon' | 'viewLimitations'> & {
...
export declare type InternalId = VersionedPublicId & {
    created: number;
};
