...
class TableViews {
    ...
    async getNode(tableViewItemId, dev, variant) {
        ...
        const compilerQArg = types_1.compilerToDb(variant.compiler);
        const kernelQArg = types_1.kernelToDb(variant.kernel);
        ...
        return _.isEmpty(rows) ? undefined : tree_1.rowToPresentationNode(rows[0]);
    }
    /**
    ...
    async getTableViewItemsOnPackageNode(nodeId, filters, packageGroupId) {
        ...
        return _.map(rows, row => ({
            ...
            viewLimitations: (types_1.recomposeStringArray(row.view_limitations) || undefined),
            ...
            variants: _.map(_.split(row.variants, ','), s => ({
                ...
                compiler: types_1.dbToCompiler(s[0]),
                kernel: types_1.dbToKernel(s[1])
            })),
    ...
    async getAvailableTableViewFiltersOnPackageNode(nodeId, filters, packageGroupId) {
        ...
        return _.isEmpty(rows)
            ...
            : {
                kernels: _.map(_.split(rows[0].kernels, ','), k => types_1.dbToKernel(k)),
                compilers: _.map(_.split(rows[0].compilers, ','), c => types_1.dbToCompiler(c))
            };
    }
}
