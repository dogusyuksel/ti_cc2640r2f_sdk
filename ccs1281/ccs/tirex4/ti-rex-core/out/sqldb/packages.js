    ...
    async getOverviews(packageCriteria, groupStates = ['published']) {
        ...
        for (const row of rows) {
            if (!pkg || pkg.id !== row.p.id) {
                ...
                const dbPackage = types_1.rowToPackage(row.p);
                pkg = {
                    ...dbPackage,
            ...
            if (row.d.id) {
                const dbPackageDepend = types_1.rowToPackageDepend(row.d);
                const packageDepend = {
                ...
                switch (dbPackageDepend.type) {
                    ...
                    case 'd':
                        ...
                        break;
                    default:
                        util_1.assertNever(dbPackageDepend.type);
    ...
    async getPackageGroups(criteria, latest = true, includeNodeHeads = true) {
            ...
            (_.isEmpty(fullCriteria.packageGroupUids)
                ...
                : 'and ( ' +
                    _.map(fullCriteria.packageGroupUids, uid => {
                        const { publicId, version } = types_1.stringToVPublicId(uid);
                        return ('(' +
        ...
        for (const row of rows) {
            if (!packageGroup || packageGroup.id !== row.id) {
                ...
                packageGroup = {
                    ...dbPackageGroupToPackageGroup(types_1.rowToPackageGroup(row)),
