...
class Tree {
    constructor(db, dinfraLibPath) {
        ...
        this.treeTopNodes = {}; // foundation and content nodes
        this.rootNode = null;
        this.queryMetrics = {};
    ...
    async lookupNodeOnPublicIdInternal(nodePublicId, packageGroup, packagePublicId, widenToAllGroups = false // widen search to all groups, for legacy URLs only
        ...
        // numeric string form.
        const { publicIdWord0, publicIdWord1 } = types_js_1.nodePublicIdToDbWords(nodePublicId);
        if (!packageGroup) {
    ...
    async lookupNodesOnResourcePublicId(resourcePublicId, pkg, filter, maxNodes) {
        ...
        if (filter) {
            if (filter.type === 'device') {
                ...
                util_1.assertNever(filter.type);
            }
        }
        ...
        return _.isEmpty(rows)
            ...
            : _.map(rows, row => ({
                nodeDbId: row.id,
                hashedNodePublicId: types_js_1.mysqlWordsToPublicId(row.public_id_w0, row.public_id_w1),
                packageGroup: {
            ...
            if (forDescendantCount) {
                ...
                switch (filteredEntity) {
                    ...
                    case 'resource':
                        ...
                        break;
                    default:
                        util_1.assertNever(filteredEntity);
...
exports.Tree = Tree;
Tree.LATEST_VERSION = 'LATEST';
function rowToPresentationNode(row) {
    return {
        id: row.id,
        publicId: types_js_1.nodeRowToPublicId(row),
        name: row.name,
        ...
        viewLimitations: (types_js_1.recomposeStringArray(row.view_limitations) || undefined),
        readmeNodePublicId: row.readme_public_id_w0 == null || row.readme_public_id_w1 == null
            ? undefined
            : types_js_1.mysqlWordsToPublicId(row.readme_public_id_w0, row.readme_public_id_w1)
    };
}
