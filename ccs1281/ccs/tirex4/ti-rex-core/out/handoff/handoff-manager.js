...
class HandoffManager {
    constructor({ defaultLog, loggerManager, vars }) {
        this.handoffManagerEventEmitter = new HandoffManagerEventEmitter();
        this.updateQueue = new PQueue({ concurrency: 1 });
        this.handoffManagerState = "up" /* UP */;
    ...
    isAcceptingSubmissions() {
        return this.handoffManagerState === "up" /* UP */;
    }
    ...
    async maintenanceModeEnable() {
        if (this.handoffManagerState !== "up" /* UP */) {
            throw new Error(`Already in ${this.handoffManagerState}`);
        }
        this.handoffManagerState = "teardown" /* TEARDOWN */;
        this.handoffManagerEventEmitter.emit("stateChange" /* STATE_CHANGE */);
        await this.updateQueue.add(async () => {
            this.handoffManagerState = "maintanceMode" /* MAINTANCE_MODE */;
            this.handoffManagerEventEmitter.emit("stateChange" /* STATE_CHANGE */);
        });
    }
    /**
    ...
    async maintenanceModeDisable() {
        if (this.handoffManagerState === "up" /* UP */) {
        ...
        else if (this.handoffManagerState === "maintanceMode" /* MAINTANCE_MODE */) {
            this.handoffManagerState = "up" /* UP */;
            this.handoffManagerEventEmitter.emit("stateChange" /* STATE_CHANGE */);
        }
        else {
            return new Promise((resolve, reject) => {
                asyncLib.doUntil(callback => {
                    this.handoffManagerEventEmitter.once("stateChange" /* STATE_CHANGE */, callback);
                }, () => {
                    return this.handoffManagerState !== "teardown" /* TEARDOWN */;
                }, err => {
                    if (err) {
                        return reject(err);
                    }
    ...
    async cleanupStagedPackagesInternal() {
        ...
        const rollbackEntries = await promise_utils_1.mapSerially(initialStagedEntires, async (stagedEntry) => {
        ...
        // Rollback
        await promise_utils_1.mapSerially(rollbackEntries, async ({ stagedEntry: entry, rollbackEntry }) => {
