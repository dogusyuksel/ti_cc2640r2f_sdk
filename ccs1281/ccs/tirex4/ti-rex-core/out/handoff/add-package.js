...
class AddPackage {
    ...
    async preAddPackage(params) {
        ...
        try {
            // Setup the logs
            ({ log, refreshLog, refreshOut, onSubmissionDone } = await handoff_logs_1.manageLogsSubmission({
            ...
            submittedItems = await promise_utils_1.mapSerially(entries, async (entry) => {
        ...
        try {
            if (onSubmissionDone && err) {
                ...
                await onSubmissionDone({
                    ...
                    handoffChecklistValid: !!submittedItems && submittedItems.every(item => item.handoffChecklistValid)
                });
            }
        }
        catch (e) {
    ...
    async addPackage(params) {
        ...
        try {
            if (outerEntries && log && !err) {
                ...
                const emailTag = outerEntries.map(item => `${item.id}__${item.version}`).join(', ');
                await db_release_and_import_1.dbImportAndPublish(email, 'add', this.vars.dbBasePath, log, emailTag);
            }
        }
        catch (e) {
            err = util.getCombinedError(err, e);
    ...
    async _rollbackPackages(params) {
        ...
        try {
            // Stage
            const stagedItems = await promise_utils_1.mapSerially(entries, async (entry) => {
            ...
            // Rollback
            await promise_utils_1.mapSerially(stagedItems, async ({ entry, stagedEntry }) => {
    ...
    async stagePackages({ submittedItems, log, submissionId }) {
        ...
        try {
            ...
            try {
                processedSubmittedItems = await process_modules_1.processModules(submittedItems, this.packageManagerAdapter, this.vars.contentBasePath, this.vars.zipsFolder, log);
            }
            catch (e) {
                // Cleanup submissions
            ...
            // Stage
            await promise_utils_1.mapSerially(processedSubmittedItems, async (item) => {
                ...
                await handoff_checklist_helpers_1.saveHandoffChecklists({
    ...
    async savePackages({ stagedItems, log }) {
        return promise_utils_1.mapSerially(stagedItems, entry => {
    ...
    async getSubmission(assetUploads, uploadedEntry, installLogFiles, log) {
        ...
        if (uploadedEntry.submissionType === "installer" /* INSTALLER */ &&
        ...
        // Get the submission
        const { downloadFolder, extractFolder, packageFolders, nonPackageFolders, zips, handoffChecklistValid } = uploadedEntry.submissionType === "installer" /* INSTALLER */
            ? await prepare_handoff_1.prepareHandoff({
                uploadedEntry,
        ...
        try {
            ...
            if (uploadedEntry.submissionType === "installer" /* INSTALLER */) {
                const info = await Promise.all(submission.packageFolders.map(async (folder) => {
                    const info = await package_helpers_1.getPackageInfo(folder);
                    if (!info) {
                ...
                const groupedPackageFolders = _.chain(info)
                    ...
                    .value();
                stagedEntries = await promise_utils_1.mapSerially([
