...
const getUniqueFolderPath = util_1.promisify(path_helpers_1.PathHelpers.getUniqueFolderPath);
const _sendEmail = util_1.promisify(helpers_node_1.sendEmail);
...
async function downloadFile(url, dst, onProgressUpdate) {
    ...
    const requestStream = request_1.getRequest().get(url);
        ...
        try {
            if (totalSize) {
                onProgressUpdate({
                    ...
                    progressType: "Definite" /* DEFINITE */
                });
            }
            else {
                onProgressUpdate({
    ...
    await Promise.all([
        promise_utils_1.promisePipe([requestStream, writeStream]),
...
async function install(installCommand, workingDirectory, installDir, onProgressUpdate, out) {
    ...
    onProgressUpdate({ name: 'Installing', progressType: "Indefinite" /* INDEFINITE */ });
...
async function getRefreshParams(items, allEntires, contentFolder, zipsFolder) {
    ...
    return Promise.all(allEntiresWithModules
        ...
        const refreshRequestEntry = {
            ...
            uid: dbBuilderUtils_1.formUid(entry.id, entry.version),
            operation: "excludePackage" /* EXCLUDE_PACKAGE */,
        ...
        switch (request) {
            case 'add':
                refreshRequestEntry.operation = "replacePackage" /* REPLACE_PACKAGE */;
                break;
            case 'remove':
                refreshRequestEntry.operation = "removePackage" /* REMOVE_PACKAGE */;
                break;
            case 'nothing':
                refreshRequestEntry.operation = "doNothing" /* DO_NOTHING */;
                break;
            default:
                util_2.assertNever(request);
...
async function getInstallCommand(entry, contentFolder) {
    ...
    const allPackageTirexJson = await package_helpers_1.getAllPackageTirexJsonPaths(packageFolder);
    const potentialInstallCommands = await Promise.all(allPackageTirexJson.map(async (item) => {
        const info = await package_helpers_1.getPackageInfo(packageFolder, item);
...
async function getEntryWithModules(entry, contentFolder) {
    ...
    const allPackageTirexJsonFiles = await package_helpers_1.getAllPackageTirexJsonPaths(path.join(contentFolder, packageFolder));
    ...
    const modulesInfo = await Promise.all(allPackageTirexJsonFiles
        ...
        .map(async (item) => ({ info: await package_helpers_1.getPackageInfo(packageFolder, item), path: item })));
