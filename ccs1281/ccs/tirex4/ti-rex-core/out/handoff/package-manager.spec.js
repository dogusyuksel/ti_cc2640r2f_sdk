...
const { log } = rex_1._getRex();
////////////////////////////////////////////////////////////////////////////////
/// Data - Do not change the values as the tests rely on them (i.e same id / version)
////////////////////////////////////////////////////////////////////////////////
const entry1 = test_helpers_2.createPackageEntryValid({
    id: 'hello',
    version: '6_5_0',
    content: [],
    zips: []
});
const entry2 = test_helpers_2.createPackageEntryValid({
    id: 'goodbye',
    version: '0.10.26',
    content: [],
    zips: []
});
const entry3 = test_helpers_2.createPackageEntryValid({
    id: 'goodbye',
    version: '0.10.27',
    content: [],
    zips: ['something/hi']
});
const entry4 = test_helpers_2.createPackageEntryValid({
    id: 'goodbye',
    version: '0.10.27',
    content: ['hello/goodbye'],
    zips: []
});
const entry5 = test_helpers_2.createPackageEntryValid({
    id: 'goodbye',
    version: '0.10.27',
    content: ['hello/howdy', 'foo/bar'],
    zips: ['I/am/a/foo.tar.gz', 'I/am/bar.tar.gz']
});
const entry7 = test_helpers_2.createPackageEntryValid({
    id: 'goodbye',
    version: '0.10.26',
    content: ['baz/fuz'],
    zips: []
});
const entry8 = test_helpers_2.createPackageEntryValid({
    id: 'howdy',
    version: '8',
    content: ['howdy_8_0_0', 'another'],
    zips: ['howdy_8_0_0__all.zip']
});
const entry9 = test_helpers_2.createPackageEntryValid({
    id: 'howdy',
    version: '8',
    content: ['later_8_0_0', 'something'],
    zips: ['later_8_0_0__all.zip']
});
const entry10 = test_helpers_2.createPackageEntryValid({
    id: 'howdy',
    version: '8',
...
describe('[handoff] PackageManager', function () {
    describe('getPackageEntry', function () {
        it('Should get a valid entry from the file', async function () {
            const { pm } = await test_helpers_2.preparePackageManager({
            ...
            chai_1.expect(idx).to.not.equal(-1);
            chai_1.expect(entry).to.contain.all.keys(entry1);
        });
        it('Should handle a request for a package that does not exist', async function () {
            ...
            chai_1.expect(idx).to.equal(-1);
            chai_1.expect(entry).to.not.exist;
        });
    ...
    describe('stageAddPackage', () => {
        it('Should handle new packages', async function () {
            const { pm, contentFolder, zipsFolder } = await test_helpers_2.preparePackageManager({
                packageManagerFileJson: {
                    packages: [entry1]
                }
            });
            // Stage the entry
            {
                ...
                chai_1.expect(oldEntry).to.not.exist;
        ...
        it('Should not allow staging a staged package', async function () {
            const { pm, contentFolder, zipsFolder } = await test_helpers_2.preparePackageManager({
                contentFolder: test_helpers_1.getUniqueFolderName(),
                packageManagerFileJson: {
                    packages: [entry1]
                }
            });
            ...
            // Stage again
            await util_2.verifyError(pm._stagePackagesFileAndBackupAssets({
                entry: entry7,
                log
            }), 'already staged');
        });
        it('Should backup a valid entry', async function () {
            ...
            // Stage the entry
            {
                ...
                chai_1.expect(oldEntry).to.exist;
                await test_helpers_2.prepareContentAndZips({
                    contentFolder,
                    zipsFolder,
                    zips: entry9.zips,
        ...
        async function verifyEntry({ insertedEntry, resultEntry, contentFolder, zipsFolder, isUpdate = false, prevContent, prevZips }) {
            ...
            if (isUpdate) {
                chai_1.expect(resultEntry).to.deep.equal({
                    ...resultEntry,
                    id,
                    version,
                    content,
                ...
                if (resultEntry.state === "staged" /* STAGED */) {
                    ...
                    chai_1.expect(resultEntry.backupContent).to.have.length(prevContent.length);
                    chai_1.expect(resultEntry.backupZips).to.have.length(prevZips.length);
                ...
                await Promise.all([
                    test_helpers_2.verifyContentAndZipsGone({
                        content: prevContent
                    ...
                    test_helpers_2.verifyContentAndZipsPresent({
    ...
    describe('rollbackPackage', function () {
        ...
        it('Should handle trying to rollback a non-staged package', async function () {
            ...
            await util_2.verifyError(pm.stageRollbackPackage({
                id: entry2.id,
                version: entry2.version,
        ...
        it('Should handle an entry with no backup (delete)', async function () {
            ...
            chai_1.expect(rollbackEntry).to.be.null;
        });
        ...
        function verifyRollbackSuccessful({ entry, previousEntry, stagedEntry, contentFolder, zipsFolder }) {
            util_1.getObjectKeys(entry).map(key => {
                chai_1.expect(previousEntry[key]).to.deep.equal(entry[key]);
            });
            return Promise.all([
                test_helpers_2.verifyContentAndZipsGone({
    ...
    describe('deletePackage', () => {
        ...
        it('Should delete a staged apackage', async function () {
            const entry = test_helpers_2.createPackageEntryStaged({
                id: entry1.id,
                version: entry1.version,
                content: entry1.content,
                zips: entry1.zips
        ...
        it('Should handle a non-existing package', async function () {
            ...
            await util_2.verifyError(body(), 'No entry');
        });
        async function verifyDeleteSuccessful({ entry, packageManager, contentFolder, zipsFolder }) {
            ...
            chai_1.expect(pmEntry).to.not.exist;
        }
        async function verifyDeleteUnSuccessful({ entry, packageManager, contentFolder, zipsFolder }) {
            ...
            chai_1.expect(pmEntry).to.exist;
        }
    });
});
