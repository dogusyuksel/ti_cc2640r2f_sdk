...
async function prepareHandoff(args) {
    ...
    try {
        ...
        const handoffChecklistValid = handoff_checklist_helpers_1.validateHandoffChecklist(uploadedEntry.handoffChecklist, log);
        const { allAssets, topLevelItems } = await extract_submitted_assets_1.extractSubmittedAssets(assetUploads, uploadedEntry, downloadFolder, extractFolder, args.submissionType === "installer" /* INSTALLER */ ? args.installOut : null, log);
        ...
        await validatePackageFolders(packageFoldersInitial, extractFolder, args.submissionType === "installer" /* INSTALLER */, log);
        // Filter nonPackageFolders if they exist on the fs as a packageFolder
        const nonPackageFolders = args.submissionType === "installer" /* INSTALLER */
        ...
        const { id, version } = args.uploadedEntry.submissionType === "installer" /* INSTALLER */
            ? {
                id: args.uploadedEntry.submittedPackage.id,
                version: args.uploadedEntry.submittedPackage.version
            }
            : { id: null, version: null };
        const packageFolders = args.submissionType === "installer" /* INSTALLER */
        ...
        const finalPackageFolders = await Promise.all(packageFolders.map(async ({ item: packageFolder, isSubmittedPackage }) => {
            const transformedFolder = await PackageHelpers.moveDevPackageToSubfolder({
                packageFolder,
                extractFolder
            });
        ...
        if (args.submissionType === "installer" /* INSTALLER */ &&
            !args.skipVerifyInstallerInSync &&
            args.uploadedEntry.submissionType === "installer" /* INSTALLER */) {
