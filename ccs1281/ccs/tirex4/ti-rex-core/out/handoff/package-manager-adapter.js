...
class PackageManagerAdapter {
    ...
    async scanPackages(log) {
        ...
        const newEntires = await create_package_entries_1.createPackageEntries({
            entries: allEntries,
            packageFolders,
            nonPackageFolders,
        ...
        const deleteEntries = (await Promise.all(allEntries.map(async (entry) => {
            ...
            return {
                keepEntry: pathExists || entry.state === "staged" /* STAGED */,
                entry
            };
        })))
    ...
    async getStagedEntries(log) {
        ...
        return entries.filter((entry) => entry.state === "staged" /* STAGED */);
    }
    getAllEntries(log) {
