...
class RemovePackage {
    ...
    async removePackage(params) {
        ...
        try {
            const result = await handoff_logs_1.manageLogsDeletion({
        ...
        try {
            if (!err && log) {
                ...
                await db_release_and_import_1.dbImportAndPublish(params.email, 'remove', this.vars.dbBasePath, log, `${params.packageInfo.id}__${params.packageInfo.version}`);
            }
        }
        catch (e) {
            err = util.getCombinedError(err, e);
