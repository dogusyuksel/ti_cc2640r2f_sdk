...
describe('[handoff] process Modules', function () {
    ...
    it('Should handle a new core package with modules', async function () {
        ...
        const processedSubmittedItems = await process_modules_1.processModules(submittedItems, packageManagerAdapter, contentFolder, zipsFolder, log);
        ...
        chai_1.expect(processedSubmittedItems.map(item => ({ ...item, zips: item.zips.sort() }))).to.deep.equal(expectedSubmittedItems);
        const innerCorePackageContent = await fs.readdir(path.join(extractFolder, corePackage.packageFolderName));
        innerContetnt.map(item => chai_1.expect(innerCorePackageContent).contains(path.basename(item)));
    });
    it('Should handle a submission with both an independent package & a core package with modules', async function () {
        ...
        chai_1.expect(processedSubmittedItems).to.deep.equal(expectedSubmittedItems);
    ...
    it('Should not allow submitting a module whose core package is not available', async function () {
        ...
        const { packageManagerAdapter, contentFolder, zipsFolder } = await preparePackageManagerAdapter();
        const promise = process_modules_1.processModules(submittedItems, packageManagerAdapter, contentFolder, zipsFolder, log);
        await chai_1.expect(promise).to.eventually.be.rejectedWith(errors_1.GracefulError);
        await chai_1.expect(promise).to.eventually.be.rejectedWith(new RegExp('corePackage for.*does not exist on the server.*'));
    });
    ...
    async function generateSubmittedItems(data) {
        return Promise.all(data.map(async ({ packageFolderName, packageInfo, zipFileName, uploadedEntry, modulePrefix }) => {
            const downloadFolder = test_helpers_1.getUniqueFolderName();
            const extractFolder = test_helpers_1.getUniqueFolderName();
    ...
    async function preparePackageManagerAdapter() {
        const packageManagerFile = path.join(scriptsUtil.generatedDataFolder, test_helpers_1.getUniqueFileName());
