...
describe('[handoff] prepareHandoff - zips', function () {
    ...
    it('Should handle a mix of platform dependent and all zips (fail)', async function () {
        await util_2.verifyError(prepareHandoffWrapper({
    ...
    async function prepareHandoffWrapper({ assetLinks, assetUploads }) {
        const contentFolder = test_helpers_1.getUniqueFolderName();
        const localAssets = {};
        Object.entries(assetUploads).map(([platform, upload]) => {
            localAssets[platform] = path.join('some', 'random', 'path', upload.originalname);
        });
        const { downloadFolder, extractFolder } = await prepare_handoff_1.prepareHandoff({
            contentFolder,
...
describe('[handoff] prepareHandoff - installers', function () {
    ...
    it('Should handle no linux install command (error)', async function () {
        ...
        await chai_1.expect(promise).to.eventually.be.rejectedWith('Linux install command missing');
    });
    it('Should handle an installer with an invalid install command', async function () {
        ...
        await chai_1.expect(promise).to.eventually.be.rejectedWith(new RegExp('Missing @install-location.*'));
    });
    it('Should handle an installer command failing', async function () {
        ...
        await chai_1.expect(promise).to.eventually.be.rejectedWith(new RegExp('Install exited with code.*'));
    });
    ...
    it('Should handle submitted package not in installer', async function () {
        ...
        await chai_1.expect(promise).to.eventually.be.rejectedWith(errors_1.GracefulError);
        await chai_1.expect(promise).to.eventually.be.rejectedWith(new RegExp('Submitted package not in installer.*'));
    });
    ...
    it('Should handle an installer with the install command not in sync between package.tirex.json and handoff.json (fail)', async function () {
        ...
        await chai_1.expect(promise).to.eventually.be.rejectedWith('Install command not in sync');
    });
    ...
    it('Should handle the submitted package existing in the content folder but not in the package manager file', async function () {
        const submittedPackage = { id: 'com.ti.CORE_MSP432_SDK', version: '3.01.00.11_eng' };
        await test_helpers_2.createPackages({
    ...
    it('Should handle a secondary non tirex package existing in the content folder but not in the package manager file', async function () {
        ...
        await test_helpers_2.createPackages({ packages: [packageContent] });
        // empty dir to remove metadata (so it becomes a non-tirex package);
        await fs.emptyDir(packageContent);
        const result = prepareHandoffWrapper({
            contentFolder,
    ...
    async function setup() {
        const contentFolder = test_helpers_1.getUniqueFolderName();
        const zipsFolder = test_helpers_1.getUniqueFolderName();
        const overridesDir = test_helpers_1.getUniqueFolderName();
        const packageManagerFile = path.join(test_helpers_1.getUniqueFolderName(), 'package-manager.json');
    ...
    async function prepareHandoffWrapper({ contentFolder, packageManagerAdapter, installer, installCommand, submittedPackage, checkInstallerInSync = false }) {
        ...
        const uploadedEntry = {
            submissionType: "installer" /* INSTALLER */,
            installCommand,
            submittedPackage,
            email: '',
            assets: { [util_1.Platform.LINUX]: installer }
        };
        const result = prepare_handoff_1.prepareHandoff({
            assetUploads: [],
            contentFolder,
            log,
            uploadedEntry,
    ...
    async function getInstallOut() {
        const file = path.join(test_helpers_1.getUniqueFolderName(), 'install-output.text');
    ...
    async function verifyPrepareHandoff(result, { expectedNonPackageFolders, expectedPackageFolders, expectedZips }) {
        ...
        chai_1.expect(normalizeAndSort(relPackageFolders)).to.deep.equal(normalizeAndSort(expectedPackageFolders));
        chai_1.expect(normalizeAndSort(relNonPackageFolders)).to.deep.equal(normalizeAndSort(expectedNonPackageFolders));
        chai_1.expect(normalizeAndSort(relZips)).to.deep.equal(normalizeAndSort(expectedZips));
        await test_helpers_2.verifyContentAndZipsPresent({
            contentFolder: extractFolder,
