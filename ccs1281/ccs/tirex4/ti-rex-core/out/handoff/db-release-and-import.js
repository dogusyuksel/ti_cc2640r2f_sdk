...
async function dbImportAndPublish(email, refreshRequest, dbBasePath, log, emailTag) {
    ...
    try {
        ...
        err = util_1.getCombinedError(err, e);
    }
    finally {
        // Send out email
        ...
        await util_1.sendEmail(email, subject, (err ? '' : (refreshRequest === 'remove' ? removeMessage : addMessage) + '<br><br>') +
...
function manageLogs(log) {
    ...
    log.userLogger.on('data', message => {
        ...
        messageBody += util_1.transformLogMessage(message);
    });
    return () => {
        return messageBody;
    };
...
async function spawnCmd(args, cmdTag, log, onMessage) {
    ...
    const cp = child_process_1.spawn(command, _args);
    // Setup log file
    const { logOut, logFile } = await util_1.prepareLogFile(vars_1.Vars.CONTENT_BASE_PATH, `${cmdTag}.log`);
...
async function getDbImportArgs() {
    const { dinfra, dconfig, appConfig } = await getAppConfig_1.getAppConfig();
