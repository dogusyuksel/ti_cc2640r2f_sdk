...
class CCSAdapter {
    constructor(logger, desktopQueue, ccsPort, isTheia, vars, offlineMetadataManager, progressManager, triggerEvent) {
        ...
        this.syncCounter = new counter_1.Counter();
    ...
    onProductsChanged() {
        ...
        this.runNonUserInitiatedTask(async () => {
            this.triggerEvent("OnInstallInfoUpdated" /* ON_INSTALL_INFO_UPDATED */, await this.getSearchPaths());
        }, 'Updating search paths');
        // Send updating list of packages
        this.runNonUserInitiatedTask(async () => {
            this.triggerEvent("OnInstalledPackagesUpdated" /* ON_INSTALLED_PACKAGES_UPDATED */, await this.getInstalledPackages());
        }, 'Updating list of installed pacakges');
        // Do a sync
        this.doSync();
    }
    /**
    ...
    async importProject(resourceType, packageUid, location, targetId, projectName) {
        ...
        switch (resourceType) {
            ...
            case "project.energia" /* ENERGIA */:
                ...
                break;
            default:
                util_2.assertNever(resourceType);
    ...
    async addNewPackages(onProgressUpdate) {
        ...
        const newIdePackages = _.differenceWith(idePackagesPackageInfo, rexPackages, (idePackage, rexPackage) => {
            ...
            else if (!versioning_1.convertToSemver(rexPackage.packageVersion) ||
                !versioning_1.convertToSemver(idePackage.packageVersion)) {
            ...
            return versioning_1.compare(rexPackage.packageVersion, idePackage.packageVersion) === 0;
        });
        for (const packageInfo of newIdePackages) {
            await this.offlineMetadataManager.offlinePackageMetadata(dbBuilderUtils_1.formUid(packageInfo.packagePublicId, packageInfo.packageVersion), packageInfo.localPackagePath, onProgressUpdate, true);
        }
    }
    /**
    ...
    async getPackageInfoForProducts(idePackages) {
        ...
        for (const newIdePackage of idePackagesWithFixedXdctools) {
            ...
            const allPackageTirexJson = await package_helpers_1.getAllPackageTirexJsonPaths(newIdePackage.location);
            const packageInfos = await Promise.all(allPackageTirexJson.map(item => package_helpers_1.getPackageInfo(newIdePackage.location, item)));
            for (const packageInfo of packageInfos) {
                if (packageInfo) {
                    idePackagesPackageInfo.push({
                        ...packageInfo,
                        location: newIdePackage.location,
                        uid: dbBuilderUtils_1.formUid(packageInfo.id, packageInfo.version)
                    });
                }
                else {
    ...
    async getCCSBaseItems() {
        ...
        const paths = await package_helpers_1.getAllPackageTirexJsonPaths(ccsBase);
        const ccsBaseItems = await Promise.all(paths.map(async (pkgPath) => {
            const pkgInfo = await package_helpers_1.getPackageInfo(path.dirname(pkgPath), pkgPath);
            if (!pkgInfo) {
                throw new Error(`Cannot get packageInfo for ${pkgPath}`);
            }
            return { ...pkgInfo, location: ccsBase, uid: dbBuilderUtils_1.formUid(pkgInfo.id, pkgInfo.version) };
        }));
        return ccsBaseItems;
    }
}
...
function pathsEqual(a, b) {
    if (util_1.getPlatform() === response_data_1.Platform.WINDOWS) {
