...
/// Data
///////////////////////////////////////////////////////////////////////////////
const devices = [database_utils_1.generateDevice('DEVICE1', 'device1')];
const devtools = [database_utils_1.generateDevtool('BOARD1', 'board1')];
// overviews
const overview1 = database_utils_1.generatePackageOverview('package1', '1.0.0.0', {
    localPackagePath: test_helpers_1.getUniqueFolderName()
});
// projects
const project1 = database_utils_1.generateResource('test.projectspec', overview1, devices[0], devtools[0], {
...
const project2 = database_utils_1.generateResource('test.projectspec', overview1, devices[0], devtools[0], {
...
const project3 = database_utils_1.generateResource('test.projectspec', overview1, devices[0], devtools[0], {
...
const project4 = database_utils_1.generateResource('test.projectspec', overview1, devices[0], devtools[0], {
...
const project5 = database_utils_1.generateResource('test.projectspec', overview1, devices[0], devtools[0], {
...
describe('[cloudagent] CCSAdapter', function () {
    describe('initial discovery', function () {
        it('should query for packages discovered with IDE at startup', async function () {
            ...
            await testing_util_1.createCCSAdapter({ onIDERequest });
            await testing_util_1.waitForCallWithArgs(onIDERequest, "/ide/getProducts" /* GET_PRODUCTS */);
        });
        it('should request that deleted packages have their metadata deleted', async function () {
            const { offlineMetadataManager, progressManager } = await testing_util_1.createCCSAdapter({
                metadata: metadata1
            });
            ...
            expect_1.expect(spy.args[0][0]).to.equal(overview1.packageUId);
        });
        it('should request that new discovered packages have their metadata downloaded', async function () {
            ...
            const location = test_helpers_1.getUniqueFolderName();
            ...
            const onIDERequest = sinon.stub().callsFake((pathname) => {
                if (pathname === "/ide/getProducts" /* GET_PRODUCTS */) {
                    return [{ location }];
                }
                return;
            });
            ...
            await progressManager.waitForTasks();
            expect_1.expect(spy.calledOnce).to.be.true;
            expect_1.expect(spy.args[0][0]).to.equal('id__version');
            expect_1.expect(spy.args[0][1]).to.equal(location);
            expect_1.expect(spy.args[0][3]).to.equal(true);
        });
        it('should not request that existing packages have their metadata downloaded', async function () {
            ...
            await progressManager.waitForTasks();
            expect_1.expect(offline.calledOnce).to.be.false;
            expect_1.expect(remove.calledOnce).to.be.false;
        });
        it('should create progress tasks for initial package discovery', async function () {
            // Setup
            const triggerEventSpy = sinon.stub();
            await testing_util_1.createCCSAdapter({ triggerEventSpy });
            // Wait for initial progress message
            await testing_util_1.waitForCallWithArgs(triggerEventSpy, "OnProgressUpdated" /* ON_PROGRESS_UPDATED */);
            const firstCall = triggerEventSpy.withArgs("OnProgressUpdated" /* ON_PROGRESS_UPDATED */).firstCall;
            const progress = firstCall.args[1];
            expect_1.expect(_.size(progress)).to.equal(1);
            expect_1.expect(Object.values(progress)).to.deep.equal([
                {
            ...
            do {
                await testing_util_1.waitForCallWithArgs(triggerEventSpy, "OnProgressUpdated" /* ON_PROGRESS_UPDATED */);
                const lastCall = triggerEventSpy.withArgs("OnProgressUpdated" /* ON_PROGRESS_UPDATED */)
                    .lastCall;
            ...
            expect_1.expect(_.size(lastProgressResult)).to.equal(0);
        });
        it('should handle errors for initial package discovery', async function () {
            ...
            await testing_util_1.createCCSAdapter({ triggerEventSpy, onIDERequest });
            ...
            do {
                ...
                const lastCall = triggerEventSpy.withArgs("OnProgressUpdated" /* ON_PROGRESS_UPDATED */)
                    .lastCall;
                lastProgressResult = lastCall.args[1];
                expect_1.expect(_.size(lastProgressResult)).to.equal(1);
                lastProgress = Object.values(lastProgressResult)[0];
                if (!lastProgress.isComplete) {
                    triggerEventSpy.resetHistory();
                }
            } while (!lastProgress.isComplete);
            expect_1.expect(lastProgress.error).to.contain('IDE messed up');
        });
    });
    describe('notifyIDEPackagesChanged', function () {
        it('should notify IDE', async function () {
            const onIDERequest = sinon.spy();
            const { ccsAdapter } = await testing_util_1.createCCSAdapter({ onIDERequest });
            await ccsAdapter.notifyIDEPackagesChanged();
            onIDERequest.calledWith("/ide/rediscoverProducts" /* REDISCOVER_PRODUCTS */);
        });
        it('should report IDE errors', async function () {
            const onIDERequest = sinon.stub().callsFake((pathname) => {
                if (pathname === "/ide/rediscoverProducts" /* REDISCOVER_PRODUCTS */) {
                    throw new Error('IDE messed up');
                }
            });
            const { ccsAdapter } = await testing_util_1.createCCSAdapter({ onIDERequest });
            await expect_1.expect(ccsAdapter.notifyIDEPackagesChanged()).to.eventually.be.rejectedWith('IDE messed up');
        });
    });
    describe('getSearchPaths', function () {
        ...
        it('should return paths from IDE', async function () {
            const onIDERequest = sinon.stub().callsFake((pathname) => {
                if (pathname === "/ide/syncProductDiscoveryPath" /* SYNC_SEARCH_PATH */) {
            ...
            expect_1.expect(returnedPaths).to.deep.equal(discoveryPaths.sort());
        });
        it('should exclude paths where there is a product installed', async function () {
            ...
            expect_1.expect(returnedPaths).to.deep.equal(searchPaths.sort());
        });
        ...
        for (const api of ["/ide/syncProductDiscoveryPath" /* SYNC_SEARCH_PATH */, "/ide/getProducts" /* GET_PRODUCTS */]) {
            it(`should report IDE errors on ${api}`, async function () {
                ...
                const { ccsAdapter } = await testing_util_1.createCCSAdapter({ onIDERequest });
                await expect_1.expect(ccsAdapter.getSearchPaths()).to.eventually.be.rejectedWith('IDE messed up');
            });
        }
        it('should move the default path to the top', async function () {
            ...
            const { ccsAdapter, commonParams } = await testing_util_1.createCCSAdapter({ onIDERequest });
            ...
            const returnedPaths = await ccsAdapter.getSearchPaths();
            expect_1.expect(returnedPaths).to.deep.equal(discoveryPathsWithNonTopDefault);
        });
    });
    describe('importProject', function () {
        it('should notify ide to do an import', async function () {
            ...
            const onIDERequest = sinon.spy();
            const { ccsAdapter } = await testing_util_1.createCCSAdapter({
                metadata,
                onIDERequest
            });
            // @ts-ignore
            ...
            expect_1.expect(onIDERequest.calledWith("/ide/importProject" /* IMPORT_PROJECT */, {
        ...
        it('should report ide errors', async function () {
            ...
            const project = project1;
            const onIDERequest = sinon.stub().callsFake((pathname) => {
                if (pathname === "/ide/importProject" /* IMPORT_PROJECT */) {
                    throw new Error('IDE messed up');
                }
            });
            const { ccsAdapter } = await testing_util_1.createCCSAdapter({
            ...
            await expect_1.expect(ccsAdapter.importProject(resourceType, packageUId, link, null, null)).to.eventually.be.rejectedWith('IDE messed up');
        });
        it('should report an error trying to import from a non-local package', async function () {
            ...
            const { ccsAdapter } = await testing_util_1.createCCSAdapter({});
            await expect_1.expect(ccsAdapter.importProject(resouceType, packagePublicUid, relativeLocation, null, null)).to.eventually.be.rejectedWith('Package not installed locally');
        });
        it('should notify IDE to do Import Energia Sketch', async function () {
            ...
            expect_1.expect(onIDERequest.calledWith("/ide/importSketch" /* IMPORT_SKETCH */, {
        ...
        it('should report an error trying to Import Energia Sketch without board-Id', async function () {
            ...
            await expect_1.expect(ccsAdapter.importProject(resourceType, packageUId, link, targetId, null)).to.eventually.be.rejectedWith('Missing Energia board ID');
        });
        it('should notify IDE to import a file', async function () {
            ...
            await ccsAdapter.importProject(resourceType, packageUId, link, targetId, null);
            expect_1.expect(onIDERequest.calledWith("/ide/createProject" /* CREATE_PROJECT */, {
        ...
        it('should report an error trying to import unsupported project type', async function () {
            ...
            await expect_1.expect(ccsAdapter.importProject('bundle', packageUId, link, targetId, null)).to.eventually.be.rejectedWith('Unsupported project type: bundle');
        });
    });
