...
describe('[cloudagent] ProgressManager', function () {
    describe('getProgress', function () {
        it('Should be able to get the set of ongoing tasks', async function () {
            const task = task1;
            const progressManager = await testing_util_1.createProgressManager({});
        ...
        it('Should be able to get a valid tasks progress', async function () {
            ...
            // Verify
            expect_1.expect(progress).to.deep.equal({
                ...progress,
    ...
    describe('clearTaskProgress', function () {
        it('Should fail to clear an incompleted task', async function () {
            ...
            const [progressId] = Object.keys(tasks);
            // Clear
            expect_1.expect(() => progressManager.clearTaskProgress(progressId)).to.throw('Trying to clear incomplete progress');
        });
        it('Should be able to clear a completed task', async function () {
            ...
            // Verify
            expect_1.expect(!!progressManager.getProgress()[progressId]).to.be.false;
        });
        it('Should handle an invalid progress id', async function () {
            ...
            const [progressId] = Object.keys(tasks);
            // Verify
            expect_1.expect(() => progressManager.clearTaskProgress(progressId + '1')).to.throw('non-existent progress');
        });
    });
    describe('Updating progress', function () {
        it('Should handle new tasks being added', async function () {
            ...
            const task = task1;
            // Setup
            const progressManager = await testing_util_1.createProgressManager({ triggerEventSpy });
            const { registerTask } = progressManager.createProgressTask(task.initialProgress, 'context');
            registerTask(task.task());
            // Get progressId
            const tasks = progressManager.getProgress();
            ...
            const [progressId] = Object.keys(tasks);
            // Verify
            testing_util_1.calledWithExactlyDeep(triggerEventSpy, [
                "OnProgressUpdated" /* ON_PROGRESS_UPDATED */,
        ...
        it('Should handle tasks being removed', async function () {
            ...
            testing_util_1.calledWithExactlyDeep(triggerEventSpy, ["OnProgressUpdated" /* ON_PROGRESS_UPDATED */, {}]);
        });
        ...
        it('Should handle a non-user task completing successfully', async function () {
            ...
            expect_1.expect(_.size(tasks2)).to.equal(0);
            // Verify
            const expectedUpdateData = {
                [progressId]: {
                    ...progress,
    ...
    describe('close', function () {
        it('Should abort tasks when closed', async () => {
            ...
            expect_1.expect(() => onProgressUpdate(initialProgress)).to.not.throw;
            progressManager.close();
            expect_1.expect(() => onProgressUpdate(initialProgress)).to.throw;
        });
    });
});
