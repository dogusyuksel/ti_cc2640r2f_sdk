...
async function getBoardAndDeviceInfo(commonParams, options) {
    ...
    if (offline) {
        ...
        try {
            ...
            boardAndDeviceData = await util_1.doTirex4Request(`${tirex4RemoteserverUrl}${"api/boardsDevices" /* GET_BOARDS_AND_DEVICES */}`);
        }
        catch (e) {
    ...
    async function getOfflineBoardAndDeviceInfo() {
        ...
        const configPath = util_1.getConfigFolder();
        const latestBoardsAndDevicesPath = path.join(configPath, entry_module_1.latestBoardsAndDevicesFileName);
        const defaultBoardsAndDevicesPath = path.join(configPath, entry_module_1.defaultBoardsAndDevicesFileName);
...
async function getInstallInfoForPackageDependencies(packageInfo, ccsAdapter, commonParams, excludePacakgeAsDependency = false) {
    const tirex4RemoteserverUrl = commonParams.vars.remoteserverUrl;
    const pkgs = await util_1.doTirex4Request(`${tirex4RemoteserverUrl}${"api/packages" /* GET_PACKAGES */}`);
...
async function getInstallInfoForPackages(ccsAdapter, commonParams, options) {
    ...
    const installInfo = filteredPackages.map(item => {
        const itemAsDependency = {
            ...
            dependencyType: "mandatory" /* MANDATORY */
        };
...
async function getRecursiveDependencies(pkg, installedPackages, onlinePackages, logger, dependencies = [], installInfoForDeps = []) {
    ...
    if (_.isEmpty(newPkgs)) {
        ...
        return Object.values(groupedDeps).map(deps => {
            const sortedDeps = deps.sort((item1, _item2) => {
                if (item1.dependencyType === "mandatory" /* MANDATORY */) {
                    return -1;
                }
                else {
                    return 1;
    ...
    function getNameOrIdForInstallInfoData(installInfo) {
        if (installInfo.installInfoType === "available" /* AVAILABLE */ ||
            installInfo.installInfoType === "installed" /* INSTALLED */) {
...
function getInstallInfoForDependency(dependency, installedPackages, onlinePackages, _logger) {
    ...
    if (installedPkg) {
        ...
        const result = {
            ...resultBase,
            installInfoType: "notAvailable" /* NOT_AVAILABLE */
        };
        return result;
    }
    function getInstalledPkgInstallInfo(installPkg) {
        ...
        if (latestOnline && latestOnline.packageVersion !== installPkg.packageVersion) {
            ...
            if (latestOnlineInstallInfo.installInfoType === "available" /* AVAILABLE */) {
                updateAvaliable = {
        ...
        const packageType = onlineInstallInfo && onlineInstallInfo.installInfoType === "available" /* AVAILABLE */
            ? onlineInstallInfo.packageType
            : null;
        const result = {
            ...resultBase,
            packageVersion: installPkg.packageVersion,
            installInfoType: "installed" /* INSTALLED */,
            packageType,
            featureType: installPkg.featureType === 'deviceSupport' ? "deviceSupport" /* DEVICE_SUPPORT */ : null,
    ...
    function getOnlinePkgInstallInfo(onlinePkg) {
        const downloadUrl = onlinePkg.downloadUrl[util_1.getPlatform()];
        const installCommand = (onlinePkg.installCommand || {})[util_1.getPlatform()] || null;
        const installSize = (onlinePkg.installSize || {})[util_1.getPlatform()] || 0;
        if (!downloadUrl) {
            const result = {
                ...resultBase,
                installInfoType: "notSupportedOnCurrentPlatform" /* NOT_SUPPORTED_ON_CURRENT_PLATFORM */
            };
            return result;
        }
        let packageType;
        if (!_.isEmpty(onlinePkg.modules)) {
            packageType = "sdkComposer" /* SDK_COMPOSER */;
        }
        else {
            if (onlinePkg.subType) {
                const subType = onlinePkg.subType;
                switch (subType) {
                    case 'ccsComponent':
                        packageType = "ccsComponent" /* CCS_COMPONENT */;
                        break;
                    case 'featureSupport':
                        packageType = "featureSupport" /* FEATURE_SUPPORT */;
                        break;
                    default:
                        util_2.assertNever(subType);
                        throw new Error(`Unknown subtype ${onlinePkg.subType}`);
                }
            }
            else {
        ...
        const result = {
            ...
            installInfoType: "available" /* AVAILABLE */,
            packageType,
            featureType: onlinePkg.featureType === 'deviceSupport' ? "deviceSupport" /* DEVICE_SUPPORT */ : null,
            downloadUrl,
            installCommand,
            installSize,
            licenses: onlinePkg.licenses || [],
            updateAvaliableInNewerCCSVersion: false,
            installTo: packageType === "ccsComponent" /* CCS_COMPONENT */ ||
                packageType === "featureSupport" /* FEATURE_SUPPORT */
                ? "ccs" /* CCS */
...
async function getBoardsAndDevicesKeyedOnPublicId(commonParams) {
    ...
    const tirex4RemoteserverUrl = commonParams.vars.remoteserverUrl;
    const boardAndDeviceData = await util_1.doTirex4Request(`${tirex4RemoteserverUrl}${"api/boardsDevices" /* GET_BOARDS_AND_DEVICES */}`);
