...
function calledWithExactlyDeep(spy, args) {
    ...
    expect_1.expect(!!call).to.be.true;
}
...
async function createProgressManager({ triggerEventSpy }) {
    const commonParams = await create_mock_common_params_1.createMockCommonParams({ triggerEventSpy });
...
async function createCCSAdapter({ metadata, triggerEventSpy, onIDERequest }) {
    const commonParams = await create_mock_common_params_1.createMockCommonParams({ metadata, triggerEventSpy });
...
async function getModule(args) {
    const commonParams = await create_mock_common_params_1.createMockCommonParams(args);
    const port = await startMockIDEServer();
    const entryModule = new entry_module_1.EntryModule(commonParams.triggerEvent, commonParams.logger, new MockEventBroker(port), { ...create_mock_common_params_1.getLocalServerConfig(), ccs_port: port.toString() }, commonParams.rex3Config);
    util_1.afterTest(() => entryModule.onClose());
    return entryModule;
}
...
class MockEventBroker {
    ...
    fetchData(dataName) {
        if (dataName === entry_module_1._IDE_SERVER_PORT) {
            return this.port;
        }
        throw new Error('Unknown data: ' + dataName);
    }
    hasData(dataName) {
        return dataName === entry_module_1._IDE_SERVER_PORT;
    }
}
