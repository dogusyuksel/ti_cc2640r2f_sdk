...
///////////////////////////////////////////////////////////////////////////////
const package1 = {
    packageOverview: database_utils_1.generatePackageOverview('package1', '1.0.0.0', {
        dependencies: [
            {
                refId: 'com.ti.DEP',
                require: "mandatory" /* MANDATORY */,
                versionRange: '1.2.3'
            }
        ],
        // @ts-ignore
...
const package2 = {
    packageOverview: database_utils_1.generatePackageOverview('package2', '1.0.0.0', {
        // @ts-ignore
...
describe('[cloudagent] OfflineMetadataManager', function () {
    describe('getInstalledPackages', function () {
        it('Should find and list the rex3 packages', async function () {
            ...
            const rexModule = await testing_util_1.getModule({
                metadata: {
                    devices: [],
                    devtools: [],
        ...
        it('Should return empty array if there are no rex3 packages', async function () {
            const rexModule = await testing_util_1.getModule({});
            const packages = await rexModule.getInstalledPackages();
            expect_1.expect(packages).to.be.empty;
        });
    });
    ...
    function verifyPackages(packages, expectedPackages) {
        expect_1.expect(packages).to.deep.equal(expectedPackages.map((pkg, idx) => {
