...
async function createMockCommonParams({ metadata, triggerEventSpy }) {
    ...
    const rex3DbPath = metadata ? await database_utils_1.writeMetadata(metadata) : test_helpers_1.getUniqueFolderName();
    ...
    if (metadata) {
        ...
        const packages = packagesJson.map(() => path.join(contentPath, test_helpers_1.getUniqueFileName()));
        await test_helpers_2.createPackages({ packages, packagesJson });
        util_1.afterTest(async () => {
            await fs.remove(rex3DbPath);
            await fs.remove(contentPath);
        });
    }
    const commonParams = {
        logger: create_mock_logger_1.createMockLogger(),
