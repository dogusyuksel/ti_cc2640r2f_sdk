...
class PackageInstaller {
    constructor(commonParams, progressManager, ccsAdapter) {
        ...
        this.activeItems = {};
        this.notifyIDECounter = new counter_1.Counter();
    }
    ...
    async doInstallPackage(packagePublicUid, installLocation, onProgressUpdate) {
        const installTempDirs = await util_1.makeTempDirs(installLocation, 'install');
        try {
            ...
            await this.commonParams.desktopQueue.add(async () => {
                ...
                const packageFoldersWithInfo = await Promise.all(packageFolders.map(async (item) => {
                    ...
                    const uid = dbBuilderUtils_1.formUid(info.id, info.version);
                    return { item, uid };
                }));
                ...
                await Promise.all([...packageFoldersWithInfo, ...nonPackageFoldersWithInfo].map(async ({ item, uid }) => {
                    ...
                    try {
                        ...
                        await index_1.mergeDirs(extractItem, installItem);
                        ...
                        error = util_2.getCombinedError(error, err);
                    }
                    if (error) {
                        throw error;
                    }
                ...
                try {
                    if (error) {
                        ...
                        const coutnerId = this.notifyIDECounter.getValue();
                        this.commonParams.desktopQueue
                            .onIdle()
                            .then(async () => {
                            if (coutnerId === this.notifyIDECounter.getValue()) {
            ...
            try {
                await util_1.removeTempDirs(installTempDirs);
            }
            catch (e) {
    ...
    async downloadAndExtractPackageFiles(packageUid, { downloadDir, extractDir }, onProgressUpdate) {
        ...
        const downloadUrl = pkg.downloadUrl[util_1.getPlatform()];
        if (!downloadUrl) {
            throw new Error('platform not supported, missing downloadUrl');
        }
        const zip = await util_2.downloadFile(downloadUrl, downloadDir, onProgressUpdate);
        // Now extract or install the package zip / installer into a temporary extract folder
        const installCommand = pkg.installCommand && pkg.installCommand[util_1.getPlatform()];
        if (pkg.installCommand && !installCommand) {
            ...
            else if (util_1.getPlatform() === response_data_1.Platform.MACOS) {
                // OSX - Need to extract the .app folder first
                await util_2.extract(zip, downloadDir, onProgressUpdate);
            }
            else if (util_1.getPlatform() === response_data_1.Platform.LINUX) {
                // Set permissions (not needed on OSX since the zip will retain the permissions)
                await fs.chmod(zip, 0o775);
            }
            try {
                const items = await util_2.install(installCommandFinal, downloadDir, extractDir, onProgressUpdate, installStream);
                return items;
            }
            catch (e) {
            ...
            const items = await util_2.extract(zip, extractDir, onProgressUpdate);
            return items;
        }
    }
