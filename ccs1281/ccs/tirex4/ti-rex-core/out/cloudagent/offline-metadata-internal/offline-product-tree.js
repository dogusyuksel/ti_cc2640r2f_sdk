...
async function offlineProductTreeNoQ({ dbDevices, dbDevtools, rex3Server, tempDirs, installLocation, onProgressUpdate }) {
    ...
    const { data: deviceRecords } = await request_helpers_1.doGetRequest(rex3Server + '/api/devices');
    const { data: devtoolRecords } = await request_helpers_1.doGetRequest(rex3Server + '/api/devtools');
