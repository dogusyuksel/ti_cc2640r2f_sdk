...
class RexDBSplit extends irexdb_1.IRexDB {
    ...
    constructor(logger, dir) {
         ...
         */
        this.use = callbackifyAny_1.callbackifyAny(this.useAsync);
        /**
         * Load all packages into memory
         */
        this.useAll = callbackifyAny_1.callbackifyAny(this.useAllAsync);
        this.dir = dir;
    ...
    findOne(query, callback) {
        this._findInCache(query, true, (err, results) => {
            if (callbacks_1.isError(err, results) || results.length === 0) {
                callback(err, null);
            }
            else {
