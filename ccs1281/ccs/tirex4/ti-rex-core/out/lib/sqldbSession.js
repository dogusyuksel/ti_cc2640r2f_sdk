...
class SqldbSession {
    ...
    static async cachePackageGroupInfo(sqldb) {
        ...
        for (const packageGroupDB of allPackageGroupsDB) {
            ...
            const packageGroup = {
                ...
                packagesPublicUids,
                packageGroupPublicIdEncoded: dbBuilderUtils_1.encodePackageGroupPublicId(packageGroupDB.publicId),
        ...
        for (const packageRecordDB of packageRecordsDB) {
            ...
            if (isMainPackage && softwarePackageGroup) {
                const packageRecord = {
                    ...
                    packageType: "softwareMain" /* SOFTWARE_MAIN */
                };
                ...
                const packageRecord = {
                    ...packageRecordBase,
                    packageType: "softwareSupplemental" /* SOFTWARE_SUPPLEMENTAL */
                };
                packageRecords.push(packageRecord);
            }
        }
    ...
    static async cacheDevicesAndDevtoolsInfo(sqldb, allPackageRecords, devicePackageGroup, devtoolPackageGroup) {
        ...
        const deviceSupportPackages = _.filter(allPackageRecords, pkg => pkg.packageType === 'softwareMain' &&
            pkg.subType === 'featureSupport' &&
            pkg.featureType === 'deviceSupport');
...
class SqldbSessionFactory {
    ...
    async getCurrentSessionPromise() {
        ...
        if (!this.currentSessionPromise) {
            const dbUpdateInfo = await dbUpdateInfo_1.fetchLastUpdateInfo(require(this.dinfraPath));
