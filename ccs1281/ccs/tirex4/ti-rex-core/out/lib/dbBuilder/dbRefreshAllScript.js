...
async function dbRefreshAllScript(args) {
    ...
    const { dinfra, vars } = await appConfig_1.configureDinfraAndVars(dinfraPath, dconfigPath, appconfigPath, appconfigCmdlineOverrides);
    const dinfraLogger = logger_1.createLogger(dinfra, 'tirexDbRefreshScript');
    dinfra.dlog.console();
    ...
    if (args.defaultJson) {
        ...
        if (!args.rediscoverOnly) {
            ...
            const refreshParams = await util_1.getRefreshParams(entries.map(item => ({ entry: item, request: 'nothing' })), entries, vars.contentBasePath, vars.zipsFolder);
