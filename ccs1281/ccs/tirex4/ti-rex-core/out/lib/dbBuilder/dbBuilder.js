...
exports.refreshMessageLevel = {
    [0 /* NONE */]: 'info',
    [1 /* WARNING */]: 'warning',
    [2 /* ERROR_CONTINUE */]: 'error',
    [3 /* CRITICAL_ABORT_PACKAGE */]: 'critical',
    [4 /* EMERGENCY_ABORT_REFRESH */]: 'emergency'
};
async function updateResultAsMap(logger, operation, fullPackageList, refreshAll, resultAsMap, statusMap, dbBasePath) {
    ...
    const result = refreshPackageList.map(item => {
        ...
        switch (msgLevel) {
            case 4 /* EMERGENCY_ABORT_REFRESH */:
            case 3 /* CRITICAL_ABORT_PACKAGE */:
            case 2 /* ERROR_CONTINUE */:
                refreshResultEntry = { actionResult: "failed" /* FAILED */, msgLevel };
                break;
            case 1 /* WARNING */:
                refreshResultEntry = { actionResult: "succeeded" /* SUCCEEDED */, msgLevel };
                break;
            case 0 /* NONE */:
                if (item.operation === "doNothing" /* DO_NOTHING */) {
                    if (refreshAll) {
                        refreshResultEntry = {
        ...
        if (refreshAll && item.operation === "doNothing" /* DO_NOTHING */) {
            refreshResult.package.operation = "updatedPackage" /* UPDATED_PACKAGE */;
        }
        return refreshResult;
    });
    const refreshSucceeded = !result.some(entry => entry.status.actionResult === "failed" /* FAILED */);
    ...
    for (const key of requestMap.keys()) {
        const refreshResultEntry = {
            actionResult: "not started" /* NOTSTARTED */
        };
        resultAsMap.set(key, {
...
async function packagePresentAndValidityChecks(originalFullPackageList, contentBasePath, logger, validationType) {
    ...
    for (const packageEntry of originalFullPackageList) {
        ...
        if (!(await fs.pathExists(filepath))) {
            if (packageEntry.operation === "removePackage" /* REMOVE_PACKAGE */) {
                ...
                resultAsMap.set(uid, {
                    package: packageEntry,
                    status: {
                        actionResult: "not found" /* NOTFOUND */
                    }
                });
                allValid = false;
            }
        ...
        const jsonPath = packageEntry.modulePrefix
            ? (await package_helpers_1.getAllPackageTirexJsonPaths(filepath)).find(item => packageEntry.modulePrefix && item.includes(packageEntry.modulePrefix))
            : await package_helpers_1.getPackageTirexJsonPath(filepath);
        ...
        const packageUId = dbBuilderUtils_1.formUid(data.id, data.version);
        if (packageEntry.operation === "replacePackage" /* REPLACE_PACKAGE */ && packageUId !== uid) {
        ...
        if (data.type === 'devices' || data.type === 'devtools') {
            if (latestDevPkgMap.has(data.id)) {
                ...
                if (versioning.gtr(data.version, devPkgInfo.version)) {
                    ...
                    const packageUidOld = dbBuilderUtils_1.formUid(data.id, oldVersion);
                    const r = resultAsMap.get(packageUidOld);
                    if (r.package.operation === "doNothing" /* DO_NOTHING */ ||
                        r.package.operation === "removePackage" /* REMOVE_PACKAGE */) {
                        r.package.operation = "excludePackage" /* EXCLUDE_PACKAGE */;
                        r.status.actionResult = "not applicable" /* NOTAPPLICABLE */;
                    }
                    else {
                        // disallow explicit add/replace of an older version since it would just be ignored
                        refreshOldDevPackageError = true;
                        r.status.actionResult = "failed" /* FAILED */;
                    }
                    ...
                    if (packageEntry.operation === "doNothing" /* DO_NOTHING */ ||
                        packageEntry.operation === "removePackage" /* REMOVE_PACKAGE */) {
                        packageEntry.operation = "excludePackage" /* EXCLUDE_PACKAGE */;
                    }
                    else {
                        // disallow explicit add/replace of an older version since it would just be ignored
                        refreshOldDevPackageError = true;
                ...
                logger.info(`Multiple versions of device/devtool package ${data.id} found. Only the latest version will be used. Dropping old version ${oldVersion}`);
                if (refreshOldDevPackageError) {
                    allValid = false;
                    const packageUidForError = dbBuilderUtils_1.formUid(data.id, oldVersion);
        ...
        resultAsMap.set(uid, {
            package: packageEntry,
            status: {
                actionResult: packageEntry.operation === "doNothing" /* DO_NOTHING */ ||
                    packageEntry.operation === "excludePackage" /* EXCLUDE_PACKAGE */
    ...
    const refreshAll = Array.from(latestDevPkgMap.values()).some(e => ["addPackage" /* ADD_PACKAGE */, "replacePackage" /* REPLACE_PACKAGE */].includes(e.operation));
    ...
    const excludedPackageUids = Array.from(resultAsMap.values())
        .filter(r => r.package.operation === "excludePackage" /* EXCLUDE_PACKAGE */)
        .map(r => r.package.uid);
...
async function _refreshDatabaseDelete({ logger, packageUIds, dbs, dbBasePath }) {
    ...
    packageUIds.forEach(packageUId => {
        statusResultMap.set(packageUId, 4 /* EMERGENCY_ABORT_REFRESH */);
    });
    ...
    for (const packageUId of packageUIds) {
        try {
            ...
            statusResultMap.set(packageUId, 0 /* NONE */);
        }
        catch (err) {
    ...
    try {
        await indexer_1.index(dbBasePath);
    }
    catch (err) {
    ...
    try {
        await compacter_1.compact(dbBasePath);
    }
    catch (err) {
...
async function createOldOverviewsDBFile(dbOverviews, dbBasePath, logger) {
    ...
    const regex = new RegExp(indexer_1.regexNotIndex, 'g');
...
async function _refreshDatabase({ contentPackages, dbs, refreshAll, packageUids, contentBasePath, dbBasePath, logger }) {
    ...
    for (const contentPackage of contentPackages) {
        ...
        try {
            ...
            if (err.refreshMessageLevel === 4 /* EMERGENCY_ABORT_REFRESH */) {
                logger.emergency('An error occurred while refreshing macros');
                logger.emergency(rexError_1.stringifyError(err));
                throw err;
            }
            else if (err.refreshMessageLevel === 3 /* CRITICAL_ABORT_PACKAGE */) {
    ...
    if (refreshAll) {
        for (const contentPackage of contentPackages) {
            ...
            try {
                ...
                const msgLevel = err.refreshMessageLevel;
                if (err.refreshMessageLevel === 3 /* CRITICAL_ABORT_PACKAGE */) {
                    updateStatusMap(msgLevel, contentPathToUIdMap, contentPackage, statusResultMap);
    ...
    for (const contentPackage of contentPackages) {
        const metadataDir = await dbBuilderUtils_1.getMetadataDir(contentBasePath, contentPackage.path);
    ...
    for (const contentPackage of contentPackages) {
        ...
        const metadataDir = await dbBuilderUtils_1.getMetadataDir(contentBasePath, packagePath);
        let packageMetadata;
        try {
            const result = await dbBuilderUtils_1.getPackageMetadataAsync(contentBasePath, packagePath, metadataDir, macros[packagePath], contentPackage.modulePrefix, vars_1.Vars.PACKAGE_AUX_DATA_FILE, logger);
        ...
        const packageUId = dbBuilderUtils_1.formUid(packageMetadata.id, packageMetadata.version);
                ...
                if (dependency.require && dependency.require === 'mandatory') {
                    ...
                    if (dependencyPackageOverview == null) {
                        ...
                        if (dependencyPackageOverviews.length !== 0) {
                            ...
                            let worstMessageLevel = statusResultMap.get(packageUId) || 0 /* NONE */;
                            worstMessageLevel = Math.max(worstMessageLevel, 1 /* WARNING */);
...
function updateStatusMap(msgLevel, contentPathToUIdMap, contentPackage, statusResultMap) {
    ...
    if (!statusResultMap.has(packageUid) ||
        msgLevel > (statusResultMap.get(packageUid) || 0 /* NONE */)) {
