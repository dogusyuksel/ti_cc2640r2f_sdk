...
class RefreshManager {
    ...
    constructor(dbBasePath, defaultLogger) {
        ...
        this.refreshUsingConfigFileCb = callbackifyAny_1.callbackifyAny(this.refreshUsingConfigFile);
        this.refreshQueueP = new PQueue({ concurrency: 1 });
        this.createDB();
    }
    createDB() {
    ...
    async refreshUsingConfigFile(contentPackagesConfigFile, contentBasePath, validationType, logger = this.defaultLogger) {
        ...
        let packages;
        try {
            packages = await readJsonWithComments_1.readJsonWithComments(contentPackagesConfigFile);
        }
        catch (err) {
            logger.error(`Failed to read and parse ${contentPackagesConfigFile}: ${err.message}`);
            throw new rexError_1.RefreshError({
                refreshMessageLevel: 4 /* EMERGENCY_ABORT_REFRESH */,
        ...
        for (const packageEntry of packages) {
            ...
            const metadataDir = await dbBuilderUtils_1.getMetadataDir(contentBasePath, packageEntry);
            const [data] = await fs.readJson(path.join(dirpath, metadataDir, vars_1.Vars.PACKAGE_TIREX_JSON));
            refreshPackageList.push({
                uid: dbBuilderUtils_1.formUid(data.id, data.version),
    ...
    async removeBackup() {
        await dbBuilder_1.removeBackup(this.dbBasePath);
    }
    ...
    async refreshLayer(originalFullPackageList, contentBasePath, overrideBackup, dbBasePath, logger, forceRefreshAll, validationType) {
        const option = dbBuilder_1.updateOption(validationType);
        const validityResult = await dbBuilder_1.packagePresentAndValidityChecks(originalFullPackageList, contentBasePath, logger, validationType);
        ...
        if (validationType === 'refresh' || validationType === 'all') {
            await dbBuilder_1.createBackup(overrideBackup, dbBasePath);
        }
        ...
        if (!refreshAll) {
            for (const packageEntry of fullPackageList) {
                if (packageEntry.operation === "removePackage" /* REMOVE_PACKAGE */ ||
                    packageEntry.operation === "replacePackage" /* REPLACE_PACKAGE */) {
                    let packageOverview;
                    ...
                    if (!packageOverview) {
                        if (packageEntry.operation === "removePackage" /* REMOVE_PACKAGE */) {
                            ...
                            packageEntry.operation = "doNothing" /* DO_NOTHING */;
                            ...
                            packageEntry.operation = "addPackage" /* ADD_PACKAGE */;
                        }
                    }
        ...
        if (option === dbBuilder_1.Option.REFRESH_SCHEMA || option === dbBuilder_1.Option.ALL) {
            if (refreshAll) {
                ...
                const refreshAllSucceeded = await dbBuilder_1.updateResultAsMap(logger, null, fullPackageList, true, resultAsMap, statusMap, dbBasePath);
            ...
            if (!refreshAll) {
                // Remove packages
                const deleteList = fullPackageList.filter(item => item.operation === "removePackage" /* REMOVE_PACKAGE */);
                if (deleteList.length !== 0) {
                    ...
                    const deleteRefreshSucceeded = await dbBuilder_1.updateResultAsMap(logger, "removePackage" /* REMOVE_PACKAGE */, fullPackageList, false, resultAsMap, deleteStatusMap, dbBasePath);
                    if (!deleteRefreshSucceeded) {
                        return { resultAsMap, success: false };
                    }
                }
                // Add packages
                const addList = fullPackageList.filter(item => item.operation === "addPackage" /* ADD_PACKAGE */);
                if (addList.length !== 0) {
                    ...
                    const addRefreshSucceeded = await dbBuilder_1.updateResultAsMap(logger, "addPackage" /* ADD_PACKAGE */, fullPackageList, false, resultAsMap, addStatusMap, dbBasePath);
                    if (!addRefreshSucceeded) {
                        return { resultAsMap, success: false };
                    }
                }
                // Replace packages
                const replaceList = fullPackageList.filter(item => item.operation === "replacePackage" /* REPLACE_PACKAGE */);
                for (const item of replaceList) {
                    ...
                    const deleteForReplaceRefreshSucceeded = await dbBuilder_1.updateResultAsMap(logger, "removePackage" /* REMOVE_PACKAGE */, fullPackageList, false, resultAsMap, deleteForReplaceStatusMap, dbBasePath);
                    ...
                    const replaceRefreshSucceeded = await dbBuilder_1.updateResultAsMap(logger, "replacePackage" /* REPLACE_PACKAGE */, fullPackageList, false, resultAsMap, replaceStatusMap, dbBasePath);
            ...
            const refreshResult = fullPackageList
                .filter(item => item.operation === "doNothing" /* DO_NOTHING */)
                .map(item => {
                const entry = {
                    actionResult: "not applicable" /* NOTAPPLICABLE */
                };
            ...
            refreshResult.forEach(item => {
                if (item.status.actionResult !== "succeeded" /* SUCCEEDED */ &&
                    item.status.actionResult !== "not applicable" /* NOTAPPLICABLE */) {
                    success = false;
                }
            ...
            if (refreshAll) {
                ...
                logger.info(`Latest device/devtool packages used: ${[...latestDevPkgMap]
                    .map(([k, v]) => dbBuilderUtils_1.formUid(k, v.version))
                    .join(' ')}`);
                if (!success) {
                    ...
                    for (const key of resultAsMap.keys()) {
                        ...
                        if (result !== "not applicable" /* NOTAPPLICABLE */ &&
            ...
            if (!success) {
                ...
                await dbBuilder_1.restoreFromBackupAndDelete(logger, dbBasePath);
            }
            else {
                await dbBuilder_1.removeBackup(dbBasePath);
        ...
        if (option === dbBuilder_1.Option.URL || option === dbBuilder_1.Option.ALL) {
            ...
            util_1.insertBlankLine(logger);
            const packageMetadataList = new Map();
            for (const packageEntry of fullPackageList) {
                const metadataDir = await dbBuilderUtils_1.getMetadataDir(contentBasePath, packageEntry.content);
                ...
                if (fs.existsSync(packageFile)) {
                    ...
                    const { id, version } = dbBuilderUtils_1.splitUid(packageEntry.uid);
                    util_1.setPackageMetadataList(packageMetadataList, packageEntry.content, id, version, packageMetadata.type);
                }
            }
    ...
    async _refreshTask({ packagePaths, installPaths, installCommands, installSizes, modulePrefixes, packageUids, refreshAll, remove = false, contentBasePath, logger }) {
        ...
        if (refreshAll) {
            ...
            try {
                const statusResultMap = await dbBuilder_1._refreshDatabase({
                    contentPackages,
                    packageUids,
                    refreshAll: true,
                    dbs: this.dbs,
                ...
                logger.critical(rexError_1.stringifyError(err));
                throw err;
            }
        }
        else if (!remove) {
            ...
            let statusResultMap;
            try {
                statusResultMap = await dbBuilder_1._refreshDatabase({
                    contentPackages,
                    packageUids,
                    refreshAll: false,
                    dbs: this.dbs,
            ...
            for (const packageUid of packageUids) {
                ...
                try {
                    const pieceStatusMap = await dbBuilder_1._refreshDatabaseDelete({
    ...
    async deletePackagesHandlingOverview(packageUId, dbBasePath, logger) {
        ...
        await dbBuilder_1.createOldOverviewsDBFile(dbOverviews, dbBasePath, logger);
    }
    /**
...
async function schemaValidation(fullPackageList, contentBasePath, logger, refreshAll) {
    ...
    const validationList = refreshAll
        ? fullPackageList
        : fullPackageList.filter(item => item.operation === "addPackage" /* ADD_PACKAGE */ ||
            item.operation === "replacePackage" /* REPLACE_PACKAGE */);
...
function getLatestPacakgesToValidate(latestPkgMap) {
    const result = Array.from(latestPkgMap.entries()).filter(([_, { operation }]) => {
        return (operation === "addPackage" /* ADD_PACKAGE */ ||
