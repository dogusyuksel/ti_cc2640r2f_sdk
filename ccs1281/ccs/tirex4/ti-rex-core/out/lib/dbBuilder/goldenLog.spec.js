...
const { loggerManager } = rex_1._getRex();
/**
...
describe.skip('refresh', async function () {
    ...
    it('should successfully do a refresh', async () => {
        ...
        const goldenFiles = fsutils_1.readDirRecursive(goldenLogDbFolder);
        const refreshFiles = fsutils_1.readDirRecursive(dbBasePath);
        chai_1.expect(refreshFiles).to.deep.equal(goldenFiles);
        ...
        chai_1.expect(stdout).to.be.empty;
    });
});
