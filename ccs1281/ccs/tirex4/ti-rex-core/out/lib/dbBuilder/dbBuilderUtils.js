...
function base64UrlSafeToHex(base64UrlSafe) {
    return Buffer.from(custom_url_helpers_1.undoBase64UrlSafe(base64UrlSafe), 'base64').toString('hex');
}
...
function hexToBase64UrlSafe(hex) {
    return custom_url_helpers_1.makeBase64UrlSafe(Buffer.from(hex, 'hex').toString('base64'));
}
...
function encodePackageGroupPublicId(s) {
    return custom_url_helpers_1.makeBase64UrlSafe(crypto
        .createHash('sha1')
        .update(s, 'utf8')
        .digest('base64')
...
async function getPackageDataWithAux(packageFile, packageAuxDataFile, packageMacros, logger) {
    ...
    if (packageMetadata.moduleOf) {
        ...
        const { rootCategory: _rootCategory = null, name = null, addDependences = [], removeSupplements = false, hideByDefault = false, packageIdAliases = [] } = auxData;
