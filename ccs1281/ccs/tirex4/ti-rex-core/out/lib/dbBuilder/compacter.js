...
async function compact(dbPath) {
    ...
    const resourceDbFiles = fsutils_1.readDirRecursive(resourceDbFullDir, regexNotIndex, 1);
    const overviewDbFiles = fsutils_1.readDirRecursive(overviewDbDir, regexNotIndex, 1);
    ...
    const resourceDbFullFiles = fsutils_1.readDirRecursive(resourceDbFullDir, regexNotIndex, 1);
...
const writeStreamDBAsync = promisifyAny_1.promisifyAny(writeStreamDB);
...
async function mergeCompact(dbPath, logger) {
    ...
    const resourceDbFiles = fsutils_1.readDirRecursive(resourceDbDir, regexNotIndex, 1);
    try {
...
async function mergeCompactReadStream(file, resourceDbDir, allResourcesMap) {
    ...
    await promise_utils_1.promisePipe([fse.createReadStream(path.join(resourceDbDir, file)), jsonStream]);
}
function hashRecord(record) {
    // remove the package path portion to make these fields version independent
    ['link', 'icon', 'image'].forEach(prop => {
        util_1.setValueForPair(record, record, prop, item => item && item.replace(record.packagePath, ''));
    });
    // remove fields to make record same between versions
    delete record.packageUId;
    delete record.packagePath;
    delete record._id;
    return idHelperTirex3_1.createUuid(record).idVal;
}
