...
async function createContentMap(args) {
    ...
    const { dinfra } = await appConfig_1.configureDinfraAndVars(dinfraPath, dconfigPath, appconfigPath);
    dinfra.dlog.console();
...
async function fillInLatestUrlForContentLinks(contentLinkToUrlMap) {
    ...
    for (const packageGroup of packageGroupsDef) {
        ...
        const encodedPgUidLatest = `${dbBuilderUtils_1.encodePackageGroupPublicId(pgId)}__LATEST`;
