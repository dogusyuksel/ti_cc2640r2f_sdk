...
describe('getLatestVersionPackageGroups', async function () {
    it('should generate a list of package groups with the latest version', async () => {
        ...
        const latestPackageGroups = sitemapGenerator_1.getLatestPackageGroups(fakePackageGroups);
        const result = [];
...
describe('getDuplicateResourceLinks', async () => {
    it('Should take an array of resources and return an object containing resources with duplicate links', () => {
        ...
        const duplicateResourceLinks = sitemapGenerator_1.getDuplicateResourceLinks(fakeResources);
        const expectedResult = {
            FakeLink0: {
                name: 'Resource 0',
                duplicatePaths: {
        ...
        chai_1.expect(JSON.stringify(duplicateResourceLinks)).to.equal(JSON.stringify(expectedResult));
    });
});
describe('writeOverviewToSitemap', async () => {
    ...
    it('should return false if no data is being written', async () => {
        ...
        const wroteData = sitemapGenerator_1.writeOverviewToSitemap(fakeOverviews, fakeID, fakeID2, writeStream, linkSummary);
        chai_1.expect(wroteData).to.equal(false);
    ...
    it('should write overviews to a WriteStream', done => {
        ...
        const linkSummary = initLinkSummary();
        const wroteData = sitemapGenerator_1.writeOverviewToSitemap(fakeOverviews, fakeID, fakeID2, writeStream, linkSummary);
        chai_1.expect(wroteData).to.equal(true);
        writeStream.end();
    ...
    it('should have written correct data', async () => {
        ...
        chai_1.expect(fileData).to.equal(expectedXmlContent);
    });
    after(async () => {
        await fs.unlink(testFile);
    });
...
describe('writeResourcesToSitemap', async () => {
    ...
    it('should return false if no data is being written', async () => {
        ...
        const writeStream = fs.createWriteStream(testFile);
        const linkSummary = initLinkSummary();
        const wroteData = sitemapGenerator_1.writeResourcesToSitemap(fakeResources, 'fakeFilePath', fakeID, fakeID2, writeStream, linkSummary);
        chai_1.expect(wroteData).to.equal(false);
        await fs.unlink(testFile);
    });
    ...
    it('Should be able to handle multiple resources having the same link', async () => {
        const fakeResources = makeDuplicateResources();
        const duplicateOverviewLinks = sitemapGenerator_1.getDuplicateResourceLinks(fakeResources);
        ...
        const writeStream = fs.createWriteStream(testFile);
        const linkSummary = initLinkSummary();
        const wroteData = sitemapGenerator_1.writeResourcesToSitemap(makeDuplicateResources(), 'fakeJSON', 'fakeID', 'fakeID2', writeStream, linkSummary);
        writeStream.end();
        ...
        chai_1.expect(content.toString()).to.equal(expectedXmlContent);
    });
    after(async () => {
        await fs.unlink(testFile);
    });
...
function makeDuplicateResources() {
    ...
    fakeResources[8].fullPaths = [['FakeStart', 'FakePath7.1'], ['FakeStart', 'FakePath7.2']];
