...
describe('lastUpdateInfo', () => {
    beforeEach(async function () {
        ...
        await dinfra
            .queryResources()
            .withNamePrefix(dbUpdateInfo_1._getLastUpdateResourceName())
            .invoke()
            .progress(queryResult => {
            versionsToDestroy.push(queryResult.version);
        });
        await p_iteration_1.forEach(versionsToDestroy, version => dinfra.destroyResource(dbUpdateInfo_1._getLastUpdateResourceName(), version));
    });
    ...
    it('should return timestamp of 0 and tablePrefix of null if there is no resource', async () => {
        const lastUpdate = await dbUpdateInfo_1.fetchLastUpdateInfo(dinfra);
    ...
    it('should create a valid initial resource on a first fetch (REX-2117)', async () => {
        await dbUpdateInfo_1.fetchLastUpdateInfo(dinfra);
    ...
    it('should create a resource with valid set values (REX-2117)', async () => {
        ...
        await dbUpdateInfo_1.setLastUpdateInfo(dinfra, aTablePrefix);
    ...
    it('should fetch the last update info', async () => {
        const aTablePrefix = 'tirex0';
        await dbUpdateInfo_1.setLastUpdateInfo(dinfra, aTablePrefix);
        const info = await dbUpdateInfo_1.fetchLastUpdateInfo(dinfra);
        const meta = await getAndVerifyResource();
...
async function getAndVerifyResource() {
    ...
    const clone = lodash_1.cloneDeep(meta);
    await resource.close();
    return clone;
}
function getResource() {
    return dinfra.openResource(dbUpdateInfo_1._getLastUpdateResourceName(), {
        create: false
    });
}
