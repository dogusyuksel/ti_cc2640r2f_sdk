...
async function metadataImport(dinfra, sqldb, dbPathToImport, dBPathPreviouslyImported, options) {
    if (!logger) {
        const dinfraLogger = logger_1.createLogger(dinfra, 'tirexDbImporter');
    ...
    if (vars_1.Vars.DB_TABLE_PREFIX !== vars_1.Vars.DB_TABLE_PREFIX_AUTO || options.notify_forTestingOnly) {
        await dbUpdateInfo_1.setLastUpdateInfo(dinfra);
...
function notifyDatabaseUpdated(dinfra, logger) {
    return new Promise((resolve, reject) => {
        dinfra
            ...
            dbUpdateInfo_1.notifyDatabaseUpdated(dinfra);
            resolve();
        });
    });
}
        ...
        if (!packageRecord.supplements && !packageRecord.moduleOf) {
            ...
            if (packageRecord.dependencies) {
                for (const dependency of packageRecord.dependencies) {
                    const depPackage = await findBundle_1.findLatestBundle(dependency.refId, dependency.versionRange, dbOverviews);
            ...
            if (packageRecord.modules) {
                for (const moduleItem of packageRecord.modules) {
                    const modulePackage = await findBundle_1.findLatestBundle(moduleItem.refId, moduleItem.versionRange, dbOverviews);
        ...
        await new Promise((resolve, reject) => {
            try {
                const pipeline = stream_chain_1.chain([
                    ...
                    Replace_1.replace({ filter: /\bpackageOrder\b/i }),
