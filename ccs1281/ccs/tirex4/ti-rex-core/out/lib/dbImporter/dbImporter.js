...
async function dbImport(args) {
    ...
    const logger = logger_1.createLogger(dinfra, 'tirexDbImporter');
    dinfra.dlog.console();
    ...
    try {
        if (args.switchTablePrefix) {
            ...
            await dbUpdateInfo_1.setLastUpdateInfo(dinfra, oppositeTablePrefix);
            ...
            if (!args.skipContent) {
                await contentImporter_1.contentImport(dinfra, vars_1.Vars.CONTENT_BASE_PATH, vars_1.Vars.DB_RESOURCE_PREFIX, {
            ...
            if (!args.skipMetadata) {
                const dbLastUpdateInfo = await dbUpdateInfo_1.fetchLastUpdateInfo(dinfra);
                ...
                const operationPerformed = await sqldbImporter_1.metadataImport(dinfra, sqldb, dbPathToImport, dBPathPreviouslyImported, {
                    include,
                    exclude,
