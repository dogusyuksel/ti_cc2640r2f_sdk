...
function processConfig(configPassedIn, configCmdlineOverrides = {}) {
    ...
    util_1.getObjectKeys(config).forEach(prop => {
        if (configCmdlineOverrides[prop]) {
            util_1.setValueForPair(config, configCmdlineOverrides, prop, value => value);
        }
    });
    util_1.getObjectKeys(config).forEach(prop => {
        util_1.setValueForPair(config, config, prop, configValue => {
