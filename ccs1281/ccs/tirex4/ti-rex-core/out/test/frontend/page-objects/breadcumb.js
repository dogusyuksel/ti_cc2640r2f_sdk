...
(function (Breadcrumb) {
    ...
    async function clickBreadcrumbItem(item) {
        await util_1.waitForPromisesAfterActionToResolve(async () => {
            const element = await util_1.getItemById(await getBreadcrumbNodes(), util_2.TEST_ID.breadcrumbNode(item.nodeDbId));
            await util_1.clickElement(element);
        });
    }
    ...
    async function verifyBreadcrumb(path, _selectedItem) {
        ...
        expect_1.expect(itemIds).to.deep.equal(path.map(node => util_2.TEST_ID.breadcrumbNode(node.nodeDbId)));
    }
