...
(function (Filter) {
    ...
    async function openAllFiltersPopup() {
        await util_1.waitForPromisesAfterActionToResolve(() => util_1.clickElement(protractor_1.element(protractor_1.by.id(util_2.TEST_ID.filterAllFiltersButton))));
    }
    Filter.openAllFiltersPopup = openAllFiltersPopup;
    // Verify
    async function verifyRadioButtonPresence(key, publicId) {
        const isRadioButtonDisplayed = await protractor_1.element(protractor_1.by.id(util_2.TEST_ID.filterRadioButton(key, publicId))).isPresent();
        expect_1.expect(isRadioButtonDisplayed).to.equal(true);
    }
    Filter.verifyRadioButtonPresence = verifyRadioButtonPresence;
    async function verifyRadioButtonValue(key, publicId, value) {
        const selected = await protractor_1.element(protractor_1.by.id(util_2.TEST_ID.filterRadioButton(key, publicId))).isSelected();
        expect_1.expect(selected).to.equal(value);
    }
...
class FilterInput {
    static async click() {
        ...
        await open_close_helpers_1.waitUntilItemOpen(util_2.TEST_ID.suggestionsList);
    }
    static async clickSuggestion(id) {
        await util_1.waitForPromisesAfterActionToResolve(() => util_1.clickElement(protractor_1.element(protractor_1.by.id(util_2.TEST_ID.suggestionsItem(id)))));
    }
    static async enterText(text) {
        await util_1.waitForPromisesAfterActionToResolve(() => {
...
class BoardDeviceInput extends FilterInput {
    static getInput() {
        return protractor_1.element(protractor_1.by.id(util_2.TEST_ID.filterBoardDeviceFilter)).element(protractor_1.by.css('input'));
    }
}
exports.BoardDeviceInput = BoardDeviceInput;
class KeywordInput extends FilterInput {
    static getInput() {
        return protractor_1.element(protractor_1.by.id(util_2.TEST_ID.filterKeywordsFilter)).element(protractor_1.by.css('input'));
    }
}
