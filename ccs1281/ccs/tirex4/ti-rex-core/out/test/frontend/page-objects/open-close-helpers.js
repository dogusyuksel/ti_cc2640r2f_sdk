...
async function openItem(itemButtonId, itemId) {
    await util_1.clickElement(protractor_1.element(protractor_1.by.id(itemButtonId)));
...
async function waitUntilItemOpen(itemId) {
    await protractor_1.browser.wait(protractor_1.ExpectedConditions.presenceOf(protractor_1.element(protractor_1.by.id(itemId))), util_1.BROWSER_WAIT, 'Item did not open');
}
...
async function openNestedDropdownMenu(menuButtonId, _menuId) {
    await protractor_1.browser
        .actions()
        .mouseMove(protractor_1.element(protractor_1.by.id(menuButtonId)))
        .perform();
