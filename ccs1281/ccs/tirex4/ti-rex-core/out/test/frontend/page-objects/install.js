...
(function (Install) {
    ...
    async function togglePackageCheckbox(packageId, versionRange) {
        const item = await util_2.getItemById(await getPackageRowsCheckboxes(), util_1.TEST_ID.installSelectCheckbox(packageId, versionRange));
        await util_2.clickElement(item);
    }
    Install.togglePackageCheckbox = togglePackageCheckbox;
    async function next() {
        await util_2.clickElement(protractor_1.element(protractor_1.by.id(util_1.TEST_ID.installNextButton)));
    }
    Install.next = next;
    async function cancel() {
        await util_2.clickElement(protractor_1.element(protractor_1.by.id(util_1.TEST_ID.installCancelButton)));
        // TODO consider verifying modal closed
    }
    Install.cancel = cancel;
    async function openInstallPathsDropdown() {
        await util_2.clickElement(protractor_1.element(protractor_1.by.id(util_1.TEST_ID.installLocationContextMenuButton)));
    }
    // Verify
    async function verifyInstallDialogOpen() {
        await open_close_helpers_1.waitUntilItemOpen(util_1.TEST_ID.installDialog);
    }
    Install.verifyInstallDialogOpen = verifyInstallDialogOpen;
    async function verifyInstallPathsDropdownOpen() {
        await open_close_helpers_1.waitUntilItemOpen(util_1.TEST_ID.installLocationContextMenuButton);
    }
    ...
    async function verifyPackageRows(pkg, options, allPackages, unsupported) {
        ...
        expect_1.expect(descriptions.length).to.equal(pkg.dependencies.length + 1);
        expect_1.expect(checkboxes.length).to.equal(pkg.dependencies.length + 1);
        expect_1.expect(locations.length).to.equal(pkg.dependencies.length + 1);
        // Verify package
        {
        ...
        async function verifyPackageInner(onlinePkg, isLocal, descriptionStartingText, idx, unsupported) {
            ...
            expect_1.expect(description).to.deep.equal(expectedDesc);
            // Validate checkbox
            const checkbox = checkboxes[idx];
            const checked = await checkbox.isSelected();
            expect_1.expect(checked).to.equal(!isLocal && !unsupported && !!onlinePkg);
            // Validate location
            ...
            expect_1.expect(location).to.deep.equal(expectedLocation || '');
            ...
            expect_1.expect(isLocal).to.equal(!!location);
        }
    }
    ...
    async function verifyInstallPaths(options) {
        ...
        expect_1.expect(paths).to.deep.equal(options.installInfo);
    }
    Install.verifyInstallPaths = verifyInstallPaths;
    async function verifyInstallConfirmation() {
        await open_close_helpers_1.waitUntilItemOpen(util_1.TEST_ID.installConfirmationDialog);
    }
    Install.verifyInstallConfirmation = verifyInstallConfirmation;
    async function verifyNoInstallConfirmation() {
        await open_close_helpers_1.waitUntilItemClosed(util_1.TEST_ID.installConfirmationDialog);
    }
    Install.verifyNoInstallConfirmation = verifyNoInstallConfirmation;
    async function verifyNext() {
        await open_close_helpers_1.waitUntilItemOpen(util_1.TEST_ID.installNextButton);
    }
    Install.verifyNext = verifyNext;
    async function verifyNoNext() {
        await open_close_helpers_1.waitUntilItemClosed(util_1.TEST_ID.installNextButton);
    }
...
})(Install = exports.Install || (exports.Install = {}));
