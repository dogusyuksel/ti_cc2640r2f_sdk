...
(function (PackageManager) {
    ...
    async function toggleAll() {
        await util_1.clickElement(protractor_1.element(protractor_1.by.id(util_2.TEST_ID.packageManagerShowInTreeAllCheckbox)));
    }
    PackageManager.toggleAll = toggleAll;
    async function toggleVersionCheckbox(packageUid, isLatest) {
        const item = await util_1.getItemById(await getVersionRowsCheckboxes(), util_2.TEST_ID.packageManagerShowInTreeCheckbox(packageUid, isLatest));
        await util_1.clickElement(item);
    }
    PackageManager.toggleVersionCheckbox = toggleVersionCheckbox;
    async function openActionsMenu(packageUid) {
        await open_close_helpers_1.openItem(util_2.TEST_ID.packageManagerActionsMenuButton(packageUid), util_2.TEST_ID.packageManagerActionsMenu);
    }
    PackageManager.openActionsMenu = openActionsMenu;
    async function closeActionsMenu(packageUid) {
        await open_close_helpers_1.closeItem(util_2.TEST_ID.packageManagerActionsMenuButton(packageUid), util_2.TEST_ID.packageManagerActionsMenu);
    }
    PackageManager.closeActionsMenu = closeActionsMenu;
    async function apply() {
        await util_1.clickElement(protractor_1.element(protractor_1.by.id(util_2.TEST_ID.packageManagerApplyButton)));
    }
    PackageManager.apply = apply;
    async function cancel() {
        await util_1.clickElement(protractor_1.element(protractor_1.by.id(util_2.TEST_ID.packageManagerCancelButton)));
    }
    PackageManager.cancel = cancel;
    async function openDetailedView(packageId) {
        await util_1.clickElement(protractor_1.element(protractor_1.by.id(util_2.TEST_ID.packageManagerMoreInfoButton(packageId))));
    }
    PackageManager.openDetailedView = openDetailedView;
    async function openSummaryView() {
        await util_1.clickElement(protractor_1.element(protractor_1.by.id(util_2.TEST_ID.packageManagerSummaryButton)));
    }
    ...
    async function verifyPackageSummaryRows(expectedPackages) {
        ...
        expect_1.expect(packageNames.length).to.equal(expectedPackages.length);
        await Promise.all(expectedPackages.map(async (expected, idx) => {
            const pkgName = packageNames[idx];
            expect_1.expect(pkgName).to.deep.equal(expected.expectedPkg.name);
        }));
    }
    ...
    async function verifyPackageDetailedRows(expectedPackages) {
        ...
        expect_1.expect(versions.length).to.equal(expectedPackages.length);
        ...
        {
            expect_1.expect(checkboxesState.length).to.equal(expectedPackages.filter(item => item.hasCheckbox).length);
            let i = 0;
        ...
        await Promise.all(expectedPackages.map(async (expected, idx) => {
            ...
            if (expected.isLatest != null) {
                if (expected.isLatest) {
                    expect_1.expect(version).to.contain('Latest');
                }
                else {
                    expect_1.expect(version).to.not.contain('Latest');
                }
            }
            expect_1.expect(checkboxState).to.equal(expected.isSelected);
            expect_1.expect(version).to.contain(expected.expectedPkg.packageVersion);
        }));
    }
    PackageManager.verifyPackageDetailedRows = verifyPackageDetailedRows;
    async function verifySelectAll(checked) {
        const checkbox = protractor_1.element(protractor_1.by.id(util_2.TEST_ID.packageManagerShowInTreeAllCheckbox));
        const isSelected = await checkbox.isSelected();
        expect_1.expect(isSelected).to.equal(checked);
    }
    PackageManager.verifySelectAll = verifySelectAll;
    async function verifyApply(expectedUrlQuery, apis) {
        await util_1.verifyUrl({ apis, urlQuery: expectedUrlQuery });
    }
