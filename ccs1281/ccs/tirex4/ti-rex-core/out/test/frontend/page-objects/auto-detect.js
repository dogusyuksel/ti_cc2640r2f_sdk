...
(function (AutoDetect) {
    ...
    async function clickButton() {
        await util_1.clickElement(protractor_1.element(protractor_1.by.id(util_2.TEST_ID.autoDetectButton)));
    }
    AutoDetect.clickButton = clickButton;
    async function closeDialog() {
        await open_close_helpers_1.closeItem(util_2.TEST_ID.autoDetectDialogClose, util_2.TEST_ID.autoDetectDialogContent);
    }
    AutoDetect.closeDialog = closeDialog;
    async function openDetectedDevicesMenu() {
        await open_close_helpers_1.openItem(util_2.TEST_ID.autoDetectButton, util_2.TEST_ID.autoDetectDetectedMenu);
    }
    ...
    async function selectDetectedDevice(id) {
        ...
        await util_1.clickElement(await util_1.getItemById(items, util_2.TEST_ID.autoDetectDetectedMenuItem(id)));
    }
    ...
    async function verifyButton(expectedText = auto_detect_1.BEGIN_TEXT, allowSubstring = false) {
        await open_close_helpers_1.waitUntilItemOpen(util_2.TEST_ID.autoDetectButton);
        // Make sure we resolve all cloud agent communication before testing
        await browser_scripts_1.waitForPromisesToResolve(await browser_scripts_1.getActivePromiseIndicies());
        const text = await protractor_1.element(protractor_1.by.id(util_2.TEST_ID.autoDetectButton)).getText();
    ...
    async function verifyDialogOpen(expectedText, allowSubstring = false) {
        await open_close_helpers_1.waitUntilItemOpen(util_2.TEST_ID.autoDetectDialogContent);
        const text = await protractor_1.element(protractor_1.by.id(util_2.TEST_ID.autoDetectDialogContent)).getText();
        compareText(text, expectedText, allowSubstring);
    }
    AutoDetect.verifyDialogOpen = verifyDialogOpen;
    async function verifyDialogClosed() {
        await open_close_helpers_1.waitUntilItemClosed(util_2.TEST_ID.autoDetectDialogContent);
    }
    ...
    async function verifyDetectedDevices(numDevices = initialFilteredDevices) {
        ...
        expect_1.expect(listItems).to.have.lengthOf(numDevices);
    ...
    function compareText(text1, text2, allowSubstring) {
        ...
        if (allowSubstring) {
            expect_1.expect(text1).to.contain(text2);
        }
        else {
            expect_1.expect(text1).to.deep.equal(text2);
        }
    }
})(AutoDetect = exports.AutoDetect || (exports.AutoDetect = {}));
