...
async function goToNode(node, apis) {
    ...
    await updateBrowserUrl({ apis, node, urlQuery: routing_helpers_1.getUrlQuery(queryString) });
}
...
async function updateBrowserUrl({ apis, urlQuery, node, page }) {
    const link = node
        ? await util_1.getNodeLinkForTest(node, urlQuery, apis, page)
        : util_1.getLinkForTest(urlQuery, page);
    await waitForPromisesAfterActionToResolve(() => browser_scripts_1.updateUrlForTesting(link));
}
exports.updateBrowserUrl = updateBrowserUrl;
async function waitForPromisesAfterActionToResolve(action) {
    const prevIndicies = await browser_scripts_1.getActivePromiseIndicies();
    await action();
    const indicies = await browser_scripts_1.getActivePromiseIndicies();
    await browser_scripts_1.waitForPromisesToResolve(_.difference(indicies, prevIndicies));
}
...
async function verifyUrl({ apis, urlQuery, node, page = page_1.Page.EXPLORE }) {
    ...
    expect_1.expect(browserUrl.startsWith(prefix)).to.be.true;
    if (node) {
        const link = await util_1.getNodeLinkForTest(node, urlQuery, apis, page);
        expect_1.expect(browserUrl.endsWith(link)).to.be.true;
    }
    else {
