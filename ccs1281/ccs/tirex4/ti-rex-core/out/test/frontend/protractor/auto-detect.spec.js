...
describe('[frontend] AutoDetect', function () {
    ...
    it('Should update the selection when an auto-detected device is clicked', async function () {
        expect_1.expect(data1.filterData.devices.length).to.be.greaterThan(0);
        const device = data1.filterData.devices[0];
        await setup({});
        await auto_detect_1.AutoDetect.selectDetectedDevice(device.publicId);
        await util_2.verifyUrl({ apis, urlQuery: { devices: [device.publicId] } });
    });
    ...
    it('Should resolve with the files needed if files must be downloaded', async function () {
        ...
        await protractor_1.element(protractor_1.by.id(util_3.TEST_ID.autoDetectButton)).click();
        ...
        await expect_1.expect(protractor_1.element(protractor_1.by.id(util_3.TEST_ID.autoDetectDownloadProgressBar)).isPresent()).to
            .eventually.be.true;
