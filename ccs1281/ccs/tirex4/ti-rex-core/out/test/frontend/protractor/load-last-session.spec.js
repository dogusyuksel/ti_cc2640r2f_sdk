...
describe('[frontend] RestoreLastSession', function () {
    ...
    afterEach(async function () {
        ...
        await util_1.logBrowserConsole(testName);
        await util_1.cleanupProtractorTest();
    });
    const data1 = {
        ...
        nodeData: util_3.objectFromKeyValuePairs([packageNode2, packageNode3].map(node => ({ key: node.nodeDbId, value: node })))
    };
    ...
    it("Should not show load last session if we are set to automatically load last session & we don't have a url query", async function () {
        ...
        await util_1.setupProtractorTest({ data: data1, apis, clearLocalStorage: false });
    ...
    it('Should not show load last session if we are set to not automatically load last session & we have a url query', async function () {
        ...
        await util_1.setupProtractorTest({
            data: data1,
            apis,
    ...
    it("Should not show load last session if we didn't have a url query in the last session", async function () {
        ...
        await util_1.setupProtractorTest({ data: data1, apis });
        await verifyLoadLastSession(false);
    });
    it('Should not show load last session once we add a url query (we start without one)', async function () {
        ...
        await util_2.updateBrowserUrl({ apis, urlQuery: { kernels: [kernel2.publicId] } });
        await verifyLoadLastSession(false);
    });
    ...
    it('Should be able to load last session', async function () {
        ...
        await util_2.verifyUrl({ apis, urlQuery: lastSessionUrlQuery });
    });
    ...
    it('Should not load the last session if we open with an initial url query', async function () {
        ...
        await util_2.verifyUrl({ apis, urlQuery: currentSessionUrlQuery });
    });
});
async function createLastSession({ data, apis, urlQuery, duringSession }) {
    await util_1.setupProtractorTest({ data, apis, urlQuery });
    if (duringSession) {
        await duringSession();
    }
    await util_1.cleanupProtractorTest();
...
async function loadLastSession() {
    ...
    await util_2.clickElement(protractor_1.element(protractor_1.by.id(util_4.TEST_ID.navbarMenuLoadLastSession)));
    await protractor_1.browser.wait(protractor_1.ExpectedConditions.not(protractor_1.ExpectedConditions.visibilityOf(protractor_1.element(protractor_1.by.id(util_4.TEST_ID.navbarMenu)))), 500, 'Menu did not close');
}
async function toggleAutoLoadLastSession() {
    await navbar_1.Navbar.openMenu();
    await util_2.clickElement(protractor_1.element(protractor_1.by.id(util_4.TEST_ID.navbarMenuAutoLoadLastSession)));
    await navbar_1.Navbar.closeMenu();
}
// Verify
async function verifyLoadLastSession(expectedIsPresent) {
    await navbar_1.Navbar.openMenu();
    await expect_1.expect(protractor_1.element(protractor_1.by.id(util_4.TEST_ID.navbarMenuLoadLastSession)).isPresent()).to.eventually.equal(expectedIsPresent);
    await navbar_1.Navbar.closeMenu();
}
