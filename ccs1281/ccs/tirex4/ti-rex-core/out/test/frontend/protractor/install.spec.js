...
describe('[frontend] Install - REX-1465#1, REX-2604#1, REX-2750#1', function () {
    ...
    describe('Licenses', function () {
        ...
        it('Should proceed if user agrees', async function () {
            ...
            expect_1.expect(spies.installPackage.callCount).to.be.greaterThan(0);
        });
        it('Should not proceed if user disagrees', async function () {
            ...
            // Verify call
            const spies = await browser_scripts_1.getRexCloudAgentModuleSpies();
            expect_1.expect(spies.installPackage.callCount).to.equal(0);
        });
    });
    describe('Install request', function () {
        ...
        async function verifyCall(expectedPackagePublicUids, expectedInstallLocation) {
            ...
            expect_1.expect(args.length).to.equal(expectedPackagePublicUids.length);
            const packagePublicUids = args.map(item => {
                expect_1.expect(item.length).to.equal(2);
                if (expectedInstallLocation) {
                    expect_1.expect(item[1]).to.deep.equal(expectedInstallLocation);
                }
                return item[0].packagePublicUid;
            });
            expect_1.expect(packagePublicUids.sort()).to.deep.equal(expectedPackagePublicUids.sort());
        }
        async function verifyNotCalled(packagePublicUid) {
            ...
            expect_1.expect(installCall).to.be.false;
        }
    });
    async function setup(options) {
        await util_1.lightSetupProtractorTest({
            ...
            additionalSetup: () => browser_scripts_1.installCloudAgent(options)
        });
    }
});
