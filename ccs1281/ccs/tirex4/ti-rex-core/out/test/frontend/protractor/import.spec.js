...
describe('[frontend] Import - REX-2607#1', function () {
        ...
        describe('ImportCloud', function () {
            describe('Import', function () {
                ...
                it('Should do a basic import successfully (no target id)', async function () {
                    await util_2.waitForPromisesAfterActionToResolve(async () => {
            ...
            async function verifyAndCleanupCloudImport() {
                ...
                try {
                    expect_1.expect(handles.length).to.equal(2);
            ...
            async function verifyNoCloudImport() {
                const handles = await protractor_1.browser.getAllWindowHandles();
                await protractor_1.browser.switchTo().window(handles[0]);
                expect_1.expect(handles.length).to.equal(1);
            }
        });
    }
    describe('ImportDesktop', function () {
        ...
        async function verifyCall(node, targetIdExpected, idx = 0, numCalls = 1) {
            const spies = await browser_scripts_1.getRexCloudAgentModuleSpies();
            const args = spies.importProject.args;
            expect_1.expect(args.length).to.equal(numCalls);
            const params = args[idx];
            expect_1.expect(params.length).to.equal(4);
            ...
            expect_1.expect(resourceType).to.deep.equal('project.ccs');
            expect_1.expect(packageUid).to.deep.equal(node.packagePublicUid);
            expect_1.expect(location).to.deep.equal(`${node.nodeDbId}/some/path`);
            expect_1.expect(targetId).to.deep.equal(targetIdExpected);
        }
        async function verifyNotCalled() {
            const spies = await browser_scripts_1.getRexCloudAgentModuleSpies();
            const args = spies.importProject.args;
            expect_1.expect(args.length).to.equal(0);
        }
        async function beforeEachHelper() {
            await util_1.lightSetupProtractorTest({
                apis,
                additionalSetup: () => browser_scripts_1.installCloudAgent(displayTestOptions)
            });
        }
        async function afterEachHelper() {
            await browser_scripts_1.uninstallCloudAgent();
        }
    });
});
