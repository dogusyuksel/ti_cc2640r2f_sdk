...
describe('[frontend] Filter - REX-1063#1, REX-2749#1', function () {
    ...
    it('Should display device when device suggestion is clicked', async function () {
        await util_1.lightSetupProtractorTest({ apis, urlQuery: {} });
        ...
        expect_1.expect(selected).to.equal(device1.name);
    });
    ...
    it('Should update URL when device suggestion is clicked', async function () {
        ...
        await filter_1.BoardDeviceInput.click();
        await filter_1.BoardDeviceInput.clickSuggestion(`Device-${device1.publicId}`);
        await util_2.verifyUrl({ apis, urlQuery: { devices: [device1.publicId] } });
    });
    it('Should update URL when keyword suggestion is clicked', async function () {
        await util_1.lightSetupProtractorTest({ apis, urlQuery: {} });
        await filter_1.KeywordInput.enterText(device1.name);
        await filter_1.KeywordInput.clickSuggestion(device1.name);
        await util_2.verifyUrl({ apis, urlQuery: { search: device1.name } });
    });
    radioButtonTestData.forEach(radioButtonTest => {
        it(`Should update URL when ${radioButtonTest.queryString.toLowerCase()} radio button is clicked`, async function () {
            await util_1.lightSetupProtractorTest({ apis, urlQuery: {} });
            await util_2.waitForPromisesAfterActionToResolve(() => util_2.clickElement(protractor_1.element(protractor_1.by.id(util_3.TEST_ID.filterRadioButton(radioButtonTest.queryString, radioButtonTest.publicId)))));
            await util_2.verifyUrl({
                apis,
    ...
    it('Should display device from URL', async function () {
        await util_1.lightSetupProtractorTest({
            apis,
            node: packageNode2,
    ...
    it('Should update non-default URL when keyword suggestion is clicked', async function () {
        ...
        await util_2.verifyUrl({ apis, urlQuery: { kernels: [kernel1.publicId], search: device1.name } });
    });
    radioButtonTestData.forEach(radioButtonTest => {
        it(`Should update non-default URL when ${radioButtonTest.queryString.toLowerCase()} radio button is clicked`, async function () {
            ...
            await filter_1.Filter.openAllFiltersPopup();
            const prevIndcies = await browser_scripts_1.getActivePromiseIndicies();
            await util_2.clickElement(protractor_1.element(protractor_1.by.id(util_3.TEST_ID.filterRadioButton(radioButtonTest.queryString, radioButtonTest.publicId))));
            const indcies = await browser_scripts_1.getActivePromiseIndicies();
            await browser_scripts_1.waitForPromisesToResolve(_.difference(indcies, prevIndcies));
            await util_2.verifyUrl({
                apis,
                urlQuery: {
