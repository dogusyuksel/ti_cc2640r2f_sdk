...
const data1 = {
    ...
    nodeData: util_3.objectFromKeyValuePairs([packageNode2, packageNode4, leafNodeLiveAction, softwareFolder, folderNode2].map(node => ({
        key: node.nodeDbId,
        value: node
    })))
};
...
describe('[frontend] LiveActions - REX-2608#1', function () {
    ...
    before(async function () {
        ...
        await util_1.setupProtractorTest({ data: data1, apis, urlQuery: {} });
    });
    ...
    describe('jumpToNode', function () {
        ...
        it('Should jump to node in the same package', async function () {
            await util_2.goToNode(leafNodeLiveAction, apis);
            await clickInIframe(liveActionJumpToNodeIds.SAME_PACKAGE);
            await util_2.verifyUrl({ apis, urlQuery: {}, node: packageNode4 });
        });
        it('Should jump to node with a filter set', async function () {
            ...
            await util_2.updateBrowserUrl({ apis, urlQuery });
            await util_2.goToNode(leafNodeLiveAction, apis);
            await clickInIframe(liveActionJumpToNodeIds.SAME_PACKAGE);
            await util_2.verifyUrl({ apis, urlQuery, node: packageNode4 });
        });
        ...
        it('Should handle an invalid node', async function () {
            ...
            expect_1.expect(text).to.contain('This resource does not exist');
        });
    ...
    async function clickInIframe(linkId) {
        await protractor_1.browser.switchTo().frame(protractor_1.element(protractor_1.by.id(util_4.TEST_ID.iframeElement)).getWebElement());
        await protractor_1.browser.executeScript('arguments[0].click();', protractor_1.element(protractor_1.by.id(linkId)));
        await protractor_1.browser.switchTo().defaultContent();
    }
    async function getMessageText() {
        const message = protractor_1.element(protractor_1.by.id(util_4.TEST_ID.messageText));
