...
async function setupEnzymeTest({ data }) {
    ...
    // Other setup
    await fake_server_helpers_1.setupFakeServer(data, '');
    promise_syncronization_helpers_1.setupPromiseSyncronization();
}
exports.setupEnzymeTest = setupEnzymeTest;
function cleanupEnzymeTest() {
    fake_server_helpers_1.cleanupFakeServer();
    promise_syncronization_helpers_1.cleanupPromiseSyncronization();
}
...
async function lightSetupProtractorTest({ apis, node, urlQuery, additionalSetup, page, customUrl }) {
    await browser_scripts_1.clearCaches();
    await util_3.updateBrowserUrl({ apis, urlQuery: {}, page: page_1.Page.BLANK });
    /*
    ...
    if (customUrl) {
        await browser_scripts_1.updateUrlForTesting(customUrl);
    }
    else {
        await util_3.updateBrowserUrl({
            apis,
            urlQuery: urlQuery || {},
            node,
            page
...
async function cleanupProtractorTest() {
    fake_server_helpers_1.cleanupFakeServer();
    await browser_scripts_1.cleanupPromiseSyncronization();
}
...
async function getNodeLinkForTest(nodeInput, urlQuery, apis, page = page_1.Page.EXPLORE) {
    ...
    const link = routing_helpers_1.getNodeLinkFromNode(node, await create_app_props_1.createAppProps({
        page,
        urlQuery
    }), true);
    return link;
...
async function setupProtractorTestInner(data, clearLocalStorage, initialPage = '', additionalSetup) {
    ...
    protractor_1.browser.wait(protractor_1.ExpectedConditions.presenceOf(protractor_1.element(protractor_1.by.id(util_2.TEST_ID.testLandingPageButton))), 10000, "Landing page button didn't show up"); // Button will be loaded once page is ready
    // Hook in our promise replacement
    await browser_scripts_1.setupPromiseSyncronization();
    // Setup the fake server
    await browser_scripts_1.setupFakeServer(data, initialPage);
    const indicies = await browser_scripts_1.getActivePromiseIndicies();
    await browser_scripts_1.waitForPromisesToResolve(indicies);
    // Clear cookies
    ...
    await protractor_1.element(protractor_1.by.id(util_2.TEST_ID.testLandingPageButton)).click();
    const indicies2 = await browser_scripts_1.getActivePromiseIndicies();
    await browser_scripts_1.waitForPromisesToResolve(indicies2);
}
