    ...
    [
        ...
        it(`Simulate ${description} load (${numUsers} users) on ${test_helpers_1.testingGlobals.remoteserverUrl}`, async function () {
            ...
            try {
                ...
                expect_1.expect(exceptions).to.be.empty;
            }
            finally {
...
async function getRootNode() {
    const response = await expect_1.chai.request(test_helpers_1.testingGlobals.remoteserverUrl).get("api/rootNode" /* GET_ROOT_NODE */);
...
async function getInitialData() {
    try {
        ...
        await delay_1.delay(2 * 60 /* seconds in a minute */ * 1000 /* ms in a second */);
        throw e;
    }
}
async function setup(description) {
    ...
    await ajaxHarness.setupTestProxyServer(test_helpers_1.testingGlobals.remoteserverUrl, util_1.createLoggingResponseHandler(logStream));
    // Setup statistics
