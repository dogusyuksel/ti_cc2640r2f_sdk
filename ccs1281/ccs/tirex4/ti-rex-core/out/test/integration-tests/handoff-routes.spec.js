...
const { vars } = rex_1._getRex();
const handoffManager = handoff_1.getHandoffManagerForTesting();
const refreshManager = handoffManager.getRefreshManager();
const sendEmail = util_1.promisify(helpersNode.sendEmail);
////////////////////////////////////////////////////////////////////////////////
...
describe('[handoff] Handoff routes - REX-1874#1', function () {
    ...
    afterEach(async function () {
        ...
        const apiResult = await expect_1.chai.request(app).get(`${"api/get-maintenance-mode" /* GET_GET_MAINTENACE_MODE */}`);
        expect_1.expect(apiResult.text).to.deep.equal('off');
    });
    describe(`${"api/add-package" /* POST_ADD_PACKAGE */} (no backup / package to be replaced)`, function () {
        ...
        it('Should be able to add a package to the server', async function () {
            await AddPackageValidation.testResultDuringProcessing("Success" /* SUCCESS */, false);
        });
        it('Should handle a bulk request', async function () {
            // Setup
            setupRefreshStubFn({ result: "Success" /* SUCCESS */ });
            ...
            expect_1.expect(err).to.be.null;
            expect_1.expect(refreshStub.calledOnce).to.be.true;
            // Verify the expected zips and package folders exist
            await test_helpers_2.verifyContentAndZipsPresent({
            ...
            });
            {
                ...
                expect_1.expect(packages).to.have.length(2);
                packages.map(entry => {
                    expect_1.expect(entry).to.deep.equal({
                        ...entry,
                        state: "valid" /* VALID */,
                        submissionId
                    });
        ...
        it('Should handle a non tirex package (as a secondary package in an installer)', async function () {
            ...
            });
            {
                ...
                expect_1.expect(packages).to.have.length(4);
        ...
        it('Should handle a graceful error before package processing occurs', async function () {
            ...
            const addPackagePromise = getFirstCall(addPackageSpy);
            // Validate
            await expect_1.expect(addPackagePromise).to.be.rejectedWith(errors_1.GracefulError);
            {
                ...
                const expectedContent = expectedContent2;
                {
                    ...
                    await test_helpers_2.verifyContentAndZipsGone({
                        contentFolder: vars.contentBasePath,
                ...
                }
                {
                    // Check the package manager file
                    const { packages } = (await fs.readJson(vars.packageManagerFile));
                    expect_1.expect(packages).to.have.length(0);
                }
                expect_1.expect(refreshStub.called).to.be.false;
            }
        });
        it('Should handle a graceful error during package processing', async function () {
            await AddPackageValidation.testResultDuringProcessing("GracefulError" /* GRACEFUL_ERROR */, false);
        });
        ...
        it('Should handle a fatal error before package processing occurs', async function () {
            ...
            expect_1.expect(err).to.be.not.null;
            expect_1.expect(err instanceof errors_1.GracefulError).to.be.false;
            expect_1.expect(refreshStub.called).to.be.false;
        });
        it('Should handle a fatal error during package processing', async function () {
            await AddPackageValidation.testResultDuringProcessing("FatalError" /* FATAL_ERROR */, false);
        });
        it('Should handle a fatal error after package processing', async function () {
            ...
            try {
                ...
                expect_1.expect(err).to.be.not.null;
                expect_1.expect(err instanceof errors_1.GracefulError).to.be.false;
                expect_1.expect(refreshStub.called).to.be.true;
            }
            catch (e) {
                err = err || e;
            }
    ...
    describe(`${"api/add-package" /* POST_ADD_PACKAGE */} (with a backup / package to be replaced)`, function () {
        beforeEach(beforeEachHandoffRouteTest);
        afterEach(afterEachHandoffRouteTest);
        it('Should be able to add a package to the server', async function () {
            await AddPackageValidation.testResultDuringProcessing("Success" /* SUCCESS */, false);
            resetStubsAndSpies();
            await AddPackageValidation.testResultDuringProcessing("Success" /* SUCCESS */, true);
        });
        it('Should handle a graceful error before package processing occurs', async function () {
            // Add the original
            await AddPackageValidation.testResultDuringProcessing("Success" /* SUCCESS */, false);
            ...
            // Validate - behaves the same as a graceful error during processing
            {
                ...
                expect_1.expect(err).to.be.not.null;
                expect_1.expect(err instanceof errors_1.GracefulError).to.be.true;
            }
            await AddPackageValidation.postValidateAddPackageResultDuringProcessing({
                result: "GracefulError" /* GRACEFUL_ERROR */,
                submissionId,
        ...
        it('Should handle a graceful error during package processing', async function () {
            ...
            await AddPackageValidation.testResultDuringProcessing("Success" /* SUCCESS */, false);
            resetStubsAndSpies();
            await AddPackageValidation.testResultDuringProcessing("GracefulError" /* GRACEFUL_ERROR */, true);
        });
        it.skip('TODO Should handle a graceful error after package processing', async function () {
            // Currently no use case for this
        });
        ...
        it('Should handle a fatal error during package processing', async function () {
            ...
            await AddPackageValidation.testResultDuringProcessing("Success" /* SUCCESS */, false);
            resetStubsAndSpies();
            await AddPackageValidation.testResultDuringProcessing("FatalError" /* FATAL_ERROR */, true);
        });
        it('Should handle a fatal error after package processing', async function () {
            // Add the original
            await AddPackageValidation.testResultDuringProcessing("Success" /* SUCCESS */, false);
    ...
    describe(`${"api/add-package" /* POST_ADD_PACKAGE */} overridesFiles and showEntry handling`, function () {
        beforeEach(beforeEachHandoffRouteTest);
        afterEach(afterEachHandoffRouteTest);
            ...
            expect_1.expect(refreshStub.calledOnce).to.be.true;
            {
                ...
                await Promise.all(packages.map(async (entry) => {
                    await Promise.all(entry.content.map(async (item) => {
                        ...
                        expect_1.expect(overridesExists).to.equal(!entry.showEntry);
                    }));
                }));
            }
        });
        it('Should revert the overrides files to the previous showEntry states, and call refresh correctly - if we had a gracefull error during processing', async function () {
            ...
            const items = [
                {
                    ...
                    asset: asset11,
                    handoffResult: ["GracefulError" /* GRACEFUL_ERROR */, "Success" /* SUCCESS */],
            ...
            await promise_utils_1.mapSerially([...itemsStart, ...items], async (item, idx) => {
                ...
                try {
                    ...
                    if (handoffResult === "Success" /* SUCCESS */) {
                        throw e;
                    }
                }
                if (idx >= itemsStart.length) {
                    // Verify;
                    expect_1.expect(refreshStub.args.length).to.equal(2);
                    const refreshItems = refreshStub.args[1][0].filter(item => item.operation !== "doNothing" /* DO_NOTHING */);
                    expect_1.expect(refreshItems.length).to.equal(1);
                    ...
                    if (entry.content === expectedContent1[0]) {
                        ...
                        // installPath)
                        expect_1.expect(entry.operation).to.equal("replacePackage" /* REPLACE_PACKAGE */);
                        expect_1.expect(entry.installPath.linux).to.deep.equal(expectedZips1[0]);
                        ...
                        expect_1.expect(overridesExists).to.be.false;
                    }
                    else if (entry.content === expectedContent3[0]) {
                        // Verify refresh called correctly (add / remove is correct and
                        // content / zips is correct - important for installCommand and
                        // installPath)
                        expect_1.expect(entry.operation).to.equal("removePackage" /* REMOVE_PACKAGE */);
                        expect_1.expect(entry.installPath.linux).to.deep.equal(expectedZips3[0]);
                        // Verify overrides file state
                        ...
                        // installPath)
                        expect_1.expect(entry.operation).to.equal("removePackage" /* REMOVE_PACKAGE */);
                        expect_1.expect(entry.installPath.linux).to.deep.equal(expectedZips4[0]);
                        // Verify overrides file state
                        ...
                        expect_1.expect(overridesExists).to.be.true;
                    }
                    else {
                ...
                // Cleanup
                resetStubsAndSpies();
                await expect_1.chai.request(app).get(`${"api/maintenance-mode" /* GET_MAINTENANCE_MODE */}?switch=off`);
            });
        });
    });
    describe(`${"api/remove-package" /* DELETE_REMOVE_PACKAGE */}`, function () {
        beforeEach(beforeEachHandoffRouteTest);
        afterEach(afterEachHandoffRouteTest);
        it('Should be able to delete a package from the server', async function () {
            await RemovePackageValidation.testResultDuringProcessing("Success" /* SUCCESS */);
        });
        it('Should handle a graceful error before package processing occurs', async function () {
            let err = null;
            ...
            try {
                ...
                const result = await expect_1.chai
                    .request(app)
                    .del(`${"api/remove-package" /* DELETE_REMOVE_PACKAGE */}?id=com.ti.WATSON_CC32XX&version=1.10.00.04`);
                expect_1.expect(result.status).to.equal(200);
                const { submissionId } = result.body;
        ...
        it('Should handle a graceful error during package processing', async function () {
            await RemovePackageValidation.testResultDuringProcessing("GracefulError" /* GRACEFUL_ERROR */);
        });
        ...
        it('Should handle a fatal error during package processing', async function () {
            await RemovePackageValidation.testResultDuringProcessing("FatalError" /* FATAL_ERROR */);
        });
        it('Should handle a fatal error after package processing', async function () {
            {
                // Add the package to the server
            ...
            try {
                ...
                const result = await expect_1.chai
                    .request(app)
                    .del(`${"api/remove-package" /* DELETE_REMOVE_PACKAGE */}?id=com.ti.WATSON_CC32XX&version=1.10.00.04&email=fake@ti.com`);
                expect_1.expect(result.status).to.equal(200);
                // Get the promise to tell us when the delete is done
                const removePackagePromise = getFirstCall(removePackageSpy);
                // Validate
        ...
        it('Should handle missing fields in the request', async function () {
            ...
            const resultNoVersion = await expect_1.chai
                .request(app)
                .del(`${"api/remove-package" /* DELETE_REMOVE_PACKAGE */}?id=foo&email=bar`);
            expect_1.expect(resultNoVersion.status).to.equal(400);
            // Missing id
            const resultNoId = await expect_1.chai
                .request(app)
                .del(`${"api/remove-package" /* DELETE_REMOVE_PACKAGE */}?version=foo&email=bar`);
            expect_1.expect(resultNoId.status).to.equal(400);
        });
        ...
        it('Should revert the overides file to the previous show entry state if we had an error during gracefull error processing (showEntry = false)', async function () {
            ...
            // Setup
            setupRefreshStubFn({ result: "GracefulError" /* GRACEFUL_ERROR */ });
            const result = await expect_1.chai
                .request(app)
                .del(`${"api/remove-package" /* DELETE_REMOVE_PACKAGE */}?id=com.ti.WATSON_CC32XX&version=1.10.00.04`);
            expect_1.expect(result.status).to.equal(200);
            ...
            expect_1.expect(err).to.be.not.null;
            expect_1.expect(err instanceof errors_1.GracefulError, JSON.stringify(err)).to.be.true;
    ...
    describe(`${"api/remove-package" /* DELETE_REMOVE_PACKAGE */} overridesFiles and showEntry handling`, function () {
        beforeEach(beforeEachHandoffRouteTest);
        afterEach(afterEachHandoffRouteTest);
    });
    describe(`${"api/sync-packages" /* GET_SYNC_PACKAGES */}`, function () {
        beforeEach(beforeEachHandoffRouteTest);
        afterEach(afterEachHandoffRouteTest);
        it('Should import an untracked package', async function () {
            ...
            await test_helpers_2.prepareContentAndZips({
                contentFolder: vars.contentBasePath,
                zipsFolder: vars.zipsFolder,
                zips: item.zips,
            ...
            const result = await expect_1.chai.request(app).get(`${"api/sync-packages" /* GET_SYNC_PACKAGES */}`);
            expect_1.expect(result.status).to.equal(200);
            // Check the package manager file
            const { packages } = (await fs.readJson(vars.packageManagerFile));
            expect_1.expect(packages).to.have.length(1);
            expect_1.expect(packages[0]).to.deep.equal({
                ...test_helpers_2.createPackageEntryValid(item),
                submissionId: ''
            });
        });
        it('Should pickup the show entry value from the fs', async function () {
            ...
            // Check the package manager file
            const { packages } = (await fs.readJson(vars.packageManagerFile));
            expect_1.expect(packages).to.have.length(2); // OverridesDir gets treated like a package (so length is 2)
            expect_1.expect(packages[0]).to.deep.equal({
                ...test_helpers_2.createPackageEntryValid(item),
                submissionId: '',
                showEntry: false
    ...
    describe(`${"api/cleanup-packages" /* GET_CLEANUP_PACKAGES */}`, function () {
        this.timeout(60000);
        beforeEach(beforeEachHandoffRouteTest);
        afterEach(afterEachHandoffRouteTest);
        it('Should be able to cleanup staged packages', async function () {
            // Add the items
            const items = [
                { asset: asset1, handoffResult: "Success" /* SUCCESS */, platform: util_3.Platform.ALL },
                { asset: asset1, handoffResult: "FatalError" /* FATAL_ERROR */, platform: util_3.Platform.ALL },
                { asset: asset3, handoffResult: "Success" /* SUCCESS */, platform: util_3.Platform.ALL },
                {
                    asset: asset4,
                    handoffResult: "FatalError" /* FATAL_ERROR */,
                    platform: util_3.Platform.LINUX
                }
            ];
            for (const item of items) {
                ...
                const addPackagePromise = getFirstCall(addPackageSpy);
                if (handoffResult === "Success" /* SUCCESS */) {
                    await expect_1.expect(addPackagePromise).to.be.not.rejected;
                }
                else {
                    await expect_1.expect(addPackagePromise).to.be.rejected;
                }
                resetStubsAndSpies();
                await expect_1.chai.request(app).get(`${"api/maintenance-mode" /* GET_MAINTENANCE_MODE */}?switch=off`);
            }
            // Call cleanup
            setupRefreshStubFn({ result: "Success" /* SUCCESS */ });
            await expect_1.expect(expect_1.chai.request(app).get("api/cleanup-packages" /* GET_CLEANUP_PACKAGES */)).to.be.not.rejected;
            ...
            expect_1.expect(packages).to.have.length(2);
            packages.map(entry => {
                expect_1.expect(entry.state).to.equal("valid" /* VALID */);
            });
        });
        it('Should handle processing failing during cleanup', async function () {
            ...
            // Call cleanup
            setupRefreshStubFn({ result: "FatalError" /* FATAL_ERROR */ });
            const response = await expect_1.expect(expect_1.chai.request(app).get("api/cleanup-packages" /* GET_CLEANUP_PACKAGES */)).to.be.not
                .rejected;
            expect_1.expect(response.status).to.not.equal(200);
            // Get the promise to tell us when cleanup is done
            const cleanupPromise = getFirstCall(cleanupStagedPackagesSpy);
            await expect_1.expect(cleanupPromise).to.be.rejected;
            // Check the package manager file
            const { packages } = (await fs.readJson(vars.packageManagerFile));
            expect_1.expect(packages).to.have.length(2);
            const stagedEntries = packages.filter(entry => entry.state === "staged" /* STAGED */);
            expect_1.expect(stagedEntries).to.have.length(1);
            const validEntries = packages.filter(entry => entry.state === "valid" /* VALID */);
            expect_1.expect(validEntries).to.have.length(1);
        });
        it('Should revert the overrides files to the previous show entry states, and call refresh correctly', async function () {
            ...
            for (const item of [...itemsStart, ...items]) {
                ...
                if (handoffResult === "Success" /* SUCCESS */) {
                    ...
                    await expect_1.expect(addPackagePromise).to.be.eventually.rejected;
                }
                resetStubsAndSpies();
                await expect_1.chai.request(app).get(`${"api/maintenance-mode" /* GET_MAINTENANCE_MODE */}?switch=off`);
            }
    ...
    describe('Maintenance mode', function () {
        ...
        it('Should be able to get out of maintenance mode (resume service)', async function () {
            {
                const result = await turnOnMaintenanceMode();
                expect_1.expect(result.status).to.deep.equal(200);
            }
            {
                const result = await expect_1.chai
                    .request(app)
                    .get(`${"api/maintenance-mode" /* GET_MAINTENANCE_MODE */}?switch=off`);
                expect_1.expect(result.status).to.deep.equal(200);
            }
            await AddPackageValidation.testResultDuringProcessing("Success" /* SUCCESS */, false);
        });
...
(function (AddPackageValidation) {
    async function testResultDuringProcessing(handoffResultToInject, replace) {
        // Setup
        if (handoffResultToInject === "GracefulError" /* GRACEFUL_ERROR */) {
            // Make rollback refresh pass
            // TODO should test rollback refresh failure
            const results = [handoffResultToInject, "Success" /* SUCCESS */];
            let idx = 0;
            refreshStub.callsFake(async (...params) => {
                return refreshStubFn(params, {
                    result: results[idx++]
        ...
        switch (handoffResultToInject) {
            case "Success" /* SUCCESS */:
                expect_1.expect(err).to.be.null;
                break;
            case "GracefulError" /* GRACEFUL_ERROR */:
                expect_1.expect(err).to.be.not.null;
                expect_1.expect(err instanceof errors_1.GracefulError, JSON.stringify(err)).to.be.true;
                break;
            case "FatalError" /* FATAL_ERROR */:
                expect_1.expect(err).to.be.not.null;
                expect_1.expect(err instanceof errors_1.GracefulError, JSON.stringify(err)).to.be.false;
                break;
        }
        ...
        switch (result) {
            ...
            case "GracefulError" /* GRACEFUL_ERROR */:
                ...
                if (replace) {
                    await verifyNothingExtraAddedOrRemoved({ add: expectedContentTopLevelItems, remove: [] }, { add: expectedZipsTopLevelItems, remove: [] });
                    {
                        ...
                        expect_1.expect(entry.submissionId).to.not.equal(submissionId);
                    }
                }
                else {
                    // Verify nothing additional was added to the content and zips folder
            ...
            case "FatalError" /* FATAL_ERROR */:
                ...
                }
                {
                    ...
                    if (replace && entry.state === "staged" /* STAGED */) {
...
async function addZipPackages(items, expectedStatus = 200) {
    const resultPost = await expect_1.chai
        .request(app)
        .post("api/add-package" /* POST_ADD_PACKAGE */)
        .type('form')
        .send({
    ...
    expect_1.expect(resultPost.status).to.equal(expectedStatus);
    return resultPost;
}
...
async function afterEachHandoffRouteTest() {
    ...
    await util_3.ignoreError(() => fs.remove(vars.overridesDir));
    await util_3.ignoreError(() => fs.remove(vars.packageManagerFile));
    await expect_1.chai.request(app).get(`${"api/maintenance-mode" /* GET_MAINTENANCE_MODE */}?switch=off`);
}
...
async function refreshStubFn(params, config) {
    const [rpl] = params;
    if (config.result === "FatalError" /* FATAL_ERROR */) {
        throw new Error('Fatal refresh error');
    }
    return {
        result: [...rpl],
        success: config.result === "Success" /* SUCCESS */
    };
}
...
async function turnOnMaintenanceMode() {
    const getMaintenanceMode = () => expect_1.chai.request(app).get(`${"api/maintenance-mode" /* GET_MAINTENANCE_MODE */}?switch=on`);
...
async function verifyNothingExtraAddedOrRemoved(expectedContentTopLevelItems, expectedZipsTopLevelItems) {
    ...
    expect_1.expect(expectedContentFolder).to.deep.equal(contentItems.sort());
    expect_1.expect(expectedZipFolder).to.deep.equal(zipItems.sort());
}
