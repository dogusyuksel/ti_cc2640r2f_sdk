...
describe('DB Package/Package Group APIs', () => {
    ...
    before(async () => {
        ...
        await nodes_utils_1.updateDatabase(metadata);
        packagesMetadata = metadata.packages;
    });
    describe("api/packages" /* GET_PACKAGES */, () => {
        let packages;
        before(async () => {
            packages = await makeApiRequest("api/packages" /* GET_PACKAGES */);
        });
        it('should have a valid entry for each package', async () => {
            expect_1.expect(packages.length).to.equal(packagesMetadata.length);
            for (const pkg of packagesMetadata) {
                ...
                expect_1.expect(packageData).to.exist;
                expect_1.expect(packageData.packagePublicId).to.equal(packageOverview.packageId);
                expect_1.expect(packageData.packagePublicUid).to.equal(packageOverview.packageUId);
                // These are public ids.... this check doesn't make sense
                // packageData.packageGroupPublicUids.map(id => expectValidDbId(id))
                expect_1.expect(packageData.licenses).to.deep.equal(['content/path/to/license/file']);
                if (!pkg.packageOverview.supplements) {
                    expect_1.expect(packageData.packageType).to.equal("MainPackage" /* MAIN_PACKAGE */);
                }
                else {
                    expect_1.expect(packageData.packageType).to.equal("SubPackage" /* SUB_PACKAGE */);
                }
            }
        });
        it('should have unique package group db ids', async () => {
            const packageGroupDbIds = packages.map(p => p.packageGroupPublicUids);
            expect_1.expect(_.uniq(packageGroupDbIds).length).to.equal(packageGroupDbIds.length);
        });
        it('should return packages without duplicates and sorted', async () => {
            ...
            expect_1.expect(uids).to.deep.equal(expectedOrder);
        });
    });
    describe("api/packageGroups" /* GET_PACKAGE_GROUPS */, () => {
        let packageGroups;
        let mainPackageOverviewsMetadata;
        before(async () => {
            packageGroups = await makeApiRequest("api/packageGroups" /* GET_PACKAGE_GROUPS */);
        ...
        it('should return the same number of package groups as main packages', async () => {
            expect_1.expect(packageGroups.length).to.equal(mainPackageOverviewsMetadata.length);
        });
        specify("a package group has (a) the id of main package prefixed with 'grp_', (b) the same version and hide-by-default as the main package", async () => {
            for (const mainPackageOverviewMetadata of mainPackageOverviewsMetadata) {
                ...
                expect_1.expect(packageGroup.packageGroupPublicId).to.equal(dbBuilderUtils_1.encodePackageGroupPublicId(nodes_utils_1.GROUP_ID_PREFIX + mainPackageOverviewMetadata.packageId));
                expect_1.expect(packageGroup.packageGroupVersion).to.equal(mainPackageOverviewMetadata.version);
                expect_1.expect(packageGroup.mainPackagePublicUid).to.equal(mainPackageOverviewMetadata.packageUId);
                expect_1.expect(packageGroup.hideByDefault).to.equal(mainPackageOverviewMetadata.hideByDefault || false);
            }
        });
        ...
        specify('a package group lists the UIDs of the packages it contains', async () => {
            for (const mainPackageOverviewMetadata of mainPackageOverviewsMetadata) {
                ...
                if (!mainPackageOverviewMetadata.dependencies ||
                    ...
                    expect_1.expect(packageGroup.packagesPublicUids.length).to.equal(1);
                    expect_1.expect(packageGroup.packagesPublicUids[0]).to.equal(mainPackageOverviewMetadata.packageUId);
                }
                else {
                    // must have main and supplemental packages
                    expect_1.expect(packageGroup.packagesPublicUids.length).to.equal(2);
                    expect_1.expect(packageGroup.packagesPublicUids).to.include(mainPackageOverviewMetadata.packageUId);
                    expect_1.expect(packageGroup.packagesPublicUids).to.include(dbBuilderUtils_1.formUid(mainPackageOverviewMetadata.dependencies[0].refId, mainPackageOverviewMetadata.dependencies[0].versionRange));
                }
            }
        });
        it(`should return a group's package list sorted in the same order as returned by ${"api/packages" /* GET_PACKAGES */}`, async () => {
            // Alphabetically, then newest to oldest
            const expectedOrder = [
                'com.ti.A_SUPPLEMENTAL_PACKAGE__1.23.45.00',
                'com.ti.PACKAGEA__1.23.45.00'
            ];
            expect_1.expect(packageGroups[0].packagesPublicUids).to.deep.equal(expectedOrder);
        });
        specify.skip('a device/devtool package group should not have a main package', async () => { });
    });
});
function generateMetadata() {
    const device = database_utils_1.generateDevice('DEVICE', 'device');
    const devtool = database_utils_1.generateDevtool('DEVTOOL', 'devtool');
    ...
    function createPackage(name, version, hideByDefault) {
        const packageOverview = database_utils_1.generatePackageOverview(name, version, { hideByDefault });
        const overviews = [];
        return {
            packageOverview,
            resources: [database_utils_1.generateResource('resource', packageOverview, device, devtool)],
            overviews,
            simpleCategories: []
        };
    }
...
async function makeApiRequest(api) {
    try {
        ...
        expect_1.expect(sessionId).to.not.be.undefined;
        expect_1.expect(parseInt(sessionId)).to.be.lessThan(new Date().getTime());
