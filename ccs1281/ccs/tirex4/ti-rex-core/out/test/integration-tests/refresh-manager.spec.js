...
function createPackageEntryRefreshReq({ id, version, content, operation }) {
    const prrt = {
        uid: dbBuilderUtils_1.formUid(id, version),
        content: content[0],
        operation,
        installCommand: {},
        installPath: {},
...
function changePackageOperation(id, version, opr, peb, content) {
    if (opr === "replacePackage" /* REPLACE_PACKAGE */ && !content) {
        // TODO this is an error condition
    }
    const index = peb.findIndex(item => item.uid === dbBuilderUtils_1.formUid(id, version));
    if (index !== -1) {
...
function changePackagePropertyVersion(id, version, peb, versionNew) {
    const index = peb.findIndex(item => item.uid === dbBuilderUtils_1.formUid(id, version));
    peb[index].uid = dbBuilderUtils_1.formUid(id, versionNew);
}
...
describe('refreshManager', async function () {
    ...
    beforeEach(async () => {
        ...
        peb = createFullListForRefresh("addPackage" /* ADD_PACKAGE */);
        ...
        peb = createFullListForRefresh("doNothing" /* DO_NOTHING */);
    });
    afterEach(async () => {
        try {
    ...
    describe(`Individual Refresh`, async () => {
        it('Basic Full Refresh', async () => {
            ...
            packageids.forEach(packageid => {
                ...
                chai_1.expect(actionResult).to.equal("succeeded" /* SUCCEEDED */);
            });
            ...
            packageids.forEach(packageid => {
                dbSuffixes.forEach(suffix => {
                    ...
                    chai_1.expect(index).to.not.equal(-1);
                });
            });
        });
        it('Refresh with 2 versions of the same hardware package', async () => {
            ...
            peb = [
                createPackageEntryRefreshReq({
                    ...
                    operation: "doNothing" /* DO_NOTHING */
                })
            ];
            peb.push(...createFullListForRefresh("addPackage" /* ADD_PACKAGE */));
            peb.push(createPackageEntryRefreshReq({
                id: devicesId,
            ...
            result.forEach((result, uid) => {
                ...
                if (uid === 'com.ti.fake.devices__1.00.00.00' ||
                    ...
                    chai_1.expect(actionResult).to.equal("not applicable" /* NOTAPPLICABLE */);
                }
                else {
                    chai_1.expect(actionResult).to.equal("succeeded" /* SUCCEEDED */);
                }
        ...
        it('Refresh with 2 versions of the same hardware package: Disallow add/replace an older version', async () => {
            ...
            peb = [
                createPackageEntryRefreshReq({
                    ...
                    version: devtoolsVersion1,
                    content: ['tirex-product-tree/Fdevtools_1_00_00_00/'],
                    operation: "addPackage" /* ADD_PACKAGE */ // specifically request add or replace
                })
            ];
            peb.push(...createFullListForRefresh("addPackage" /* ADD_PACKAGE */));
            peb.push(createPackageEntryRefreshReq({
                ...
                version: devicesVersion1,
                content: ['tirex-product-tree/Fdevices_1_00_00_00/'],
                operation: "replacePackage" /* REPLACE_PACKAGE */ // specifically request add or replace
            }));
            const { result, success } = await refreshManager.individualRefresh(peb, contentBasePath, false);
            chai_1.expect(success).to.be.false;
            // Test the result coming back
            result.forEach((result, uid) => {
                const actionResult = result.status.actionResult;
                if (uid === 'com.ti.fake.devices__1.00.00.00' ||
                    uid === 'com.ti.fake.devtools__1.00.00.00') {
                    chai_1.expect(actionResult).to.equal("failed" /* FAILED */);
                }
                else {
                    chai_1.expect(actionResult).to.equal("not started" /* NOTSTARTED */);
                }
            });
        });
        it('Check db backup folder got deleted', async () => {
            // Test that the backup db folder has been removed
            if (fs.existsSync(dbBuilder_1.getDbBackupBasePath(dbBasePath))) {
    ...
    describe(`Add`, async () => {
        it('Add 1 Device package', async () => {
            changePackageOperation(devicesId, devicesVersion2, "addPackage" /* ADD_PACKAGE */, peb, 'tirex-product-tree/Fdevices_2_00_00_00');
            const { result, success } = await refreshManager.individualRefresh(peb, contentBasePath, false);
            if (!success) {
                throw new Error();
            }
            await verifyResult("succeeded" /* SUCCEEDED */, packageids, result);
            await verifyDevDBsGenerated();
        });
        it('Add 1 Devtool package', async () => {
            changePackageOperation(devtoolsId, devtoolsVersion2, "addPackage" /* ADD_PACKAGE */, peb, 'tirex-product-tree/Fdevtools_2_00_00_00');
            const { result, success } = await refreshManager.individualRefresh(peb, contentBasePath, false);
            if (!success) {
                throw new Error('Refresh should have passed, but failed');
            }
            ...
            await verifyResult("succeeded" /* SUCCEEDED */, packageids, result);
            await verifyOperation("addPackage" /* ADD_PACKAGE */, [devtoolsId + '__' + devtoolsVersion2], result);
            const packageidsSub = packageids.filter(pkgUId => {
                return pkgUId !== devtoolsId + '__' + devtoolsVersion2;
            });
            await verifyOperation("updatedPackage" /* UPDATED_PACKAGE */, packageidsSub, result);
            await verifyDevDBsGenerated();
        });
        it('Add Attempt 1 software package', async () => {
            ...
            peb.push(createPackageEntryRefreshReq({
                ...
                operation: "addPackage" /* ADD_PACKAGE */
            }));
            packageids = peb.map(pkg => {
                return pkg.uid;
            });
            ...
            await verifyResult("not found" /* NOTFOUND */, [spp.id + '__' + spp.version], result);
            const packageidsSub = packageids.filter(pkgUId => {
                return pkgUId !== spp.id + '__' + spp.version;
            });
            await verifyResult("not applicable" /* NOTAPPLICABLE */, packageidsSub, result);
        });
        it('Add 1 independent software package', async () => {
            ...
            await verifyResult("succeeded" /* SUCCEEDED */, [spp.id + '__' + spp.version], result);
            const packageidsSub = packageids.filter(pkgUId => {
                return pkgUId !== spp.id + '__' + spp.version;
            });
            await verifyResult("not applicable" /* NOTAPPLICABLE */, packageidsSub, result);
        ...
        it('Add 1 sdk software package', async () => {
            ...
            await verifyResult("succeeded" /* SUCCEEDED */, [sppSDK.id + '__' + sppSDK.version], result);
            await verifyResult("not applicable" /* NOTAPPLICABLE */, packageids.filter(pkgUId => {
        ...
        it('Add Attempt 1 sla software package', async () => {
            ...
            await verifyResult("failed" /* FAILED */, [sppSLA.id + '__' + sppSLA.version], result);
            await verifyResult("not applicable" /* NOTAPPLICABLE */, packageids.filter(pkgUId => {
        ...
        it('Add 1 sla & 1 sdk software package', async () => {
            ...
            await verifyResult("succeeded" /* SUCCEEDED */, [sppSLA.id + '__' + sppSLA.version, sppSDK.id + '__' + sppSDK.version], result);
            await verifyResult("not applicable" /* NOTAPPLICABLE */, packageids.filter(pkgUId => {
                return !(pkgUId === sppSDK.id + '__' + sppSDK.version ||
        ...
        it('Add later version of SLA software package', async () => {
            ...
            await verifyResult("succeeded" /* SUCCEEDED */, [laterSLA.id + '__' + laterSLA.version], result);
            await verifyResult("not applicable" /* NOTAPPLICABLE */, packageids.filter(pkgUId => {
    ...
    describe(`Delete`, async () => {
        it('Delete Attempt 1 independent software package (The UId cannot be found: should do nothing )', async () => {
            ...
            peb[0] = createPackageEntryRefreshReq({
                ...
                version: '9.00.00.00',
                content: [spp.folder],
                operation: "removePackage" /* REMOVE_PACKAGE */
            });
            packageids = peb.map(pkg => {
                return pkg.uid;
            });
            ...
            await verifyResult("not applicable" /* NOTAPPLICABLE */, packageids, result);
        });
        ...
        it('Delete 1 independent software package', async () => {
            ...
            await verifyResult("succeeded" /* SUCCEEDED */, [spp.id + '__' + spp.version], result);
            await verifyResult("not applicable" /* NOTAPPLICABLE */, packageids.filter(pkgUId => pkgUId !== spp.id + '__' + spp.version), result);
        ...
        it('Delete 1 sla software package which supplements 1 sdk package', async () => {
            ...
            await verifyResult("succeeded" /* SUCCEEDED */, [sppSLA.id + '__' + sppSLA.version], result);
        ...
        it('Delete Attempt 1 Device package (package path does not exist and package not in DB: should still delete package)', async () => {
            changePackageOperation(devicesId, devicesVersion2, "removePackage" /* REMOVE_PACKAGE */, peb, 'tirex-product-tree/devices_2_00_00_00' // non-existent path
            );
        ...
        it('Delete Attempt 1 Devtool package (package path does not exist and package not in DB: should still delete package)', async () => {
            changePackageOperation(devtoolsId, devtoolsVersion2, "removePackage" /* REMOVE_PACKAGE */, peb, 'tirex-product-tree/devtools_1_00_00_00');
        ...
        it('Delete 1 Device package', async () => {
            changePackageOperation(devicesId, devicesVersion2, "removePackage" /* REMOVE_PACKAGE */, peb, 'tirex-product-tree/Fdevices_2_00_00_00');
            const { result, success } = await refreshManager.individualRefresh(peb, contentBasePath, false);
            if (!success) {
                throw new Error('Refresh should have passed, but failed');
            }
        ...
        it('Delete 1 Devtool package', async () => {
            changePackageOperation(devtoolsId, devtoolsVersion2, "removePackage" /* REMOVE_PACKAGE */, peb, 'tirex-product-tree/Fdevtools_2_00_00_00');
            const { result, success } = await refreshManager.individualRefresh(peb, contentBasePath, false);
            if (!success) {
                throw new Error('Refresh should have passed, but failed');
            }
    ...
    describe(`Replace`, async () => {
        it('Replace Attempt 1 independent software package, folder not found', async () => {
            ...
            peb.push(createPackageEntryRefreshReq({
                ...
                operation: "replacePackage" /* REPLACE_PACKAGE */
            }));
            packageids = peb.map(pkg => {
                return pkg.uid;
            });
            ...
            await verifyResult("not found" /* NOTFOUND */, [replacementSoftwarePackage.id + '__' + replacementSoftwarePackage.version], result);
        });
        it('Replace Attempt 1 independent software package, id does not match', async () => {
            ...
            await verifyResult("not found" /* NOTFOUND */, ['notfound.id' + '__' + replacementSoftwarePackage.version], result);
        });
        ...
        it('Replace 1 independent software package', async () => {
            ...
            await verifyResult("succeeded" /* SUCCEEDED */, packageidsSub, result);
        });
    });
    describe(`Combos`, () => {
        it('Combo Add 1, Delete 1 independent software package', async () => {
            ...
            await verifyResult("succeeded" /* SUCCEEDED */, [
        ...
        it('Combo: Delete Attempt 1 independent software package, Add 1 independent s/w package', async () => {
            ...
            await verifyResult("not applicable" /* NOTAPPLICABLE */, [spp.id + '__' + '9.00.00.00'], result);
            await verifyResult("succeeded" /* SUCCEEDED */, [aspp.id + '__' + aspp.version], result);
            const packageidsSub = packageids.filter(pkgUId => {
...
async function verifyDevDBsGenerated() {
    try {
        ...
        chai_1.expect(err, 'Refresh did not generate devices.db/devtools.db').to.not.exist;
    }
}
async function verifyDependencyToSLA(sdk, sla // null means no dependencies expected
    ...
    chai_1.expect(packageOverview).to.exist;
    const dependencies = packageOverview.dependencies;
    if (sla) {
        chai_1.expect(dependencies).to.have.deep.members([
            {
                refId: sla.id,
                versionRange: sla.version,
                require: 'optional',
                // for b/w compatibility:
                semver: versioning_1.convertToSemver(sla.version),
                version: sla.version
            }
        ]);
    }
    else {
        chai_1.expect(dependencies).to.be.empty;
    }
}
function generateMetadataContent(contentBasePath) {
    ...
    const devices = [
        content_utils_1.generateDeviceContent('DEVICE1', 'id.device1'),
        content_utils_1.generateDeviceContent('DEVICE2', 'id.device2')
    ];
    ...
    const devicesPackageOverviewContent = [
        content_utils_1.generatePackageOverviewContent('device Hardware Package', devicesId, devicesVersion1, 'devices')
    ];
    ...
    const devices2 = [
        content_utils_1.generateDeviceContent('DEVICE3', 'id.device3'),
        content_utils_1.generateDeviceContent('DEVICE4', 'id.device4')
    ];
    ...
    const devicesPackageOverviewContent2 = [
        content_utils_1.generatePackageOverviewContent('device2 Hardware Pacakge', devicesId, devicesVersion2, 'devices')
    ];
    ...
    const devtools = [content_utils_1.generateDevtoolContent('BOARD1', 'id.board1')];
    ...
    const devtoolsPackageOverviewContent = [
        content_utils_1.generatePackageOverviewContent('devtool Hardware Pacakge', devtoolsId, devtoolsVersion1, 'devtools')
    ];
    ...
    const devtools2 = [content_utils_1.generateDevtoolContent('BOARD1', 'id.board1')];
    ...
    const devtoolsPackageOverviewContent2 = [
        content_utils_1.generatePackageOverviewContent('devtool Hardware Package', devtoolsId, devtoolsVersion2, 'devtools')
    ];
...
function generateSoftwarePackageMetadataContent(pkg, contentBasePath) {
    ...
    const packageOverviewContent = [
        content_utils_1.generatePackageOverviewContent(folder, id, version, 'software', dependencies, supplements)
    ];
    ...
    const resourceContent = [
        ...
        content_utils_1.generateResourceContent('Example Resource 3', ['DEVICE1'], ['BOARD1'])
    ];
    fs.mkdirpSync(packageMetadataFolder);
