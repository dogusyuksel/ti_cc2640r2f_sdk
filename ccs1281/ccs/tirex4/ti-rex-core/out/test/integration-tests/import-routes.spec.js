...
describe('Import APIs', () => {
    ...
    describe(`${"api/importInfo" /* GET_IMPORT_INFO */}`, () => {
        it('Should have project.ccs import info as expected', async () => {
            ...
            const nodePath = nodes_utils_1.toNodePath(resourcePath);
            const id2 = await nodes_utils_1.getDbId(nodePath);
            ...
            expect_1.expect(deviceArray.length).to.equal(0); // Since there is no overrideProjectSpecDeviceId
            expect_1.expect(node.body.payload).to.deep.equal({
        ...
        it('Should have import project.energia info as expected', async () => {
            ...
            expect_1.expect(energiaBoardArray.length).to.equal(2);
            expect_1.expect(energiaBoardArray[0]).to.equal('board1' + '_energia_board_A');
            expect_1.expect(energiaBoardArray[1]).to.equal('board1' + '_energia_board_B');
        });
    });
...
async function getImportInfo(query) {
    return nodes_utils_1.makeDownloadImportRequest(`${"api/importInfo" /* GET_IMPORT_INFO */}`, query);
}
/*
...
function generateMetadata() {
    ...
    const packageOverview = database_utils_1.generatePackageOverview('importTest', '1.23.45.00');
    const resources = [
        database_utils_1.generateResource('test.projectspec', packageOverview, devices[0], devtools[0], {
        ...
        database_utils_1.generateResource('test.ino', packageOverview, devices[0], devtools[0], {
        ...
        database_utils_1.generateResource('importablefile.c', packageOverview, devices[0], devtools[0], {
            resourceType: 'file',
