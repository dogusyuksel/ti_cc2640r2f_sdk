...
describe('/api/content', () => {
    describe('valid db update', () => {
        describe('before db update', () => {
            before(async function beforeCallback() {
                this.timeout(20000);
                const emptyContentPath = test_helpers_1.getUniqueFolderName();
                fs.emptyDirSync(emptyContentPath);
                await nodes_utils_1.updateDBContent(emptyContentPath);
            });
            it('should return an error trying to fetch a not yet imported file', async () => {
                ...
                expect_1.expect(result.status).to.equal(404);
            });
        });
        describe('after db update', () => {
            ...
            before(function beforeCallback() {
                this.timeout(20000);
                contentPath = test_helpers_1.getUniqueFolderName();
                nodes_utils_1.createContent(contentPath, testContent);
                return nodes_utils_1.updateDBContent(contentPath);
            });
            it('should get an existing file', async () => {
                const res = await expect_1.chai.request(app).get('/content/sanityTest/file');
                expect_1.expect(res).to.have.status(200);
                expect_1.expect(res.text).to.equal(testContent.sanityTest.file);
                expect_1.expect(res).to.have.header('etag');
            });
            ...
            it('should get content with special characters in the name', async () => {
                ...
                expect_1.expect(res).to.have.status(200);
                expect_1.expect(res.text).to.equal(testContent.specialCharsTest['folder with spaces']['file with spaces']);
            });
            it('should get content in moderately deep folders', async () => {
                ...
                expect_1.expect(res).to.have.status(200);
                expect_1.expect(res.text).to.equal(testContent.mediumNamesTest[path].file);
            });
            it('should get content in very deep folders', async () => {
                ...
                expect_1.expect(res).to.have.status(200);
                expect_1.expect(res.text).to.equal(testContent.longNamesTest[path].file);
            });
            it('should get large file content', async () => {
                const res = await expect_1.chai.request(app).get('/content/bigFileTest/file');
                expect_1.expect(res).to.have.status(200);
                expect_1.expect(res.text).to.equal(testContent.bigFileTest.file);
            }).timeout(20000);
            describe('mime types', () => {
                ...
                _.map(expectedMimeTypes, (mimeType, fileName) => {
                    it(fileName + ': should get mime type ' + mimeType, async () => {
                        ...
                        expect_1.expect(res).to.have.status(200);
                        expect_1.expect(res).to.have.header('content-type', mimeType);
                    });
                });
            });
            describe('compression', () => {
                it('should get large content files compressed', async () => {
                    ...
                    expect_1.expect(res).to.have.status(200);
                    expect_1.expect(res.text).to.equal(testContent.compressionTest['big.txt']);
                    expect_1.expect(res).to.have.header('content-encoding', 'gzip');
                    expect_1.expect(res).to.not.have.header('content-length');
                });
                it('should get small files uncompressed', async () => {
                    ...
                    expect_1.expect(res).to.have.status(200);
                    expect_1.expect(res.text).to.equal(testContent.compressionTest['small.txt']);
                    expect_1.expect(res).to.not.have.header('content-encoding', 'gzip');
                    expect_1.expect(res).to.have.header('content-length');
                });
                it('should get uncompressible files uncompressed', async () => {
                    ...
                    expect_1.expect(res).to.have.status(200);
                    expect_1.expect(res.body.toString()).to.equal(testContent.compressionTest['big.png']);
                    expect_1.expect(res).to.not.have.header('content-encoding');
                    expect_1.expect(res).to.have.header('content-length');
                });
                ...
                it('should get weak etags for compressed content', async () => {
                    const res = await expect_1.chai.request(app).get('/content/compressionTest/big.txt');
                    expect_1.expect(res).to.have.status(200);
                    expect_1.expect(res).to.have.header('etag', /W\/".*/);
                });
                it('should get strong etags for uncompressed content', async () => {
                    ...
                    expect_1.expect(res).to.have.status(200);
                    expect_1.expect(res).to.have.header('etag');
                    expect_1.expect(res).to.not.have.header('etag', /W\/".*/);
                });
            });
            it('should download directores as a zip file', async () => {
                ...
                const tempDir = test_helpers_1.getUniqueFolderName();
                fs.emptyDirSync(tempDir);
                const file = path.join(tempDir, 'archive.zip');
                fs.writeFileSync(file, res.body);
                child_process_1.execSync('unzip ' + file, { cwd: tempDir });
                ...
                function verifyContent(expectedContent, fullPath) {
                    ...
                    expect_1.expect(expectedContent).to.equal(actualContent);
                }
            });
        ...
        describe('after db re-update', () => {
            ...
            before(async function beforeCallback() {
                this.timeout(20000);
                const initialContentPath = test_helpers_1.getUniqueFolderName();
                fs.emptyDirSync(initialContentPath);
                nodes_utils_1.createContent(initialContentPath, initialContent);
                await nodes_utils_1.updateDBContent(initialContentPath);
                contentPath = test_helpers_1.getUniqueFolderName();
                fs.emptyDirSync(contentPath);
                nodes_utils_1.createContent(contentPath, newContent);
                await nodes_utils_1.updateDBContent(contentPath);
            });
            it('should get updated data', async () => {
                const res = await expect_1.chai.request(app).get('/content/folder/file');
                expect_1.expect(res).to.have.status(200);
                expect_1.expect(res.text).to.equal(newContent.folder.file);
            });
            ...
            it('should get new data', async () => {
                ...
                expect_1.expect(res).to.have.status(200);
                expect_1.expect(res.text).to.equal(newContent.newFolder.file);
            });
            it('should set the resource stats match the file system', async () => {
                await verifyFileStats(contentPath);
            });
        ...
        describe('after db append only', () => {
            ...
            before(async function beforeCallback() {
                this.timeout(20000);
                const contentPath = test_helpers_1.getUniqueFolderName();
                fs.emptyDirSync(contentPath);
                nodes_utils_1.createContent(contentPath, initialContent);
                await nodes_utils_1.updateDBContent(contentPath);
                const contentPath2 = test_helpers_1.getUniqueFolderName();
                fs.emptyDirSync(contentPath2);
                nodes_utils_1.createContent(contentPath2, newContent);
                await nodes_utils_1.updateDBContent(contentPath2, { appendOnly: true });
            });
            it('should get updated data', async () => {
                const res = await expect_1.chai.request(app).get('/content/folder/file');
                expect_1.expect(res).to.have.status(200);
            ...
            it('should get files not in the new set', async () => {
                ...
                expect_1.expect(res).to.have.status(200);
                expect_1.expect(res.text).to.equal(initialContent.deleteTest.file);
            });
            it('should get new data', async () => {
                const res = await expect_1.chai.request(app).get('/content/newFolder/file');
                expect_1.expect(res).to.have.status(200);
    ...
    describe('skipping symlinks', () => {
        before(async function beforeCallback() {
            ...
            const contentPath = test_helpers_1.getUniqueFolderName();
            fs.emptyDirSync(contentPath);
            nodes_utils_1.createContent(contentPath, {
                realFolder: {
                    realFile: 'data'
                },
    ...
    describe('rejecting symlinks', () => {
        it('should fail if directories are symlinked', async () => {
            try {
                const linkedFolderContentPath = test_helpers_1.getUniqueFolderName();
                fs.emptyDirSync(linkedFolderContentPath);
                nodes_utils_1.createContent(linkedFolderContentPath, {
                    realFolder: {
                        realFile: 'data'
                    },
                    linkedFile: 'realFolder'
                });
                await nodes_utils_1.updateDBContent(linkedFolderContentPath, { strictValidation: true });
                expect_1.expect('should not get here').to.equal(true);
            }
            catch (e) {
                expect_1.expect(e.toString()).to.contain('Symlinks detected in content: ');
                expect_1.expect(e.toString()).to.contain('/linkedFile');
            }
        });
        it('should fail if files are symlinked', async () => {
            try {
                const linkedFilePath = test_helpers_1.getUniqueFolderName();
                fs.emptyDirSync(linkedFilePath);
                nodes_utils_1.createContent(linkedFilePath, {
                    realFolder: {
                        realFile: 'data'
                    },
                    linkedFolder: {
                ...
                await nodes_utils_1.updateDBContent(linkedFilePath, { strictValidation: true });
                expect_1.expect('should not get here').to.equal(true);
            }
            catch (e) {
                expect_1.expect(e.toString()).to.contain('Symlinks detected in content: ');
                expect_1.expect(e.toString()).to.contain('/linkedFolder/linkedFile');
            }
        });
    });
    describe('after db update of specified include folders', () => {
        ...
        before(async function beforeCallback() {
            ...
            fs.emptyDirSync(contentPath2);
            nodes_utils_1.createContent(contentPath2, newContent);
            const configFileFolder = test_helpers_1.getUniqueFolderName();
        ...
        it('should change included data', async () => {
            ...
            expect_1.expect(res).to.have.status(200);
            expect_1.expect(res.text).to.equal(newContent.folder2.file3);
        });
        it('should insert new included data', async () => {
            const res = await expect_1.chai.request(app).get('/content/folder3/file');
            expect_1.expect(res).to.have.status(200);
            expect_1.expect(res.text).to.equal(newContent.folder3.file);
        });
    });
});
...
async function verifyFileStats(contentPath) {
    ...
    expect_1.expect(results).to.not.be.empty;
    results.forEach(result => {
        const filePath = result.name.slice(prefix.length);
        const stats = fs.statSync(path.join(contentPath + filePath));
        expect_1.expect(stats.mtime.toISOString()).to.equal(new Date(result.modified).toISOString());
        expect_1.expect(stats.size).to.equal(result.size);
    });
}
