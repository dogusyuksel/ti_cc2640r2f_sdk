...
describe('DB Nodes APIs', () => {
    before(async function () {
        ...
        const dbFolder = await nodes_utils_1.updateDatabase(generateMetadata());
        ({ packageGroupsDef } = await sqldbImporter_1.discoverPackageGroups(dbFolder));
        filterOptions = await nodes_utils_1.getFilterOptions();
    });
    describe("api/rootNode" /* GET_ROOT_NODE */, () => {
        ...
        it('should have a valid root node', async () => {
            const rootNode = await nodes_utils_1.getRootNode();
            database_utils_1.expectValidDbId(rootNode);
        });
    });
    describe("api/nodePublicIdToDbId" /* GET_NODE_PUBLIC_ID_TO_DB_ID */, () => {
        async function verify(pkg, resourceIndex) {
            ...
            expect_1.expect(packageGroup).to.exist;
            expect_1.expect(packageGroup.uid).to.exist;
            const dbId = await nodes_utils_1.getNodeDbId({
                ...
                toDbIdType: "ToDbIdNotLatest" /* TO_DB_ID_GROUP_NOT_LATEST */
            });
            const nodesData = await nodes_utils_1.getNodesData({ dbId: [dbId] });
            const expectedNodeData = nodesData.dbIdToData[dbId];
            expect_1.expect(resource.name).to.be.equal(expectedNodeData.name);
            expect_1.expect(resource.fullPathsPublicIds[0]).to.be.equal(expectedNodeData.nodePublicId);
        }
        ...
        it('should get correct node based on its publicId: same publicId and package id, but different version', async () => {
            ...
            expect_1.expect(multiVersionPackageV1.packageOverview.packageId).to.be.equal(multiVersionPackageV2.packageOverview.packageId);
            expect_1.expect(multiVersionPackageV1.packageOverview.version).to.be.not.equal(multiVersionPackageV2.packageOverview.version);
            expect_1.expect(multiVersionPackageV1.resources[resourcerIndex].fullPathsPublicIds[0]).to.be.equal(multiVersionPackageV2.resources[resourcerIndex].fullPathsPublicIds[0]);
        ...
        it('should get correct non-package node', async () => {
            ...
            const dbId = await nodes_utils_1.getNodeDbId({
                ...
                toDbIdType: "ToDbIdNoGroup" /* TO_DB_ID_NO_GROUP */
            });
            const nodesData = await nodes_utils_1.getNodesData({ dbId: [dbId] });
            const expectedNodeData = nodesData.dbIdToData[dbId];
            expect_1.expect(softwareCategory.name).to.be.equal(expectedNodeData.name);
            expect_1.expect(softwareCategory.fullPathsPublicIds[0]).to.be.equal(expectedNodeData.nodePublicId);
        });
        ...
        it('should handle non-existing query arguments', async () => {
            await expectThrows(HttpStatus.BAD_REQUEST, 'Must specify nodePublicId', nodes_utils_1.getNodeDbId({
                nodePublicId: undefined,
                packageGroupPublicUid: 'something',
                packagePublicId: 'another',
                toDbIdType: "ToDbIdNotLatest" /* TO_DB_ID_GROUP_NOT_LATEST */
            }));
            await expectThrows(HttpStatus.BAD_REQUEST, 'Must specify packageGroupPublicUid', nodes_utils_1.getNodeDbId({
    ...
    describe("api/rex3LinkToDbId" /* GET_REX3_LINK_TO_DB_ID */, async () => {
        it('should get correct node based on Rex 3 style "link"', async () => {
            const resource = multiVersionPackageV2.resources[0];
            const dbId = await nodes_utils_1.getNodeDbIdRex3({
                linkField: [...resource.fullPaths[0], resource.name].join('/')
            });
            const nodesData = await nodes_utils_1.getNodesData({ dbId: [dbId] });
            const nodeData = nodesData.dbIdToData[dbId];
            expect_1.expect(nodeData.name).to.be.equal(resource.name);
            expect_1.expect(nodeData.packagePublicUid).to.be.equal(resource.packageUId);
        });
        ...
        it('should get correct node based on Rex 3 style "link" that is a foundation node', async () => {
            ...
            const nodesData = await nodes_utils_1.getNodesData({ dbId: [dbId] });
            const nodeData = nodesData.dbIdToData[dbId];
            expect_1.expect(nodeData.name).to.be.equal(resource.fullPaths[0][1]);
            expect_1.expect(nodeData.packagePublicUid).to.not.exist;
        });
        it('should get correct node based on Rex 3 style "link": always go to latest package', async () => {
            ...
            const nodeData = nodesData.dbIdToData[dbId];
            expect_1.expect(nodeData.name).to.be.equal(resource.name);
            expect_1.expect(dbBuilderUtils_1.splitUid(nodeData.packagePublicUid).id).to.be.equal(resource.packageId);
            // latest package version expected
            expect_1.expect(dbBuilderUtils_1.splitUid(nodeData.packagePublicUid).version).to.be.equal(multiVersionPackageV2.packageOverview.packageVersion);
        });
        it('should get 404 for Rex 3 style "link" that cannot be resolved', async () => {
            ...
            await expectThrows(404, /.*Could not resolve Rex 3 link.*/, nodes_utils_1.getNodeDbIdRex3({
        ...
        it('should handle non-existing query arguments', async () => {
            await expectThrows(HttpStatus.BAD_REQUEST, 'Must specify linkField', nodes_utils_1.getNodeDbIdRex3({
                linkField: undefined
            }));
        });
    });
    describe("api/filterOptions" /* GET_FILTER_OPTIONS */, () => {
        it(`should have a device filter for each device`, async () => {
            expect_1.expect(filterOptions.devices.length).to.equal(devices.length);
            for (const deviceMetadata of devices) {
                ...
                expect_1.expect(found, deviceMetadata.id).to.exist;
            }
        });
        it(`should have a devtool filter for each devtool`, async () => {
            expect_1.expect(filterOptions.devtools.length).to.equal(devtools.length);
            for (const devtoolMetadata of devtools) {
                ...
                expect_1.expect(found).to.exist;
            }
        });
        it('should have a package group filter for each main package representing the group', async () => {
            ...
            expect_1.expect(filterOptions.packageGroups.length).to.equal(mainPackageOverviewsMetadata.length);
            for (const mainPackageOverviewMetadata of mainPackageOverviewsMetadata) {
                ...
                expect_1.expect(foundGroup).to.exist;
            }
        });
        it('should not have any package group filters with restricted packages', async () => {
            ...
            expect_1.expect(filterOptions.packageGroups.find(group => group.publicId === nodes_utils_1.GROUP_ID_PREFIX + restrictedPackageUId)).to.not.exist;
        });
        it('should have filter option for every occurring filter field value', () => {
            ...
            for (const fieldName of Object.keys(filterFieldValues)) {
                ...
                filterFieldValues[fieldName].forEach(field => expect_1.expect(filter).to.include(field));
            }
        });
    });
    describe("api/filteredChildrenNodeIds" /* GET_FILTERED_CHILDREN_NODE_IDS */, () => {
        ...
        async function verifyPath(path, { filterPackageGroup, filterDevice, filterDevtool, filterSearch, filterResourceClass, filterKernel, filterCompiler, filterLanguage, filterIde } = {}) {
            ...
            let currentId = await nodes_utils_1.getRootNode();
            let currentNode = 'root';
            while (path.length) {
                // find the current node's children
                const childNodes = await nodes_utils_1.getFilteredChildrenNodeIds({
                ...
                const nodesData = await nodes_utils_1.getNodesData({
        ...
        describe('Sanity', () => {
            ...
            it('should not find children under a leaf node', async () => {
                ...
                expect_1.expect(leafChildren).to.be.empty;
            });
            it('should operate in bulk mode', async () => {
                const idsToTest = [
                    await nodes_utils_1.getDbId(simplePackage1.resources[0]),
                    await nodes_utils_1.getDbId(devFilterPackage.resources[1]),
                    await nodes_utils_1.getDbId(simplePackage1.packageOverview),
                    await nodes_utils_1.getRootNode()
                ];
                // Get the children in bulk mode
                const result = await nodes_utils_1.getFilteredChildrenNodeIds({
                    parentDbId: idsToTest,
                    filterPackageGroup: defaultFilterPackageGroups
                });
                // Verify it matches individually
                for (const id of idsToTest) {
                    const expected = await nodes_utils_1.getFilteredChildrenNodeIds({
                        parentDbId: [id],
                        filterPackageGroup: defaultFilterPackageGroups
                    });
                    expect_1.expect(result.parentToChildDbId[id]).to.deep.equal(expected.parentToChildDbId[id]);
                }
            });
        });
        describe('Filters', () => {
            it('should have no children for root if all packages filtered', async () => {
                ...
                expect_1.expect(rootChildren).to.be.empty;
            });
            ...
            it('should not find the node of the package when it is filtered out by package', async () => {
                ...
                expect_1.expect(folderChildren).to.not.be.empty;
                expect_1.expect(folderChildren).to.not.contain(resource.package);
            });
            ...
            it('should not find node for a resource filtered out by device', async () => {
                ...
                expect_1.expect(folderChildren).to.not.be.empty;
                expect_1.expect(folderChildren).to.not.contain(resource.name);
            });
        ...
        describe('Additional Filters', async () => {
            ...
            it('should find all nodes in the package when no filter used', async () => {
                ...
                expect_1.expect(folderChildren).to.include.members(variedFiltersPackage.resources.map(r => r.name));
            });
            const filtersToTest = [
            ...
            for (const filterToTest of filtersToTest) {
                ...
                it(`should filter individually by ${fieldName}`, async () => {
                    ...
                    expect_1.expect(folderChildren).to.have.members(variedFiltersPackage.resources
        ...
        describe('Search', () => {
            ...
            it('should not find node for a resource filtered out by search', async () => {
                ...
                // So the resource shouldn't be there, but there are other resources at this path
                expect_1.expect(folderChildren).to.not.be.empty;
                expect_1.expect(folderChildren).to.not.contain(resource.fullPaths[0][0]);
            });
        });
        describe('Search and filtering', () => {
            ...
            it('should not find node for a resource excluded by filter when searching', async () => {
                ...
                expect_1.expect(folder).to.not.be.empty;
                expect_1.expect(folder).to.not.include(resourceExcludedByFilter.name);
            ...
            it('should not find node for a resource excluded by search when filtering', async () => {
                ...
                expect_1.expect(folder).to.not.be.empty;
                expect_1.expect(folder).to.not.include(resourceExcludedBySearch.name);
        ...
        describe('Sort', () => {
            ...
            for (const { sort: sort, description, topFolderName: folderName, expected } of testData) {
                it('should sort ' + description + ' when ' + sort + ' sort', async () => {
                    ...
                    expect_1.expect(resource).to.exist;
                    const parentDbId = await nodes_utils_1.getDbId(resource);
                    ...
                    const childNodeDbIds = (await nodes_utils_1.getFilteredChildrenNodeIds({
                    ...
                    expect_1.expect(childNames).to.have.ordered.members(expected);
                });
            }
        });
        describe('Negative testing', () => {
            ...
            it('should reject invalid parent ids', async () => {
                await expectThrows(400, 'Id is NaN: foo', nodes_utils_1.getFilteredChildrenNodeIds({
                    parentDbId: ['foo'],
                    filterPackageGroup: defaultFilterPackageGroups
                }));
            });
            it('should handle unknown parent ids', async () => {
                ...
                expect_1.expect(result.parentToChildDbId[Number.MAX_SAFE_INTEGER.toString()]).to.be.empty;
            });
            it('should reject non-existent filter ids', async () => {
                await expectThrows(400, 'No package group found with packageGroupPublicUid foo', nodes_utils_1.getFilteredChildrenNodeIds({
                    parentDbId: [await nodes_utils_1.getRootNode()],
                    filterPackageGroup: ['foo']
                }), 'filterPackage');
                for (const key of ['filterDevice', 'filterDevtool']) {
                    await expectThrows(400, /No (device|devtool) found with publicId foo/, nodes_utils_1.getFilteredChildrenNodeIds({
                        parentDbId: [await nodes_utils_1.getRootNode()],
                        filterPackageGroup: defaultFilterPackageGroups,
                        [key]: ['foo']
                    }), key);
            ...
            it('should reject multiple values for a filter', async () => {
                for (const key of filterNames) {
                    ...
                    await expectThrows(400, `Can only specify one ${type} in filter params`, nodes_utils_1.getFilteredChildrenNodeIds({
                        parentDbId: [await nodes_utils_1.getRootNode()],
                        filterPackageGroup: defaultFilterPackageGroups,
            ...
            it('should reject no ids given', async () => {
                await expectThrows(400, 'Required id parameter was not provided', nodes_utils_1.getFilteredChildrenNodeIds({
                    parentDbId: [],
                    filterPackageGroup: defaultFilterPackageGroups
                }));
                ...
                nodes_utils_1.getFilteredChildrenNodeIds({
                    filterPackageGroup: defaultFilterPackageGroups
                }));
            });
            it('should reject unknown filters', async () => {
                await expectThrows(400, 'Filter param filterFoo is not supported', nodes_utils_1.getFilteredChildrenNodeIds({
                    parentDbId: [await nodes_utils_1.getRootNode()],
                    filterPackageGroup: defaultFilterPackageGroups,
                    // @ts-ignore - intentionally violating type system to ensure it's caught
                    filterFoo: ['bar']
    ...
    describe("api/nodesData" /* GET_NODES_DATA */, () => {
        it('should test viewConstraint', async () => {
            let resource = viewConstrainedPackage.resources[0];
            let id = await nodes_utils_1.getDbId(resource);
            let result = await nodes_utils_1.getNodesData({ dbId: [id] });
            let data = result.dbIdToData[id];
            expect_1.expect(data).to.exist;
            expect_1.expect(data.descriptor.viewConstraint).to.equal("DownloadOnly" /* DOWNLOAD_ONLY */);
            resource = viewConstrainedPackage.resources[1];
            id = await nodes_utils_1.getDbId(resource);
            result = await nodes_utils_1.getNodesData({ dbId: [id] });
            data = result.dbIdToData[id];
            expect_1.expect(data).to.exist;
            expect_1.expect(data.descriptor.viewConstraint).to.not.exist;
            ...
            data = result.dbIdToData[id];
            expect_1.expect(data).to.exist;
            expect_1.expect(data.descriptor.viewConstraint).to.equal("ExternalOnly" /* EXTERNAL_ONLY */);
        });
        it('should return valid data for a folder node', async () => {
            const resource = simplePackage1.resources[0];
            const id = await nodes_utils_1.getDbId(resource.fullPaths[0]); // parent folder of the resource
            ...
            const result = await nodes_utils_1.getNodesData({ dbId: [id] });
            ...
            expect_1.expect(data).to.be.deep.equal(expectedData);
        });
        it('should return valid data for a folder with hidden resource node', async () => {
            ...
            const id = await nodes_utils_1.getDbId(resource);
            const result = await nodes_utils_1.getNodesData({ dbId: [id] });
            const data = result.dbIdToData[id];
            expect_1.expect(data).to.exist;
            const expectedData = {
        ...
        it('should return valid data for a package folder node', async () => {
            ...
            const id = await nodes_utils_1.getDbId(packageOverview);
            const result = await nodes_utils_1.getNodesData({ dbId: [id] });
            const data = result.dbIdToData[id];
            expect_1.expect(data).to.exist;
            const expectedData = {
        ...
        const leafNodeExpectedData = [
            {
                name: 'cppFileByLink',
                icon: "SourceCode_CPP" /* SOURCE_CODE_CPP */,
                contentType: "Code" /* SOURCE_CODE */,
                isDownloadable: true
            },
            {
                name: 'energiaProject (folderWithResource)',
                icon: "Project_Energia" /* PROJECT_ENERGIA */,
                contentType: "Code" /* SOURCE_CODE */,
                nodeType: response_data_1.Nodes.NodeType.FOLDER_WITH_HIDDEN_RESOURCE_NODE,
                isImportable: true,
            ...
            {
                name: 'asmFile.asm',
                icon: "SourceCode_ASM" /* SOURCE_CODE_ASM */,
                contentType: "Code" /* SOURCE_CODE */,
                isDownloadable: true
            },
            {
        ...
        for (const expected of leafNodeExpectedData) {
            it(`should validly decode file extensions: ${expected.name}`, async () => {
                ...
                if (expected.contentType === "Code" /* SOURCE_CODE */) {
        ...
        it('should operate in bulk mode', async () => {
            ...
            for (const expected of leafNodeExpectedData) {
                const resource = variedTypesPackage.resources.find(r => r.name === expected.name);
                idsToTest.push(await nodes_utils_1.getDbId(resource));
            }
            // Get the node data in bulk mode
            const result = await nodes_utils_1.getNodesData({
                dbId: idsToTest
            });
            // Verify it matches individually
            for (const id of idsToTest) {
                const expected = await nodes_utils_1.getNodesData({
                    dbId: [id]
                });
                expect_1.expect(result.dbIdToData[id]).to.deep.equal(expected.dbIdToData[id]);
            }
        });
        it('should handle missing ids', async () => {
            // @ts-ignore - explicitly breaking types to verify invalid request
            await expectThrows(400, 'Required id parameter was not provided', nodes_utils_1.getNodesData({}));
        });
        it('should handle invalid ids', async () => {
            await expectThrows(400, 'Id is NaN: foo', nodes_utils_1.getNodesData({
                dbId: ['foo']
            }));
        });
        it('should handle unknown ids', async () => {
            await expectThrows(404, `Unknown node id: ${Number.MAX_SAFE_INTEGER.toString()}`, nodes_utils_1.getNodesData({
                dbId: [Number.MAX_SAFE_INTEGER.toString()]
            }));
        });
        // TODO: Add tests for isDownloadable/isImportable once set by the server
    });
    describe("api/nodeExtendedData" /* GET_NODE_EXTENDED_DATA */, () => {
        async function verifyAndFetchExtData(parameter) {
            const nodePath = nodes_utils_1.toNodePath(parameter);
            const id = await nodes_utils_1.getDbId(nodePath);
            const result = await nodes_utils_1.getNodeExtendedData({ dbId: id });
            const data = result.dbIdToChildExtData[id];
            expect_1.expect(data).to.exist;
            expect_1.expect(data.nodeDbId).to.be.equal(id);
            const expectedNodeIdPath = await getExpectedNodeIdPath(nodePath);
            expect_1.expect(data.nodeDbIdPath).to.be.deep.equal(expectedNodeIdPath);
            return data;
            async function getExpectedNodeIdPath(currentNodePath) {
                if (currentNodePath.length) {
                    return [
                        ...
                        await nodes_utils_1.getDbId(currentNodePath)
                    ];
                }
                return [];
            }
        ...
        it('should return valid data for a folder node', async () => {
            ...
            expect_1.expect(data.nodeType).to.be.equal(response_data_1.Nodes.NodeType.FOLDER_NODE);
        });
        it('should return valid data for a folder with hidden resource node', async () => {
            const resource = variedTypesPackage.resources.find(r => r.name === 'folderWithResource');
            const data = await verifyAndFetchExtData(resource);
            expect_1.expect(data.nodeType).to.be.equal(response_data_1.Nodes.NodeType.FOLDER_WITH_HIDDEN_RESOURCE_NODE);
            ...
            if (typedData.overview) {
                if (resource.resourceType === 'project.energia') {
                    ...
                    expect_1.expect(typedData.overview.overviewLink).to.be.equal(`content/${resource.link}`);
                }
                else {
                    // regression check: resource should not be displayed otherwise
                    expect_1.expect(typedData.overview.overviewLink).to.not.be.equal(`content/${resource.link}`);
                }
            }
        });
        it('should return valid data for a package folder node', async () => {
            ...
            expect_1.expect(data.nodeType).to.be.equal(response_data_1.Nodes.NodeType.PACKAGE_FOLDER_NODE);
            expect_1.expect(data.overview).to.be.deep.equal({
                overviewText: packageOverview.description,
                overviewImage: `content/${packageOverview.image}`,
                overviewType: "Overview" /* OVERVIEW */
            });
        });
        it(`should return valid data for a leaf node`, async () => {
            const resource = variedTypesPackage.resources.find(r => r.name === 'cppFileByLink');
            const data = await verifyAndFetchExtData(resource);
            expect_1.expect(data.nodeType).to.be.equal(response_data_1.Nodes.NodeType.LEAF_NODE);
            const typedData = data;
            expect_1.expect(typedData.description).to.be.equal(resource.description);
            expect_1.expect(typedData.link).to.be.equal(`content/${resource.link}`);
        });
        it('should not operate in bulk mode', async () => {
            await expectThrows(400, 'Can only request extended data for a single node id', nodes_utils_1.getNodeExtendedData({
                // @ts-ignore - explicitly breaking types to verify invalid request
                dbId: ['1', '2']
            }));
        });
        it('should handle missing ids', async () => {
            await expectThrows(400, 'Required id parameter was not provided', 
            // @ts-ignore - explicitly breaking types to verify invalid request
            nodes_utils_1.getNodeExtendedData({}));
        });
        it('should handle invalid ids', async () => {
            await expectThrows(400, 'Id is NaN: foo', nodes_utils_1.getNodeExtendedData({
                dbId: 'foo'
            }));
        });
        it('should handle unknown ids', async () => {
            await expectThrows(404, new RegExp(`.*Unknown node id: ${Number.MAX_SAFE_INTEGER.toString()}`), nodes_utils_1.getNodeExtendedData({
                dbId: Number.MAX_SAFE_INTEGER.toString()
            }));
        });
    });
    describe("api/expandedFilteredDescendantNodesData" /* GET_EXPANDED_FILTERED_DESCENDANT_NODES_DATA */, () => {
        ...
        async function verifyExpandedChildren(parameter, { filterPackage, filterDevice, filterDevtool, filterSearch } = {}) {
            ...
            const id = await nodes_utils_1.getDbId(parameter);
            const result = await nodes_utils_1.getExpandedFilteredDescendantNodesData({
                parentDbId: id,
            ...
            expect_1.expect(sortResult(result.parentToChildData)).to.be.deep.equal(sortResult(expectedResult));
            async function getExpectedResult(parentId) {
                // Get the children ids
                const children = (await nodes_utils_1.getFilteredChildrenNodeIds({
                    parentDbId: [parentId],
                    filterPackageGroup: filterPackage || defaultFilterPackage,
                    filterDevice,
                    filterDevtool,
                ...
                const nodesData = await nodes_utils_1.getNodesData({ dbId: children });
        ...
        it('should not operate in bulk mode', async () => {
            await expectThrows(400, 'Can only request expanded filtered descendant data for a single parent node id', nodes_utils_1.getExpandedFilteredDescendantNodesData({
                // @ts-ignore - explicitly breaking types to verify invalid request
                parentDbId: ['1', '2'],
        ...
        it('should handle missing ids', async () => {
            await expectThrows(400, 'Required id parameter was not provided', 
            // @ts-ignore - explicitly breaking types to verify invalid request
            nodes_utils_1.getExpandedFilteredDescendantNodesData({ filterPackageGroup: defaultFilterPackage }));
        });
        it('should handle invalid ids', async () => {
            await expectThrows(400, 'Id is NaN: foo', nodes_utils_1.getExpandedFilteredDescendantNodesData({
                parentDbId: 'foo',
                filterPackageGroup: defaultFilterPackage
            }));
        });
        it('should handle unknown ids', async () => {
            ...
            expect_1.expect(result.parentToChildData[Number.MAX_SAFE_INTEGER.toString()]).to.be.empty;
        });
        it('should handle unknown filter ids', async () => {
            await expectThrows(400, 'No package group found with packageGroupPublicUid foo', nodes_utils_1.getExpandedFilteredDescendantNodesData({
                parentDbId: await nodes_utils_1.getRootNode(),
                filterPackageGroup: ['foo']
            }));
        });
    });
    describe("api/searchSuggestions" /* GET_SEARCH_SUGGESTIONS */, () => {
        ...
        it('should suggest whole words in descriptions', async () => {
            ...
            const suggestions = await nodes_utils_1.getSearchSuggestions({
                text: wordInDescription.toLowerCase(),
                filterPackageGroup
            });
            expect_1.expect(suggestions).to.include(wordInDescription);
        });
        const searches = [
            {
                searchString: '123',
        ...
        for (const search of searches) {
            it(`should suggest keywords (partial included) from a ${search.description} in a ` +
                ...
                expect_1.expect(suggestions).to.have.same.members(search.expected);
            });
        }
        it('should suggest devices', async () => {
            ...
            expect_1.expect(suggestions).to.include(deviceName);
        });
        it('should suggest boards', async () => {
            const boardName = 'board1';
            const suggestions = await nodes_utils_1.getSearchSuggestions({ text: boardName, filterPackageGroup });
            expect_1.expect(suggestions).to.include(boardName);
        });
        it('should handle undefined search text', async () => {
            await expectThrows(HttpStatus.BAD_REQUEST, 'Search text is undefined', nodes_utils_1.getSearchSuggestions({ text: undefined, filterPackageGroup }));
        });
...
describe('Database Update', () => {
    ...
    async function doUpdate(updateFunc) {
        ...
        const { packageGroupsDef: packageGroupsDef } = await sqldbImporter_1.discoverPackageGroups(dbFolder);
        // Verify this by ensuring the filter options list only one board/device/package
        const filterInfo = await nodes_utils_1.getFilterOptions();
        expect_1.expect(filterInfo.devices.length).to.be.equal(1);
        expect_1.expect(filterInfo.devices[0].name).to.be.equal('loneDevice' + timestamp);
        expect_1.expect(filterInfo.devtools.length).to.be.equal(1);
        expect_1.expect(filterInfo.devtools[0].name).to.be.equal('loadBoard' + timestamp);
        expect_1.expect(filterInfo.packageGroups.length).to.be.equal(1);
        // verify the lone resource is there
        const nodePublicId = metadata.packages[0].resources[0].fullPathsPublicIds[0];
        const nodeDbId = await nodes_utils_1.getNodeDbId({
            nodePublicId,
            packageGroupPublicUid: packageGroupsDef[0].uid,
            packagePublicId: metadata.packages[0].packageOverview.packageId,
            toDbIdType: "ToDbIdNotLatest" /* TO_DB_ID_GROUP_NOT_LATEST */
        });
        const nodesData = await nodes_utils_1.getNodesData({ dbId: [nodeDbId] });
        const nodeData = nodesData.dbIdToData[nodeDbId];
        expect_1.expect(nodeData.name).to.be.equal('loneResource' + timestamp);
    }
...
async function expectThrows(status, message, promise, context) {
    try {
        ...
        expect_1.expect(e.status).to.equal(status, context);
        if (typeof message === 'string') {
            expect_1.expect(e.text).to.equal(message, context);
        }
        else {
            expect_1.expect(e.text).to.match(message, context);
        }
        return;
    }
...
function generateMinimalMetadata(timestamp) {
    ...
    const devices = [database_utils_1.generateDevice('LONE_DEVICE', 'loneDevice' + timestamp)];
    const devtools = [database_utils_1.generateDevtool('LOAD_BOARD', 'loadBoard' + timestamp)];
    const packageOverview = database_utils_1.generatePackageOverview('loneOverview' + timestamp, '1.23.45.00');
    const resources = [
        database_utils_1.generateResource('loneResource' + timestamp, packageOverview, devices[0], devtools[0])
    ];
    const overviews = [];
    const simpleCategories = database_utils_1.generateSimpleCategories(resources, packageOverview);
    const packages = [
        {
            packageOverview,
            resources,
...
function generateDevices() {
    return [
        database_utils_1.generateDevice('DEVICE1', 'device1'),
        database_utils_1.generateDevice('DEVICE2', 'device2'),
        database_utils_1.generateDevice(searchText, searchText)
    ];
}
function generateDevtools() {
    return [
        database_utils_1.generateDevtool('BOARD1', 'board1'),
        database_utils_1.generateDevtool('BOARD2', 'board2'),
        database_utils_1.generateDevtool(searchText, searchText)
    ];
}
function generatePackage(generateFunc, devices, devtools) {
    ...
    const simpleCategories = database_utils_1.generateSimpleCategories(resources, packageOverview, overviews);
    return {
        packageOverview,
        resources,
        overviews,
...
function generateViewConstrainedPackage(devices, devtools) {
    const packageOverview = database_utils_1.generatePackageOverview('viewConstrained', '1.23.45.00');
    const resources = [
        database_utils_1.generateResource('resource0', packageOverview, devices[1], devtools[1], {
            link: 'abc.xyz'
        }),
        database_utils_1.generateResource('resource1', packageOverview, devices[1], devtools[1], {
            link: 'abc.txt'
        }),
        database_utils_1.generateResource('resource2', packageOverview, devices[0], devtools[0], {
            viewLimitations: ['e2e', 'aws']
        }),
        database_utils_1.generateResource('resource3', packageOverview, devices[0], devtools[0], {
            link: 'abc.txt',
            viewLimitations: ['e2e', 'aws']
        })
    ];
...
function generateSimplePackage1(devices, devtools) {
    const packageOverview = database_utils_1.generatePackageOverview('overview1', '1.23.45.00');
    const resources = [
        database_utils_1.generateResource('resource1', packageOverview, devices[0], devtools[0]),
        database_utils_1.generateResource('resource2', packageOverview, devices[1], devtools[1])
    ];
    const overviews = [];
    return {
        packageOverview,
...
function generateSubFoundationNodePackage(devices, devtools) {
    ...
    const packageOverview = database_utils_1.generatePackageOverview('mmWave', '1.23.45.00', {
...
function generateRestrictedPackage(devices, devtools) {
    const packageOverview = database_utils_1.generatePackageOverview('package group with restricted package', '1.23.45.00');
    packageOverview.restrictions = vars_1.Vars.METADATA_PKG_IMPORT_ONLY; // TODO: this should be an array REX-2219
    const resources = [database_utils_1.generateResource('resource', packageOverview, devices[1], devtools[1])];
    const overviews = [];
    return {
        packageOverview,
        overviews,
...
function generateDevFilterPackage(devices, devtools) {
    const packageOverview = database_utils_1.generatePackageOverview('overview2', '1.23.45.00');
    ...
    const resources = [
        database_utils_1.generateResource('resource3', packageOverview, devices[0], devtools[1]),
        database_utils_1.generateResource('resource4', packageOverview, devices[1], devtools[0])
    ];
    const overviews = [];
    return {
        packageOverview,
...
function _generateMultiVersionPackages(devices, devtools) {
    ...
    // package version 1
    const overviewPackageVersion1 = database_utils_1.generatePackageOverview(multiVersionPackageName, '1.00.00.00');
    const resourcesPackageVersion1 = [
        database_utils_1.generateResource(resourceName1, overviewPackageVersion1, devices[0], devtools[0]),
        database_utils_1.generateResource(resourceName2, overviewPackageVersion1, devices[1], devtools[1])
    ];
    // package version 2
    const overviewPackageVersion2 = database_utils_1.generatePackageOverview(multiVersionPackageName, '2.00.00.00');
    const resourcesPackageVersion2 = [
        database_utils_1.generateResource(resourceName1, overviewPackageVersion2, devices[0], devtools[0]),
        database_utils_1.generateResource(resourceName2, overviewPackageVersion2, devices[1], devtools[1])
    ];
    const overviews = [];
    return [
        {
...
function generateSearchablePackage(devices, devtools) {
    const packageOverview = database_utils_1.generatePackageOverview('searchablePackage', '1.23.45.00');
    const resources = searchableKeys.map(key => {
        ...
        const resource = database_utils_1.generateResource('searchable' + key, packageOverview, device, devtools[0]);
    ...
    database_utils_1.generateResource(searchableOneWordInDescription, packageOverview, devices[0], devtools[0]);
    resources.push(database_utils_1.generateResource(searchableOneWordInDescription, packageOverview, devices[0], devtools[0], {
...
function generateVariedResourceTypesPackage(devices, devtools) {
    const packageOverview = database_utils_1.generatePackageOverview('variedResourceTypes', '1.23.45.00');
    const subResource = database_utils_1.generateResource('subResource', packageOverview, devices[0], devtools[0]);
    subResource.fullPaths[0].push('folderWithResource');
    const resources = [
        database_utils_1.generateResource('cppFileByLink', packageOverview, devices[0], devtools[0], {
            resourceType: 'file',
            link: 'cppFile.cpp'
        }),
        database_utils_1.generateResource('cppFileByType', packageOverview, devices[0], devtools[0], {
            resourceType: 'file',
            link: 'cppFile',
            fileType: '.cpp'
        }),
        database_utils_1.generateResource('asmFile.asm', packageOverview, devices[0], devtools[0], {
            resourceType: 'file',
            link: 'asmFile.asm'
        }),
        database_utils_1.generateResource('markdown.md', packageOverview, devices[0], devtools[0], {
            resourceType: 'file',
            link: 'markdown.md',
            fileType: '.md'
        }),
        database_utils_1.generateResource('pdf.pdf', packageOverview, devices[0], devtools[0], {
            resourceType: 'file',
            link: 'pdf.pdf'
        }),
        database_utils_1.generateResource('pdf on ti.com', packageOverview, devices[0], devtools[0], {
        ...
        database_utils_1.generateResource('folderWithResource', packageOverview, devices[0], devtools[0], {
            link: `path/to/folder/resource`
        }),
        database_utils_1.generateResource('energiaProject (folderWithResource)', packageOverview, devices[0], devtools[0], {
            resourceType: 'project.energia',
            link: 'energiaFile.ino'
        }),
        database_utils_1.generateResource('custom_icon_overrides_md_icon.md', packageOverview, devices[0], devtools[0], {
            resourceType: 'file',
            link: 'anotherMarkdown.md',
            fileType: '.md',
            icon: 'path/to/icon/icon_s_forum_a.png'
        }),
        database_utils_1.generateResource('external_webpage_with_no_extension', packageOverview, devices[0], devtools[0], {
            resourceType: 'web.page',
    ...
    const overviews = [
        database_utils_1.generateOverview('folderWithResource', packageOverview, {
...
function generateVariedFiltersPackage(devices, devtools) {
    const packageOverview = database_utils_1.generatePackageOverview('variedFilterFields', '1.23.45.00');
    const categories = [[database_utils_1.firstFolderInAllPackages, packageOverview.name, 'Examples']];
    const resources = [];
    resources[0] = database_utils_1.generateResource('CCS example', packageOverview, devices[0], devtools[0], {
    ...
    resources[1] = database_utils_1.generateResource('Energia example', packageOverview, devices[0], devtools[0], {
    ...
    resources[2] = database_utils_1.generateResource('IAR example', packageOverview, devices[0], devtools[0], {
    ...
    resources[3] = database_utils_1.generateResource('CCS example (projectSpec)', packageOverview, devices[0], devtools[0], {
    ...
    resources[4] = database_utils_1.generateResource('Source file', packageOverview, devices[0], devtools[0], {
    ...
    resources[5] = database_utils_1.generateResource('Folder with source files', packageOverview, devices[0], devtools[0], {
    ...
    resources[6] = database_utils_1.generateResource('A document', packageOverview, devices[0], devtools[0], {
        description: 'asearchstring',
        resourceType: 'file',
        resourceClass: [],
        compiler: undefined,
    ...
    resources[7] = database_utils_1.generateResource('A Chinese document', packageOverview, devices[0], devtools[0], {
        resourceType: 'file',
        resourceClass: [],
        compiler: undefined,
        ide: undefined,
...
function generateSortNodePackage(devices, devtools) {
    const packageOverview = database_utils_1.generatePackageOverview('sortTest', '1.23.45.00');
    ...
    for (const folderData of testFoldersData) {
        ...
        resources.push(database_utils_1.generateResource(folderData.name, packageOverview, devices[0], devtools[0], {
        ...
        for (const child of folderData.children) {
            ...
            if (nodeType === 'file') {
                resources.push(database_utils_1.generateResource(childName, packageOverview, devices[0], devtools[0], {
                    fullPaths: [fullPath],
                    categories: [fullPath],
                    order: ++order,
                    customOrder
                ...
                overviews.push(database_utils_1.generateOverview(childName, packageOverview, {
                    fullPaths: [fullPath],
                    categories: [fullPath],
                    order: ++order,
                    customOrder
            ...
            if (nodeType === 'overview-folder' || nodeType === 'folder') {
                ...
                resources.push(database_utils_1.generateResource('a grandchild', packageOverview, devices[0], devtools[0], {
