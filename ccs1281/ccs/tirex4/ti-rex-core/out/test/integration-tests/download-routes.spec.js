...
describe('Download APIs', () => {
    before(async function () {
        ...
        nodes_utils_1.createContent(scriptsUtil.getMochaConfig().contentPath, testContent);
        return nodes_utils_1.updateDBContent(contentPath);
    });
    after(async function () {
        await fs.remove(contentPath);
    });
    describe(`${"api/nodeDownload" /* GET_NODE_DOWNLOAD */}`, () => {
        async function doDownload(resource) {
            const nodePath = nodes_utils_1.toNodePath([...resource.fullPaths[0], resource.name]);
            const dbId = await nodes_utils_1.getDbId(nodePath);
        ...
        it(`should download a node's local file`, async () => {
            ...
            expect_1.expect(res.text).to.deep.equal(fileContent);
        });
        ...
        it(`should download a node's local linkForDownload file`, async () => {
            ...
            expect_1.expect(res.text).to.deep.equal(linkForDownloadFileContent);
        });
        it(`should download a node's external file`, async () => {
            ...
            expect_1.expect(res).to.redirectTo(resource.link);
        });
        it(`should download a node's external linkForDownload file`, async () => {
            ...
            const res = await doDownload(resource);
            expect_1.expect(res).to.redirectTo(resource.linkForDownload.any);
        });
    });
});
async function getDownloadedNode(query) {
    return nodes_utils_1.makeDownloadImportRequest(`${"api/nodeDownload" /* GET_NODE_DOWNLOAD */}`, query);
}
...
function generateMetadata() {
    ...
    const packageOverview = database_utils_1.generatePackageOverview('downloadTest', '1.23.45.00');
    const resources = [
        database_utils_1.generateResource('local test file', packageOverview, devices[0], devtools[0], {
            resourceType: 'file',
            link: fileName
        }),
        database_utils_1.generateResource('local test file with invalid char', packageOverview, devices[0], devtools[0], {
            resourceType: 'file',
            link: fileNameWithInvalidChar
        }),
        database_utils_1.generateResource('local linkForDownload test file', packageOverview, devices[0], devtools[0], {
            resourceType: 'file',
            link: fileName,
            linkForDownload: {
        ...
        database_utils_1.generateResource('external link', packageOverview, devices[0], devtools[0], {
            resourceType: 'file',
            // make the local file look like an external one ...
            link: `${scriptsUtil.mochaServer}content/${fileName}`,
            linkType: 'external'
        }),
        database_utils_1.generateResource('external linkForDownload', packageOverview, devices[0], devtools[0], {
            resourceType: 'file',
            linkForDownload: {
                // make the local file look like an external one ...
        ...
        packages: [
            {
                ...
                resources,
                overviews,
                simpleCategories: database_utils_1.generateSimpleCategories(resources, packageOverview)
            }
        ]
    };
}
