...
async function getDuplicateLinks(nodeDbId, sqldb) {
    ...
    if (!(await fs.pathExists(path.join(vars_1.Vars.SEO_PATH, `${packagePublicUid}__duplicateLinks`)))) {
        return null;
    }
    const duplicateLinks = await fs.readJson(path.join(vars_1.Vars.SEO_PATH, `${packagePublicUid}__duplicateLinks`));
...
function handlePublicId(req, _res, packageGroups, _packages) {
    const publicIdData = public_id_helpers_1.getPublicIdData(req.query.node, packageGroups);
    if (!publicIdData) {
        return null;
    }
