...
async function doGetRequest(url, json = false) {
    const data = await doRequest(callback => request_1.getRequest().get(url, callback), url, json);
    return data;
}
exports.doGetRequest = doGetRequest;
async function doPostRequest(url, body, json = false) {
    const data = await doRequest(callback => request_1.getRequest().post(url, { body, json: true }, callback), url, json);
    return data;
}
exports.doPostRequest = doPostRequest;
async function doFormDataPostRequest(url, body, json = false) {
    const data = await doRequest(callback => request_1.getRequest().post(url, { formData: body }, callback), url, json);
    return data;
}
exports.doFormDataPostRequest = doFormDataPostRequest;
async function doDeleteRequest(url, json = false) {
    const data = await doRequest(callback => request_1.getRequest().delete(url, callback), url, json);
    return data;
}
