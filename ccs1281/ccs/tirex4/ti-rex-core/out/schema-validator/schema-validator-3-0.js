...
class SchemaValidator {
    constructor() {
        ...
        this.devtoolFiles = [];
        this.macrosFiles = [];
        this.settings = { rootFolders: '' };
        this.isSummary = true;
    }
    async config(logger, config) {
        this.logger = logger;
        const cfg = await fs.readJson(util_2.resolvePath(config.cfg));
    ...
    async validate() {
        ...
        this.logIntro(title);
        this.pkgDirs = [];
        const { packageFolders } = await package_helpers_1.getPackageFolders(this.rootDir);
        ...
        for (const pkgDir of this.pkgDirs) {
            const allPackageTirexJson = await package_helpers_1.getAllPackageTirexJsonPaths(pkgDir);
    ...
    async validatePackage(pkgDir, pkgData, modulePrefix) {
        ...
        const allPackageTirexJsonPaths = await package_helpers_1.getAllPackageTirexJsonPaths(pkgDir);
        const pkgFile = modulePrefix
            ? allPackageTirexJsonPaths.find(item => modulePrefix && item.includes(modulePrefix))
            : await package_helpers_1.getPackageTirexJsonPath(pkgDir);
        if (!pkgFile) {
        ...
        const metadataDir = await dbBuilderUtils_1.getMetadataDir(this.rootDir, pkgDir);
        const pkgMetadataDir = path.join(pkgDir, metadataDir);
        // always process macros first
        const macrosFiles = util_1.findMacrosFiles(pkgMetadataDir);
        this.macrosFiles.push(macrosFiles);
        macrosFiles.forEach(_file => {
            this.validateMacrosJson(_file);
        });
        const deviceFiles = util_1.findDeviceFiles(pkgMetadataDir);
        ...
        const devtoolFiles = util_1.findDevtoolFiles(pkgMetadataDir);
        ...
        const contentFiles = util_1.findContentFiles(pkgMetadataDir);
    ...
    runFileValidation(filePath, v, isMacroFile = false, pkgData) {
        ...
        try {
            ...
            let msg = e.message;
            msg = util_1.extendJsonErrorMsg(filePath, msg);
