...
class ConsoleLogger extends logging_1.BaseLogger {
    constructor(file) {
        super();
        this.recommended = (_message, _tags = []) => 
        // tslint:disable-next-line:semicolon
        this.logger.log(`[recommended] ${_message}`);
        this.gap = () => (this.output ? this.output.write('\n') : this.logger.log('\n'));
        if (file) {
...
function setPackageMetadataList(packageMetadataList, packageFolder, id, version, type) {
    packageMetadataList.set(dbBuilderUtils_1.formUid(id, version), {
        id,
        version,
        packageFolder,
        type
