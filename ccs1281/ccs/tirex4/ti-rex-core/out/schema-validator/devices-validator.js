...
class DevicesValidator extends base_validator_1.Validator {
    constructor(logger, isSummary, logLimit) {
        super('Devices', "devices.tirex.json" /* DEVICES */, logger, isSummary, logLimit);
    }
    ...
    validateRecord(record, idx) {
        ...
        util_1.runValidateField(this, record, schema_types_1.DEVICES_METADATA_SCHEMA);
    ...
    validateField(fieldName, record, schema) {
        let ret = this.result.clone();
        if (!schema) {
            schema = util_1.getSchema(schema_types_1.DEVICES_METADATA_SCHEMA, fieldName);
            if (!schema) {
