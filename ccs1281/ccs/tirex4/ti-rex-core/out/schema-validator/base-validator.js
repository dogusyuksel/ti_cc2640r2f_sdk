...
class BaseValidator {
    constructor(title, _name, logger, isSummary, logLimit) {
        ...
        this.logCount = 0;
        //
        this.result = new validate_result_1.ValidateResult();
    ...
    getIdxToLineMap(filePath) {
        try {
            ...
            this.idxToLineMap = util_1.jsonLineNum(filePath);
        }
        catch (err) {
    ...
    reportNotInSpec(fields, specs) {
        fields.forEach(field => {
            ...
            const suggestion = util_1.getSuggestion(field, specs);
    ...
    ruleInSpec(record, specs) {
        ...
        this.reportNotInSpec(props.filter(p => {
            return !util_1.isInList(p, specs) && !this.isIgnore(p);
        }), specs);
        return true;
    }
        ...
        if (typeof fieldVal === 'string') {
            ...
            if (fieldVal.length === 0) {
                if (field === 'mainCategories') {
                    util_1.updateRuleSetting(schema_types_1.gRuleSettings.NonEmptyContent, schema_types_1.Priority.warning);
                }
                this.log(schema_types_1.gRuleSettings.NonEmptyContent, undefined, field);
                return false;
            }
    ...
    ruleValidLocalPath(field, record) {
        ...
        if (fieldVal) {
            let failed = 0;
            const items = helpers_1.getQueryParamAsArray(fieldVal);
            items.forEach(item => {
                if (this.filePath) {
            ...
            items.forEach(item => {
                if (listed.indexOf(item) < 0) {
                    if (field === 'name' || warning) {
                        util_1.updateRuleSetting(schema_types_1.gRuleSettings.ItemListed, schema_types_1.Priority.warning);
                    }
    ...
    ruleValidCoreType(field, record) {
        ...
        let failed = 0;
        if (Array.isArray(coreTypes) &&
            (coreTypes.length > 0 && typeof coreTypes[0] !== 'string')) {
