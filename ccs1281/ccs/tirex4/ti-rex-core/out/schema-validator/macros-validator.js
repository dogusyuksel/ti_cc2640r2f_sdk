...
class MacrosValidator extends base_validator_1.Validator {
    constructor(logger, isSummary, logLimit) {
        super('Macros', "macros.tirex.json" /* MACROS */, logger, isSummary, logLimit);
    ...
    validateRecord(record, idx) {
        ...
        Object.keys(this.MACROS_METADATA_SCHEMA).forEach(fieldName => {
            const schema = util_1.getSchema(this.MACROS_METADATA_SCHEMA, fieldName);
    ...
    validateField(fieldName, record, schema) {
        let ret = this.result.clone();
        if (!schema) {
            schema = util_1.getSchema(this.MACROS_METADATA_SCHEMA, fieldName);
            if (!schema) {
...
MacrosValidator.Text = 'textmacro';
MacrosValidator.Array = 'arraymacro';
MacrosValidator.Set = 'setmacro';
