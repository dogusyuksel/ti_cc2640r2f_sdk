...
class UrlValidator extends base_validator_1.BaseValidator {
    constructor(logger, isSummary, logLimit) {
        ...
        this.validateCb = callbackifyAny_1.callbackifyAny(this.validate);
    }
    ...
    async checkRequest(link, fieldName) {
        try {
            await request_helpers_1.doGetRequest(link);
        }
        catch (err) {
    ...
    fieldNamesToVerify(filePath) {
        ...
        return ["buyLink" /* BUY_LINK */, "toolsPage" /* TOOLS_PAGE */];
    }
    async validate(rootDir) {
        ...
        const { packageFolders } = await package_helpers_1.getPackageFolders(rootDir);
        ...
        for (const packageFolder of packageFolders) {
            const metadataDir = await dbBuilderUtils_1.getMetadataDir(rootDir, packageFolder);
            const [data] = await fs.readJson(path.join(rootDir, packageFolder, metadataDir, vars_1.Vars.PACKAGE_TIREX_JSON));
            util_1.setPackageMetadataList(packageMetadataList, packageFolder, data.id, data.version, data.type);
            PackageList.push({
        ...
        const validityResult = await dbBuilder_1.packagePresentAndValidityChecks(PackageList, rootDir, this.logger, 'url');
        const { latestPkgMap } = validityResult;
        util_1.insertBlankLine(this.logger);
    ...
    async runFileValidation(rootDir, latestPkgMap, packageMetadataList) {
        for (const [latestPkgId, latestPkg] of latestPkgMap) {
            const pkg = packageMetadataList.get(dbBuilderUtils_1.formUid(latestPkgId, latestPkg.version));
            if (pkg) {
                const metadataDir = await dbBuilderUtils_1.getMetadataDir(rootDir, pkg.packageFolder);
                const files = (await resources_1.getFiles(metadataDir, rootDir, pkg.packageFolder, null)).filter(file => !file.endsWith(vars_1.Vars.PACKAGE_TIREX_JSON));
                if (pkg.type && pkg.type === 'devtools') {
                    if (await fs.pathExists(path.join(rootDir, pkg.packageFolder, metadataDir, "devtools.tirex.json" /* DEVTOOLS */))) {
                        files.push(path.join(metadataDir, "devtools.tirex.json" /* DEVTOOLS */));
                    }
                    if (await fs.pathExists(path.join(rootDir, pkg.packageFolder, metadataDir, "devtools-aux.tirex.json" /* DEVTOOLS_AUX */))) {
                        files.push(path.join(metadataDir, "devtools-aux.tirex.json" /* DEVTOOLS_AUX */));
                    }
                }
                if (files.length > 0) {
