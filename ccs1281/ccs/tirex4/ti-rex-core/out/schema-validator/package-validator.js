...
class PackageValidator extends base_validator_1.Validator {
    constructor(logger, isSummary, logLimit) {
        super('Package', "package.tirex.json" /* PACKAGE */, logger, isSummary, logLimit);
        this.pkgHeaderRecord = {};
        this.metaVer = { major: 0, minor: 0, patch: 0 };
        this.skipped = false;
        this.settings = {
            ignoreFields: [],
            ignorePackages: [],
    ...
    validateRecord(record, idx) {
        ...
        for (const fieldName of Object.keys(schema_types_1.PACKAGE_METADATA_SCHEMA)) {
            const schema = util_1.getSchema(schema_types_1.PACKAGE_METADATA_SCHEMA, fieldName);
    ...
    validateField(fieldName, record, schema) {
        let ret = this.result.clone();
        if (!schema) {
            schema = util_1.getSchema(schema_types_1.PACKAGE_METADATA_SCHEMA, fieldName);
            if (!schema) {
            ...
            case 'type':
                if (this.filePath) {
                    ...
                    if (record.type === schema_types_1.PackageType.DEVICES) {
                        ...
                        this.ruleFileExists(path.join(rpath, "devices.tirex.json" /* DEVICES */), fieldName);
                        this.ruleFileExistsNot(path.join(rpath, "devtools.tirex.json" /* DEVTOOLS */));
                    }
                    else if (record.type === schema_types_1.PackageType.DEVTOOLS) {
                        // Devtools package, should have devtools.tirex.json
                        this.ruleFileExists(path.join(rpath, "devtools.tirex.json" /* DEVTOOLS */), fieldName);
                        this.ruleFileExistsNot(path.join(rpath, "devices.tirex.json" /* DEVICES */));
                    }
    ...
    validateVersionFormat(field, record) {
        ...
        const match = versioning_1.valid(ver);
        if (match === null) {
    ...
    validateSupplements(field, record) {
        ...
        Object.keys(schema_types_1.SUPPLEMENTS_METADATA_SCHEMA).forEach(fieldName => {
            const schema = util_1.getSchema(schema_types_1.SUPPLEMENTS_METADATA_SCHEMA, fieldName);
            // basic field validation
    ...
    validateDependencies(field, record) {
        ...
        dependencies.forEach((dependency, idx) => {
            ...
            Object.keys(schema_types_1.DEPENDENCIES_METADATA_SCHEMA).forEach(fieldName => {
                const schema = util_1.getSchema(schema_types_1.DEPENDENCIES_METADATA_SCHEMA, fieldName);
                //  basic checking
