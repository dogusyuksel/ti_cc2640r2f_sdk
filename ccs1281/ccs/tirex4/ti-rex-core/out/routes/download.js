...
function getRoutes() {
    ...
    routes.get(`/${"api/nodeDownload" /* GET_NODE_DOWNLOAD */}`, executeRoute_1.executeRoute(getDownloadNodeData, downloadNode));
    routes.get(`/${"api/packageDownload" /* GET_PACKAGE_DOWNLOAD */}`, executeRoute_1.executeRoute(getPackageDownloadData, downloadPackage));
    return routes;
}
...
const downloadNode = async (req, res, dbResp) => {
    ...
    if (resource.linkForDownload) {
        const altLink = download_1.resolveDownloadLinkForOS(resource.linkForDownload, clientInfo);
        if (altLink) {
            file = altLink;
        }
        else {
...
const downloadPackage = async (req, res, dbResp) => {
    ...
    const platform = download_1.resolvePackageZipForOS(clientInfo.OS);
