...
function getRoutes(rex) {
    ...
    if (rex.vars.handoffServer) {
        ...
        // Primary APIs
        routes.post(`/${"api/add-package" /* POST_ADD_PACKAGE */}`, multer.any(), (req, res) => addPackage(req, res, handoffManager, logger));
        routes.delete(`/${"api/remove-package" /* DELETE_REMOVE_PACKAGE */}`, (req, res) => removePackage(req, res, handoffManager, logger));
        // Maintenance mode APIs
        routes.get(`/${"api/get-maintenance-mode" /* GET_GET_MAINTENACE_MODE */}`, (req, res) => getMaintenanceMode(req, res, handoffManager));
        routes.get(`/${"api/maintenance-mode" /* GET_MAINTENANCE_MODE */}`, (req, res) => maintenanceMode(req, res, handoffManager, logger));
        // Admin / server maintenance APIs
        routes.get(`/${"api/sync-packages" /* GET_SYNC_PACKAGES */}`, (_req, res) => syncPackages(res, handoffManager, logger));
        routes.get(`/${"api/cleanup-packages" /* GET_CLEANUP_PACKAGES */}`, (_req, res) => cleanupPackages(res, handoffManager, logger));
    }
    return routes;
}
...
function addPackage(req, res, handoffManager, logger) {
    try {
        ...
        if (req.body.entries && !error) {
            ...
            if (entries) {
                error = add_package_1.validateEntrySchema(entries);
            }
        }
        let success = false;
        if (error) {
...
async function maintanceModeOn(_req, res, handoffManager, logger) {
    ...
    if (state === "teardown" /* TEARDOWN */) {
        res.status(202);
        return res.send('Tearing down');
    }
    else if (state === "maintanceMode" /* MAINTANCE_MODE */) {
