...
function getRoutes(dinfra, config, desktopServer) {
    ...
    // use
    routes.use(express_1.static(path.join(vars_1.Vars.projectRoot, 'public')));
    routes.use('/components', components(config));
    routes.use('/content', getContentRoutes(dinfra, config, desktopServer));
    routes.use('/scripts', express_1.static(path.join(vars_1.Vars.projectRoot, 'node_modules') + path.sep));
    routes.use('/zips', express_1.static(vars_1.Vars.ZIPS_FOLDER + path.sep));
...
function getContentRoutes(dinfra, config, desktopServer) {
    ...
    if (config.mode === 'localserver') {
        ...
        if (!config.serveContentFromFs) {
            ...
            routes.use('/', express_1.static(vars_1.Vars.CONTENT_BASE_PATH + path.sep));
        }
    }
    return routes;
}
    ...
    return async (req, res) => {
        try {
            ...
            if (await fs.pathExists(filePath)) {
                ...
                await promise_utils_1.promisePipe([fs.createReadStream(filePath), res]);
            }
            else if (state.serverState.useRemoteContent) {
                await promise_utils_1.promisePipe([
                    request_1.getRequest().get({ url: vars_1.Vars.REMOTESERVER_BASEURL + req.originalUrl }),
                    res
                ]);
            }
            else {
