...
describe('replaceCharactersWithHtmlEncoding', () => {
    it('Should replace special characters with HTML Encoding', () => {
        ...
        const result = util_1.replaceCharactersWithHtmlEncoding(testString);
        chai_1.expect(result).to.equal('Wow&#33; Nice symbols &#33;&#64;&#35;&#36;&#37;&#94;&#38;&#42;&#40;&#41;');
    });
});
...
describe.skip('Search Engine Optimization Tests', function () {
    ...
    before(async () => {
        ...
        packageGroups = (await sqldbImporter_1.discoverPackageGroups(dbFolder)).packageGroupsDef;
        chai_1.expect(packageGroups.length).to.be.gt(0);
        ...
        const rex = rex_1.Rex({ dinfraLogger, vars });
        seoFunction = handle_search_bot_request_1.handleSearchBotRequest(logger, rex.vars);
        contentRedirectFunction = seo_1.redirectContentRoutes(logger);
        ...
        const tmpFolder = test_helpers_1.getUniqueFolderName('seo');
    ...
    it('Should be hooked up as middleware, and should redirect to latest version', async () => {
        ...
        chai_1.expect(result).to.redirect;
        chai_1.expect(result).to.redirectTo(`${scriptsUtil.mochaServer}explore/node?node=APm1fpvzJ4Qhq8euuP8R5Q__41Qk4L1__LATEST`);
    ...
    it("Should send noindex if it's not a node request", async () => {
        const result = await chai
            ...
            .set('user-agent', 'Googlebot');
        chai_1.expect(result).to.have.status(200);
        chai_1.expect(result).to.have.header('X-Robots-Tag', 'noindex');
    });
    ...
    it("Should send 404 if node id doesn't exist", async () => {
        const result = await chai
            ...
            .set('user-agent', 'Googlebot');
        chai_1.expect(result).to.have.status(404);
    });
    it('Should redirect content routes to an appropirate explore route', async () => {
        ...
        chai_1.expect(next.called, 'false');
        chai_1.expect(res.redirect.called, 'true');
        chai_1.expect(res.redirect.firstCall.args[0], '301');
        chai_1.expect(res.redirect.firstCall.args[1], '/explore/node?node=AFK-JYAlL94KJhzI8xCMIQ__krol.2c__LATEST');
    });
    it('Should not insert a description meta tag if it receives an html file with a description meta tag', async () => {
        ...
        chai_1.expect(result.toString()).to.equal(data);
        await fs.unlink(path.join(vars_1.Vars.CONTENT_BASE_PATH, 'SeoTest.html'));
    ...
    it("Should insert a description meta tag if the file doesn't have a description meta tag", async () => {
        ...
        await seoFunction(request, response);
        chai_1.expect(response.send.calledOnce).to.equal(true);
        chai_1.expect(response.send.firstCall.args[0]).to.equal('<html><head><meta name="description" content="device1,board1,ccs"><title>Testing</title></head><body>Fake body data </body></html>');
        await fs.unlink(path.join(vars_1.Vars.CONTENT_BASE_PATH, 'SeoTest.html'));
    });
    it('Should stream a pdf to the response', async () => {
        ...
        await seoFunction(request, response);
        const result = fs.readFileSync(tmpFile);
        chai_1.expect(result.toString()).to.equal(pdfData);
    ...
    it('Should send a text file with a description meta tag inserted into the header', async () => {
        ...
        chai_1.expect(response.send.calledOnce).to.equal(true);
        chai_1.expect(response.send.firstCall.args[0].replace(/\s/g, '')).to.equal(expectedResult.replace(/\s/g, ''));
    ...
    it('Should call getProjectDescriptionandKeywords for a projectSpec', async () => {
        ...
        chai_1.expect(response.send.calledOnce).to.equal(true);
        chai_1.expect(response.send.firstCall.args[0].replace(/\s/g, '')).to.equal(expectedResult);
    });
    ...
    it('Should return a 404 for external links', async () => {
        ...
        const response = generateFakeResponse();
        await seoFunction(request, response);
        chai_1.expect(response.sendStatus.calledOnce).to.equal(true);
        chai_1.expect(response.sendStatus.firstCall.args[0]).to.equal(404);
    });
    ...
    it('Should add a response header to the canonical link if the resource is a duplicate', async () => {
        ...
        const result = await chai
            ...
            .set('user-agent', 'Googlebot');
        chai_1.expect(result).to.have.status(200);
        chai_1.expect(result).to.have.header('link', `<${scriptsUtil.mochaServer}/explore/node?node=fakeFullPathId__41Qk4L1__LATEST>; rel="canonical"`);
        await fs.unlink(path.join(vars_1.Vars.CONTENT_BASE_PATH, 'SeoTest.html'));
    });
});
class FakeResponse extends fs.WriteStream {
    constructor() {
        super(...arguments);
        this.status = sinon.stub().returns(true);
        this.sendStatus = sinon.stub().returns(true);
        this.send = sinon.stub().returns(true);
        this.redirect = sinon.stub().returns(true);
        this.setHeader = sinon.stub().returns(true);
    }
}
async function generateFakeDuplicateLinksFile(resourcePackages) {
    const fileName = `${resourcePackages.packageOverview.packageUId}__duplicateLinks`;
    const fakeData = {};
    for (const resource of resourcePackages.resources) {
        ...
        for (let i = 0; i < resource.fullPathsPublicIds.length; i++) {
            ...
            fakeData[resource.link].duplicatePaths[fullPathsPublicId] = resource.fullPaths[i].join('/');
        }
    }
...
function generateVariedResourceTypesPackage(devices, devtools) {
    ...
    const resources = [
        database_utils_1.generateResource('TestHtmlWithMetaTag', packageOverview, devices[0], devtools[0], {
            linkType: 'local',
            link: 'SeoTest.html',
            resourceType: 'file'
        }),
        database_utils_1.generateResource('projectSpecTest', packageOverview, devices[0], devtools[0], {
            linkType: 'local',
            link: 'fakeLink',
        ...
        database_utils_1.generateResource('externalLinkTest', packageOverview, devices[0], devtools[0], {
            link: 'fake link',
            linkType: 'external'
        }),
        database_utils_1.generateResource('overviewDescriptionTest', packageOverview, devices[0], devtools[0], {
            shortDescription: 'Fake Short Description',
            description: 'Fake Description'
        }),
        database_utils_1.generateResource('fileTest', packageOverview, devices[0], devtools[0], {
            linkType: 'local',
            link: 'SeoTest.c',
            resourceType: 'file',
            description: 'Fake Description'
        }),
        database_utils_1.generateResource('pdfTest', packageOverview, devices[0], devtools[0], {
            linkType: 'local',
            link: 'SeoTest.pdf',
            resourceType: 'file'
        })
...
function generateEmptyResource(name, pkgOverview, overrides = {}) {
    const minResource = {
        ...
        linkType: 'external',
        _id: database_utils_1.generateId()
    };
    const finalResource = {
        ...minResource,
        ...overrides
