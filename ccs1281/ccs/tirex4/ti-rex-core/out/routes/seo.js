...
const ejsRenderFile = util_1.promisify(ejs.renderFile);
///////////////////////////////////////////////////////////////////////////////
/// Code
///////////////////////////////////////////////////////////////////////////////
function getRoutes(logger, vars) {
    ...
    routes.use(handle_search_bot_request_1.handleSearchBotRequest(logger, vars));
...
function redirectContentRoutes(logger) {
    ...
    return async (req, res, next) => {
        try {
            const isBot = util_2.isSearchBot(req);
            if (!(isBot || util_2.isFromSearchResults(req))) {
                return next();
            }
            ...
            if (contentLink && contentLink.length >= 2) {
                ...
                const linkPrefix = util_2.getLinkPrefix();
...
function handleDuplicateContent(logger) {
    return async (req, res, next) => {
        if (util_2.isFromSearchResults(req) && req.sqldb && req.query.node) {
            try {
                ...
                const info = util_2.handlePublicId(req, res, packageGroups, packages);
                if (!info) {
                    return next();
                }
                ...
                const nodeDbId = await util_2.getNodeDbIdforLatestVersion(req.sqldb, packageGroupPublicId, nodePublicId, packagePublicId);
                if (!nodeDbId) {
                    return next();
                }
                const link = await util_2.getDuplicateLinks(nodeDbId, req.sqldb);
                if (!link) {
                    return next();
                }
                const templateData = generateTemplateData(link, packageGroupPublicId, `${req.protocol}://${req.headers.host}${util_2.getLinkPrefix()}`);
