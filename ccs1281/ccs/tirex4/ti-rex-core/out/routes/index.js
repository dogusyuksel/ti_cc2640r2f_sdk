...
function getIndexFile(req, res, dinfra) {
    promise_utils_1.shouldNeverThrow(async () => {
        try {
            const sqldb = await executeRoute_1.getSqldbFromRequest(req, dinfra);
            const rootNodeDbId = await nodes_1.getRootNode(sqldb);
            const softwareNodePublicId = dbBuilderUtils_1.createPublicIdFromTreeNodePath(vars_1.Vars.TOP_CATEGORY.software.text);
            ...
            executeRoute_1.sendError(res, err);
        }
    });
}
