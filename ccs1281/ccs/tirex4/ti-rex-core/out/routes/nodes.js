...
function getRoutes() {
    ...
    routes.get(`/${"api/filteredTableItemsData" /* GET_FILTERED_TABLE_ITEMS_DATA */}`, executeRoute_1.executeRoute(getFilteredTableItemsData));
    routes.get(`/${"api/tableViewFilters" /* GET_TABLE_VIEW_FILTERS */}`, executeRoute_1.executeRoute(getAvailableTableViewFilters));
    routes.get(`/${"api/nodeDataForTableItemVariant" /* GET_NODE_DATA_FOR_TABLE_ITEM_VARIANT */}`, executeRoute_1.executeRoute(getNodeDataForTableItemVariant));
    routes.get(`/${"api/expandedFilteredDescendantNodesData" /* GET_EXPANDED_FILTERED_DESCENDANT_NODES_DATA */}`, executeRoute_1.executeRoute(getExpandedFilteredDescendantNodesData));
    routes.get(`/${"api/filteredChildrenNodeIds" /* GET_FILTERED_CHILDREN_NODE_IDS */}`, executeRoute_1.executeRoute(getFilteredChildrenNodeIds));
    routes.get(`/${"api/nodesData" /* GET_NODES_DATA */}`, executeRoute_1.executeRoute(getNodesData));
    routes.get(`/${"api/nodeExtendedData" /* GET_NODE_EXTENDED_DATA */}`, executeRoute_1.executeRoute(getNodeExtendedData));
    routes.get(`/${"api/filterOptions" /* GET_FILTER_OPTIONS */}`, executeRoute_1.executeRoute(getFilterOptions));
    routes.get(`/${"api/boardsDevices" /* GET_BOARDS_AND_DEVICES */}`, executeRoute_1.executeRoute(getBoardAndDeviceData));
    routes.get(`/${"api/rootNode" /* GET_ROOT_NODE */}`, executeRoute_1.executeRoute(getRootNode));
    routes.get(`/${"api/searchSuggestions" /* GET_SEARCH_SUGGESTIONS */}`, executeRoute_1.executeRoute(getSearchSuggestions));
    routes.get(`/${"api/getNodeInfoForResourceId" /* GET_NODE_INFO_FOR_RESOURCE_ID */}`, executeRoute_1.executeRoute(getNodeInfoForResourceId));
    routes.get(`/${"api/getNodeInfoForGlobalId" /* GET_NODE_INFO_FOR_GLOBAL_ID */}`, executeRoute_1.executeRoute(getNodeInfoForGlobalId));
    routes.get(`/${"api/nodePublicIdToDbId" /* GET_NODE_PUBLIC_ID_TO_DB_ID */}`, executeRoute_1.executeRoute(lookupNodeDbId));
    routes.get(`/${"api/rex3LinkToDbId" /* GET_REX3_LINK_TO_DB_ID */}`, executeRoute_1.executeRoute(lookupNodeDbIdRex3));
    return routes;
}
...
async function lookupNodeDbId(sqldb, query) {
    ...
    else if (query.toDbIdType === "ToDbIdLatest" /* TO_DB_ID_GROUP_LATEST */ ||
        query.toDbIdType === "ToDbIdNotLatest" /* TO_DB_ID_GROUP_NOT_LATEST */) {
    ...
    switch (query.toDbIdType) {
        case "ToDbIdNoGroup" /* TO_DB_ID_NO_GROUP */:
            nodeDbId = await sqldb.lookupNodeDbIdOnPublicId(query.nodePublicId, null, null);
            break;
        case "ToDbIdNotLatest" /* TO_DB_ID_GROUP_NOT_LATEST */: {
            const { id, version } = dbBuilderUtils_1.splitUid(query.packageGroupPublicUid);
            nodeDbId = await sqldb.lookupNodeDbIdOnPublicId(query.nodePublicId, { publicId: id, version }, query.packagePublicId || null);
            break;
        }
        case "ToDbIdLatest" /* TO_DB_ID_GROUP_LATEST */: {
            const { id } = dbBuilderUtils_1.splitUid(query.packageGroupPublicUid);
...
async function lookupNodeDbIdRex3(sqldb, query) {
    ...
    const nodePublicId = dbBuilderUtils_1.createPublicIdFromTreeNodePath(query.linkField);
    ...
    if (topLevel === 'Software') {
        ...
        if (indexOfPackageName < linkArr.length) {
            ...
            if (latestPackage &&
                latestPackage.packageType === "softwareMain" /* SOFTWARE_MAIN */) {
    ...
    try {
        const nodeDbId = packageGroupPublicUid
            ? await lookupNodeDbId(sqldb, {
                ...
                toDbIdType: "ToDbIdLatest" /* TO_DB_ID_GROUP_LATEST */
            })
            : await lookupNodeDbId(sqldb, {
                nodePublicId,
...
async function getNodeInfoForResourceId(sqldb, query) {
    ...
    result.forEach(item => {
        item.packageGroup.publicId = dbBuilderUtils_1.encodePackageGroupPublicId(item.packageGroup.publicId);
    });
    return result;
}
...
async function getExpandedFilteredDescendantNodesData(sqldb, query) {
    ...
    const packageNodesOnly = helpers_1.getQueryParamAsArray(query.filterNode || []).includes(response_data_1.Nodes.NodeType.PACKAGE_FOLDER_NODE);
...
async function makeTableItemData(sqldb, tableViewRow) {
    ...
    if (!dbTypes_1.isResourceSubClass(tableViewRow.subClass)) {
        throw new rexError_1.RexError({
    ...
    if (presentationData.resourceType === 'packageOverview') {
        ...
        if (actualFileType && mimeType && mimeType !== NOT_FOUND) {
            if (mimeType === 'application/pdf' ||
                ...
                nodeData.descriptor.viewConstraint = "ExternalOnly" /* EXTERNAL_ONLY */;
            }
            else {
                // if neither tirex nor browser can render, allow download only
                nodeData.descriptor.viewConstraint = "DownloadOnly" /* DOWNLOAD_ONLY */;
            }
        }
        else {
        ...
        if (!nodeData.descriptor.viewConstraint) {
            if (_.size(presentationData.viewLimitations) === 1 &&
                ...
                nodeData.descriptor.viewConstraint = "DesktopExternalOnly" /* DESKTOP_EXTERNAL_ONLY */;
            }
            else {
...
function makeNodeExtendedData(nodeType, nodeIdClient, nodeDbIdPath, resourceRecord) {
    ...
    if (nodeType === NodeType.FOLDER_NODE &&
        ...
        data.link = routes_1.makeLink(resourceRecord.link, resourceRecord.linkType);
        if (resourceRecord.linkType === 'local') {
            data.linkType = "Internal" /* INTERNAL */;
        }
...
function makeOverviewType(overviewRecord) {
    if (overviewRecord.link) {
        const result = {
            overviewLink: routes_1.makeLink(overviewRecord.link, 'local'),
            overviewType: "OverviewLink" /* OVERVIEW_LINK */
        };
        return result;
    }
        ...
        const result = {
            ...
            overviewImage: overviewRecord.image
                ? routes_1.makeLink(overviewRecord.image, 'local')
                : undefined,
...
function convertFilterParamsForDB(sqldb, filterParams, options) {
    ...
    for (const key of Object.keys(filterParams)) {
        if (key.startsWith('filter') && filterParams[key] && filterParams[key] !== '') {
            const values = helpers_1.getQueryParamAsArray(filterParams[key]);
            switch (key) {
...
function determineIsImportable(resourceType) {
    return resourceType ? dbTypes_1.isResourceTypeImportableByCCS(resourceType) : false;
}
    ...
    function getIcon(fileType, resourceType, isLeaf) {
        ...
        if (isLeaf) {
            if (testExtension(fileType, ['.c'])) {
                return "SourceCode_C" /* SOURCE_CODE_C */;
            }
            else if (testExtension(fileType, ['.h'])) {
                return "SourceCode_H" /* SOURCE_CODE_H */;
            }
            else if (testExtension(fileType, ['.cpp'])) {
                return "SourceCode_CPP" /* SOURCE_CODE_CPP */;
            }
            else if (testExtension(fileType, ['.asm'])) {
                return "SourceCode_ASM" /* SOURCE_CODE_ASM */;
            }
            else if (testExtension(fileType, ['.cmd'])) {
                return "SourceCode_CMD" /* SOURCE_CODE_CMD */;
            }
            else if (testExtension(fileType, ['.md'])) {
                return "Markdown" /* MARKDOWN */;
            }
        ...
        if (resourceType === 'project.ccs' || resourceType === 'projectSpec') {
            return "Project_CCS" /* PROJECT_CCS */;
        }
        else if (resourceType === 'project.energia') {
            return "Project_Energia" /* PROJECT_ENERGIA */;
        }
        else if (resourceType === 'project.iar') {
            return "Project_IAR" /* PROJECT_IAR */;
        }
