...
/******************************************************************************
 ...
 @brief This file contains lib search source. The main purpose of this tool
        ...
        This script was tested using Python v3.6 64 bit for Windows. An
...
def input_arg_expansion(arg):
    ...
    if not os.path.exists(arg):
        raise Exception('Error: Cannot find input file \'{}\'\n'.format(arg))
...
def strip_comments(file_str):
    ...
    if opening_regex_match:
        ...
        possible_nested_comment_regex_match = re.search(
            r'/\*', file_str[inc_end_index:])
        ...
        if (possible_nested_comment_regex_match and
                closing_regex_match and
...
def parse_lib_name(lib_name):
    ...
    while idx < len(define_list):
        if define_list[idx] == 'ext':
            output_list.append('{}_{}'.format(
                define_list[idx], define_list[idx + 1]))
            idx += 2
        else:
            ...
            for child in root:
                if child.tag == 'layer':
                    ...
                    for sub in subs:
                        ...
                        input_features_class.add_regex_pair(
                            re.compile(search), replace)
                    ...
                    for sub in subs:
                        search = sub.findtext('search')
                        replace = sub.findtext('replace')
                        input_regex_class.add_regex_pair(
                            re.compile(search), replace)
...
def lib_search(opt, lib_dir, xml_path):
    ...
    for input_features_class in input_features_class_list:
        ...
        opt_defines_list = parse_opt(
            opt, input_features_class, input_regex_class)

        for root, dirs, files in os.walk(os.path.abspath(os.path.join(
                lib_dir, layer))):
            for file in files:
                if file.endswith('.a'):
                    lib_defines_list = parse_lib_name(file[:-2])
                    ...
                    if final_opt_list == lib_defines_list:
                        full_lib_path = os.path.abspath(os.path.join(
                            root, file))
...
def generate_symbols_file(opt, src_dir, dst_dir):
    ...
    shutil.copy2(
        os.path.join(src_dir, symbol_filename),
...
def main(args):
    ...
    final_libs_list = lib_search(opt, lib_dir, args.xml)
    ...
    if final_libs_list:
        if generate_cmd(args.cmd, final_libs_list):
            ...
            print('<<< Not modifying {}. Stack configurations have not '
                  'changed. >>>'.format(os.path.basename(args.cmd)))
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        prog='lib_search', description=script_description)
    ...
    parser.add_argument(
        '--opt',
        required=True,
        help='Local project configuration file for stack project')

    parser.add_argument(
        '--lib-dir', required=True, help='Directory containing library files')

    parser.add_argument(
        '--cmd',
        required=True,
        help='Linker command file that will be written to')

    parser.add_argument('--xml', required=True, help='path to features xml')

    parser.add_argument(
        '--sym-dir',
        required=True,
        help='Directory containing symbols')
