...
#define LL_CONN_REQ_PDU( pktHdr )        (((pktHdr) & 0x0F) == LL_PKT_TYPE_CONNECT_REQ)
...
#define HW_FAIL_UNABLE_TO_SCHEDULE                     0x8F
...
// Scanner Channels
#define LL_SCAN_ADV_BASE_CHAN                          37

#define LL_SCAN_ADV_CHAN_37                            (LL_SCAN_ADV_BASE_CHAN+0)
#define LL_SCAN_ADV_CHAN_38                            (LL_SCAN_ADV_BASE_CHAN+1)
#define LL_SCAN_ADV_CHAN_39                            (LL_SCAN_ADV_BASE_CHAN+2)
...
// Byte 3 - Byte 7
#define LL_FEATURE_RESERVED06                          0x01
#define LL_FEATURE_RESERVED07                          0x02
#define LL_FEATURE_RESERVED08                          0x04
#define LL_FEATURE_RESERVED09                          0x08
#define LL_FEATURE_RESERVED10                          0x10
#define LL_FEATURE_RESERVED11                          0x20
#define LL_FEATURE_RESERVED12                          0x40
#define LL_FEATURE_RESERVED13                          0x80
...
#define LL_FEATURE_MASK_BYTE2                          LL_FEATURE_NONE
#define LL_FEATURE_MASK_BYTE3                          LL_FEATURE_NONE
...
#define LL_PHY_SLOWEST_PHY                             LL_PHY_1_MBPS
...
// V5.0 - 2Mbps PHY Control Procedure Specific Flags
...
// Connection Combine State Type
typedef struct llConnCombineData_t llConnCombineState_t;

// Data Combine State
struct llConnCombineData_t
{
  uint16 size;
  uint16 pduSize;
  uint8  state;
...
// Data Combine per LL Connection
typedef struct
{
  llConnCombineState_t *llDataCtrlToHost;
} llConnsData_t;
...
typedef struct
{
  ...
  uint8       taskID;                                // user task ID to send task event
  uint16      taskEvent;                             // user event to send at end of scan event
#endif // SCAN_EVT_NOTICE_CFG
} scanInfo_t;
...
typedef struct
{
  ...
  uint8       trxPhy;                                // 1Mbps, 2Mbps, Coded
...
#if defined(BLE_TICKETS) && (BLE_TICKETS & BLESTACK_1079)
#define LL_TEST_MODE_TP_SEC_SLA_BI_05                30
#else // !BLESTACK_1079
#define LL_TEST_MODE_TP_SEC_SLV_BI_05                30
#endif // BLESTACK_1079
...
#define LL_TEST_MODE_TP_CON_MAS_BV_03                36
#define LL_TEST_MODE_TP_CON_MAS_BV_04                37
#define LL_TEST_MODE_TP_CON_MAS_BV_05                38
#define LL_TEST_MODE_TP_CON_SLA_BV_04                39
// V5.0 - 2Mbps
...
extern llConnsData_t llCombineState;                  // link layer data combine table
...
extern uint8         sizeOfllCombineData;             // size Info of the llConnCombineState_t
...
// peer features to be ignored
extern uint8 featureSetMask[ LL_MAX_FEATURE_SET_SIZE ];
...
extern uint8              llSetupLenCtrlPkt_sPatch( llConnState_t *, uint8 ); // M, S
...
extern void                 llAlignToNextEvent_sPatch( llConnState_t *connPtr );
...
extern uint16               llTime2Octets( uint8, uint16, uint8 );
extern uint16               llOctets2Time( uint8, uint16, uint8 );
extern llConnState_t        *llDataGetConnPtr( uint8 );
extern llConnCombineState_t *llDataGetCombinePtr( uint8 );
...
extern uint8       		    llGetNextOrPreviousScanChannelIndex( uint8 getNextOrPrevious ); 
extern uint8                llGetFirstScanChannelIndex( void ); 
