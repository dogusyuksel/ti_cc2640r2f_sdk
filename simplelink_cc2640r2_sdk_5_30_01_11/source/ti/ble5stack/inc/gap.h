 ...
 *
 * A reasonCode (status of connection complete) of something other than LL_STATUS_SUCCESS
 ...
 *
 * A reasonCode Error (one of the above) is converted to a packet status of bleGAPConnNotAcceptable (0x31).
 * A reasonCode Success is converted to a packet status of HCI_SUCCESS.
 */
typedef struct
{
  osal_event_hdr_t  hdr;       //!< @ref GAP_MSG_EVENT and status
...
* @param connHandle if @ref CONNHANDLE_ALL, apply to all connections. <br>
